# H4-6A two-fresh-Endpoint qualification

Status: **active live qualifier for one selected immutable functional-alpha
candidate.** A passing run is required before H4-6A may claim that two fresh
...
`/tmp/ardents-h4-6a-two-endpoints-<hex>` root. Both Endpoint processes run as
...
```powershell
make qualification-h4-6a-two-endpoints `
  H4_6A_ARCHIVE='C:/absolute/outside-repository/candidate-linux-amd64.tar.gz' `
  H4_6A_ARCHIVE_SHA256='<64-lowercase-hex>' `
  H4_6A_MANIFEST_PIN='<64-lowercase-hex>' `
  H4_6A_ENDPOINT_SHA256='<64-lowercase-hex>' `
  H4_6A_CONTROL_SHA256='<64-lowercase-hex>' `
  H4_6A_COHORT='<exact-cohort>' `
  H4_6A_RELEASE='<exact-release>' `
  H4_6A_AT='<UTC-RFC3339-second>' `
  H4_6A_VPS='<literal-IPv4>' `
  H4_6A_SSH_KEY='C:/absolute/private-key' `
  H4_6A_VPS_USER='<exact-SSH-user>' `
  H4_6A_EVIDENCE='C:/absolute/outside-repository/previously-absent-directory'
```

`H4_6A_AT` is one UTC second such as `2026-08-29T12:00:00Z`; it must be within
...
The `H4_6A_EVIDENCE` path must be absolute, outside the repository, and absent
