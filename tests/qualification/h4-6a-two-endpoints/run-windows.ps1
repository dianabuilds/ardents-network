...
function Test-H46AUbuntuEvidence([string]$Root, [string]$ExpectedCohort, [string]$ExpectedRelease) {
    ...
    $summary = "schema=ardents-h4-6a-two-endpoints-summary-v1`nendpoint_a_release=release-accepted`nendpoint_b_release=release-accepted`nendpoint_a_control_release=release-accepted`nendpoint_b_control_release=release-accepted`nrelease_floor_inventories=canonical-byte-for-byte-equal`nlifecycle_identity=selected-cohort-release`n"
    $exactText = [ordered]@{
        ...
        'verdict.txt' = "h4-6a-two-fresh-endpoints=accepted`n"
...
function Invoke-H46ASelfTest {
    ...
    try {
        ...
        Write-Utf8 (Join-Path $root 'qualification-summary.txt') "schema=ardents-h4-6a-two-endpoints-summary-v1`nendpoint_a_release=release-accepted`nendpoint_b_release=release-accepted`nendpoint_a_control_release=release-accepted`nendpoint_b_control_release=release-accepted`nrelease_floor_inventories=canonical-byte-for-byte-equal`nlifecycle_identity=selected-cohort-release`n"
        ...
        Write-Utf8 (Join-Path $root 'verdict.txt') "h4-6a-two-fresh-endpoints=accepted`n"
        ...
        Write-Output 'h4-6a-run-windows-self-test=accepted'
    }
    finally {
...
Write-Json (Join-Path $evidencePath 'requested-inputs.json') ([ordered]@{
    schema = 'ardents-h4-6a-two-endpoints-request-v1'
...
if ([Environment]::OSVersion.Platform -ne [PlatformID]::Win32NT) {
    throw 'H4-6A two-Endpoint qualification is owned by its Windows orchestrator.'
}
foreach ($entry in @(
...
if ($dirty.Count -ne 0) { throw 'H4-6A live qualification requires a clean Git worktree.' }
$harnessRevision = (& git -C $repository rev-parse HEAD).Trim()
if ($LASTEXITCODE -ne 0 -or $harnessRevision -cnotmatch '^[0-9a-f]{40}$') {
    throw 'H4-6A harness revision is unavailable.'
}

Write-Json (Join-Path $evidencePath 'inputs.json') ([ordered]@{
    schema = 'ardents-h4-6a-two-endpoints-inputs-v1'
    attempt_started_at_utc = $attemptStartedAt
...
if (-not (Test-Path -LiteralPath $runScript -PathType Leaf)) {
    throw 'checked H4-6A Ubuntu runner is unavailable.'
}
...
Write-Json (Join-Path $evidencePath 'harness-sha256.json') ([ordered]@{ schema = 'ardents-h4-6a-harness-digests-v1'; files = @($harnessDigests) })

    $openSSH = Find-OpenSSH
    $suffix = [Guid]::NewGuid().ToString('N').Substring(0, 16)
    $remoteRoot = "/tmp/ardents-h4-6a-two-endpoints-$suffix"
    $remote = "$User@$VPS"
    ...
    $stageRoot = Join-Path ([IO.Path]::GetTempPath()) ("ardents-h4-6a-local-$suffix")
    ...
    if ($remoteRunExit -ne 0) { throw "remote H4-6A qualifier failed with exit $remoteRunExit." }
}
catch {
    ...
    if ($remoteOwnershipPossible) {
        try {
            $cleanupCommand = "set -eu; case '$remoteRoot' in /tmp/ardents-h4-6a-two-endpoints-[0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f]) ;; *) exit 2 ;; esac; test -f '$remoteRoot/.qualification-owner'; grep -x '$suffix' '$remoteRoot/.qualification-owner' >/dev/null; rm -rf -- '$remoteRoot'; test ! -e '$remoteRoot'; printf 'remote_root=absent\n'"
    ...
    if ($null -ne $stageRoot -and (Test-Path -LiteralPath $stageRoot -PathType Container)) {
        ...
        $tempPrefix = [IO.Path]::GetTempPath().TrimEnd([IO.Path]::DirectorySeparatorChar) + [IO.Path]::DirectorySeparatorChar + 'ardents-h4-6a-local-'
    ...
    if (Test-Path -LiteralPath $evidencePath -PathType Container) {
        ...
        Write-Json (Join-Path $evidencePath 'attempt-receipt.json') ([ordered]@{
            schema = 'ardents-h4-6a-two-endpoints-attempt-v1'
            attempt_started_at_utc = $attemptStartedAt
...
if ($failures.Count -ne 0) {
    throw ('H4-6A two-Endpoint qualification failed: ' + ($failures -join ' | '))
}
Write-Output "H4-6A two fresh Endpoints accepted; evidence retained at $evidencePath"
