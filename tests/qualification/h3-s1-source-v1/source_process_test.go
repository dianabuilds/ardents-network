...
func TestFiniteSourceCommandsAsBlackBoxProcesses(t *testing.T) {
	...
	verified := qualification.VerifyOffline(qualification.OfflineCase{
		Root: endpointRoot, NetworkID: fixture.networkID,
		Authorities: map[[32]byte]ed25519.PublicKey{authorityID: fixture.authorityPublic},
		Threshold:   1, Now: time.Unix(fixture.now, 0), Materializations: fixture.materializations,
	})
	if verified.Verdict != "pass" {
		t.Fatalf("independent persisted-state verdict=%s reason=%s", verified.Verdict, verified.Reason)
	}
}
