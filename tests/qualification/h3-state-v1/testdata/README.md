# H3 Network State offline golden vector v1

The lowercase hexadecimal files freeze the private record, Epoch,
...
`internal/network/state` accepts them through its product Interface and
`internal/qualification/state` reads the persisted evidence as a separate
black-box consumer and delegates protocol rules to the canonical Epoch module.
