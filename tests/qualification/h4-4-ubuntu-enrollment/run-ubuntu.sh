...
if [ "$(sha256sum "$signed_xpi" | awk '{print $1}')" != "$expected_xpi_sha256" ]; then
    echo "H4-4 Ubuntu enrollment qualifier received a different signed XPI" >&2
    exit 2
fi

umask 077
scratch=$(mktemp -d "${TMPDIR:-/tmp}/ardents-h4-4-v4.XXXXXX")
...
cohort=h4-4-ubuntu-container-qualification
release=mozilla-signed-0.1.0
platform=$platform
environment=alpha
network=h4-4-ubuntu-container-qualification
...
printf '{"schema":"ardents-alpha-enrollment-input-v1","bundle_root":"%s","cohort":"h4-4-ubuntu-container-qualification","release":"mozilla-signed-0.1.0","platform":"%s","manifest_sha256":"%s","environment":"alpha","network":"h4-4-ubuntu-container-qualification","target_path":"ardents/%s/endpoint"}\n' "$bundle" "$platform" "$manifest_sha256" "$platform" > "$input"
chmod 600 "$input"
...
if ! install_result=$("$bundle/$host_name" install --enrollment "$input" --endpoint-artifact "$bundle/$endpoint_name" --at 2026-08-26T00:00:00Z); then
    echo 'H4-4 Ubuntu enrollment manifest follows:' >&2
    sed -n '1,32p' "$bundle/SHA256SUMS" >&2
    echo 'H4-4 Ubuntu enrollment descriptor follows:' >&2
...
if [ ! -f "$native_manifest" ] || ! printf '%s' "$install_result" | grep -F '"extension_installation":"manual-required"' >/dev/null ||
    ...
    echo "H4-4 Ubuntu enrollment installation did not create the exact native manifest" >&2
    exit 1
fi
...
if [ -e "$native_manifest" ] || ! printf '%s' "$remove_result" | grep -F '"removal":"native-manifest-withdrawn"' >/dev/null; then
    echo "H4-4 Ubuntu enrollment removal left the native manifest behind" >&2
    exit 1
fi
printf '%s\n' "{\"schema\":\"ardents-h4-4-ubuntu-container-enrollment-qualification-v1\",\"signed_xpi_sha256\":\"$expected_xpi_sha256\",\"native_manifest\":\"installed-and-withdrawn\",\"extension_installation\":\"manual-required\"}"
