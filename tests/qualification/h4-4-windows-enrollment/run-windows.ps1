...
if ($signedHash -ne 'd88e8ecba84cda82a7b2354d1f445e19b9d092f3f3d068868d1173ef29eaa2a2') {
    throw "SignedXPI digest is not the approved H4-4 alpha artifact: $signedHash"
}
...
$bundle = Join-Path ([System.IO.Path]::GetTempPath()) ("ardents-h4-4-v4-" + [Guid]::NewGuid())
$inputPath = $null
$installed = $false
try {
    ...
    $descriptorLines = @(
        ...
        'cohort=h4-4-windows-qualification',
        'release=mozilla-signed-0.1.0',
        "platform=$platform",
        'environment=alpha',
        'network=h4-4-windows-qualification',
    ...
    [ordered]@{
        schema = 'ardents-h4-4-windows-enrollment-qualification-v1'
