...
func TestAuthenticatedRouteUsesSeparateRoleProcesses(t *testing.T) {
	...
	var rawEvidence bytes.Buffer
	encoder := json.NewEncoder(&rawEvidence)
	if err := encoder.Encode(clientEvidence); err != nil {
		t.Fatal(err)
	}
	for _, evidence := range completed {
	...
	if result := qualification.Evaluate(verification); result.Verdict != "pass" {
		t.Fatalf("independent verifier rejected real process evidence: %+v", result)
	}
