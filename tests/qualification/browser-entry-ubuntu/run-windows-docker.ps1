...
$artifacts = Join-Path ([System.IO.Path]::GetTempPath()) ("ardents-browser-entry-linux-artifacts-" + [Guid]::NewGuid())
try {
    ...
    $arguments = @(
        ...
        '-w', '/workspace', '-e', 'HOME=/tmp/ardents-browser-entry-home',
        'ubuntu:24.04', 'sh', './tests/qualification/browser-entry-ubuntu/run-ubuntu.sh', '/input/ardents-alpha-browser-entry.xpi', '/artifacts'
    )
    & docker @arguments
    if ($LASTEXITCODE -ne 0) {
        exit $LASTEXITCODE
