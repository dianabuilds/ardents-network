...
if [ "$#" -eq 1 ] && [ "$1" = --self-test ]; then
    ...
    printf 'alpha-control-run-ubuntu-self-test=accepted\n'
    exit 0
fi
...
die() {
    echo "Alpha-control qualifier: $*" >&2
    exit 1
}
...
case "$root" in
    /tmp/ardents-alpha-control-two-endpoints-[0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f]) ;;
...
printf 'alpha-control-two-fresh-endpoints=accepted\n' >"$evidence/verdict.txt"
