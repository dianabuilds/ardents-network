...
func run(args []string) (returnErr error) {
	...
	defer func() {
		returnErr = errors.Join(returnErr, client.Stop(context.Background()))
	}()
	...
	if opts.action == "publish" {
		if err := client.PublishLightpushEnvelope(ctx, opts.provider, networkapi.Envelope{
...
func newClient(dir string) networkapi.Service {
	return networkwaku.New(networkapi.Config{
...
func fetchRetained(ctx context.Context, client networkapi.Service, provider string, expected networkprivacy.SealedEnvelope) error {
	...
	defer ticker.Stop()
	for {
		items, err := client.FetchEnvelopes(ctx, []string{provider}, expected.ContentTopic)
		if err == nil {
