...
type PrivateProtocolPolicy struct {
	...
	ExceptionOwner string `json:"exception_owner"`
}
...
func validatePrivateProtocol(root string, policy PrivateProtocolPolicy) []error {
	var violations []error
	if policy.ExceptionOwner != "ARD-028" {
		violations = append(violations, errors.New("private protocol exception_owner must be ARD-028"))
	}
	...
	generatedDirectory := filepath.Dir(filepath.Join(root, filepath.FromSlash(policy.Generated)))
	entries, err := os.ReadDir(generatedDirectory)
	if err != nil {
		violations = append(violations, fmt.Errorf("private protocol generated directory: %w", err))
		return violations
	}
	for _, entry := range entries {
		...
		path := filepath.Join(generatedDirectory, entry.Name())
