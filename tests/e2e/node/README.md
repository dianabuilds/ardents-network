...
These tests build the real Node command and exercise the selected closed
readiness, duty replacement, drain, restart, credential isolation, and cleanup
through separate operating-system processes. Old Initiator, Introduction,
Rendezvous, Responder, and Transit-issuer Node command starts are retired. Their
former positive process cell is absent from the ordinary e2e profile; retained
tagged native-Rendezvous source is historical deletion input only. Fixtures are
generated in the test temporary directory and disappear with the run.
