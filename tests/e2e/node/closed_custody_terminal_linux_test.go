...
const closedCustodyPassword = "closed process custody fixture password"

type closedCustodyInput struct {
	prompt, value string
	secret        bool
}

type closedCustodyCapture struct {
	mu     sync.Mutex
	output bytes.Buffer
}

func (capture *closedCustodyCapture) Write(value []byte) (int, error) {
	capture.mu.Lock()
	defer capture.mu.Unlock()
	return capture.output.Write(value)
}
func (capture *closedCustodyCapture) bytes() []byte {
	capture.mu.Lock()
	defer capture.mu.Unlock()
...
func closedCustodyCommand(t *testing.T, binary string, inputs []closedCustodyInput, arguments ...string) (output []byte, resultErr error) {
	t.Helper()
	...
	capture := new(closedCustodyCapture)
	...
	defer func() {
		...
		if bytes.Contains(output, []byte(closedCustodyPassword)) {
			output = nil
