...
func TestAvailabilityFailureMatrixEndsInHonestTerminalLoss(t *testing.T) {
	...
	require.Contains(t, outcome.Decision.Denials, dataplacement.Denial{NodeID: fixture.quotaPeer, Reason: dataplacement.ReasonQuota})
	...
	require.Error(t, err)
	require.ErrorContains(t, err, dataplacement.ReasonCapability)
...
func seedFailureNodes(t *testing.T, names, dirs []string, discoveryPrivacy, dataPrivacy testkit.PrivacyGroupFixture) ([]runtimeprocess.NodeRuntime, []string, []string) {
	t.Helper()
...
func publishFailureManifest(t *testing.T, fixture availabilityFailureFixture) dataapi.ManifestSnapshot {
	t.Helper()
...
func setFailureIntent(t *testing.T, owner runtimeprocess.NodeRuntime, rootID string, now time.Time) {
	t.Helper()
...
func advanceRepairsToTerminal(t *testing.T, ownerNode *runtimeprocess.Node, repairs []dataapi.RepairSnapshot) {
	t.Helper()
