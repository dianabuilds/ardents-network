...
package localcontrolsurfacee2e_test
...
func TestOperatorServiceAndDataSurfaceReadiness(t *testing.T) {
	...
	source := testkit.StartNode(t, runtimeinfra.Config{
		...
		Boot: runtimeinfra.BootConfig{Sources: []string{"local://bootstrap"}},
		Data: runtimeinfra.NodeDataConfig{Dir: sourceDir}, Privacy: discoveryPrivacy.Sender,
	})
	...
	rt := testkit.NewRuntime(t, runtimeinfra.Config{
		...
		Transport: runtimeinfra.NodeTransportConfig{BindAddress: "127.0.0.1", ReachabilityMode: transport.ReachabilityPrivateLAN},
		Data:      runtimeinfra.NodeDataConfig{Dir: t.TempDir()}, Privacy: discoveryPrivacy.Receiver,
	...
	scenario.Step("operator starts workload and observes publication truth", func(t *testing.T) {
		require.NoError(t, rt.Runtime.StartWorkloadContext(context.Background(), "work.echo"))
	...
	scenario.Degraded("operator sees publication withdrawal and explanation after workload stop", func(t *testing.T) {
		require.NoError(t, rt.Runtime.StopWorkloadContext(context.Background(), "work.echo"))
