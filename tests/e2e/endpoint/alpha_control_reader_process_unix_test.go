...
func TestAlphaControlReaderVerifiesPinnedBundleAndCachedRestart(t *testing.T) {
	...
	bundle, artifact, input := alphaControlBundle(t, endpoint)
	state := t.TempDir()
	arguments := []string{"inspect-bundle", "--enrollment", input, "--artifact", artifact, "--state-root", state, "--at", time.Now().UTC().Format(time.RFC3339)}
...
func buildControl(t *testing.T) string {
	...
	build := exec.Command("go", "build", "-trimpath", "-o", path, "./cmd/ardents-control")
...
func alphaControlBundle(t *testing.T, endpoint string) (string, string, string) {
	t.Helper()
	platform := runtime.GOOS + "-" + runtime.GOARCH
	artifactName, targetPath := "ardents-"+platform, "ardents/"+platform+"/endpoint"
	...
	components, roots, catalogRoot, catalog := alphaControlStatements(t, decision, now)
	...
	descriptor := strings.Join([]string{
		...
		"control_release_root=release.pub", "control_network_root=network.pub", "control_compatibility_root=compatibility.pub",
	}, "\n") + "\n"
	files := map[string][]byte{"1.root.json": rootBytes, "RELEASE": []byte(descriptor), artifactName: artifact, "catalog.ac1": catalog, "catalog.pub": catalogRoot,
		"release.ac1": components[0], "network.ac1": components[1], "compatibility.ac1": components[2], "release.pub": roots[0], "network.pub": roots[1], "compatibility.pub": roots[2]}
...
func alphaControlStatements(t *testing.T, decision release.Decision, now time.Time) ([3][]byte, [3][]byte, []byte, []byte) {
	t.Helper()
	...
	return components, roots, disclosurePublic, raw
}
