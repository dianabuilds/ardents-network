...
func assertDockerStartFailureExhaustsRestartBudget(t *testing.T, ctx context.Context, config string) {
	...
	require.NoError(t, service.Register(workloadcontroller.Spec{
		ID: "work.docker.budget", Kind: "service", Owner: "node", Config: config,
		Desired: workloadcontroller.DesiredRunning, PolicyRef: "trusted",
	}))
...
func TestDockerExecutorControllerRecoversAndRemovesObservedInstance(t *testing.T) {
	...
	require.NoError(t, recovered.SetDesired("work.controller.recovery", workloadcontroller.DesiredRemoved))
...
func TestDockerExecutorPublishesOnlyAdmittedIngressOnInternalNetwork(t *testing.T) {
	...
	const port = 19090
	executor := dockerExecutorWith(t, execution.DockerExecutorConfig{
	...
	stopTimeout := 1
	_, err = engine.ContainerStop(ctx, proxy.Container.ID, client.ContainerStopOptions{Timeout: &stopTimeout})
	require.NoError(t, err)
	require.Eventually(t, func() bool { return !dockerIngressReady(httpClient, endpoint) }, 5*time.Second, 100*time.Millisecond)

	recoveredExecutor := dockerExecutorWith(t, execution.DockerExecutorConfig{
		NodeID: "stb405-ingress", Runtime: "runc", AllowedRegistries: []string{"docker.io"},
		AllowedIngressHosts: []string{host}, IngressBindAddress: "0.0.0.0", IngressProxyImage: dockerProxyImage(t),
	})
...
func dockerIngressReady(client *http.Client, endpoint string) bool {
	...
	return readErr == nil && response.StatusCode == http.StatusOK && string(body) == "ready"
}
...
func TestDockerExecutorDeniesUnsafeIntentAndRuntimeFallback(t *testing.T) {
	...
	deniedRegistry := dockerExecutorWith(t, execution.DockerExecutorConfig{
	...
	noFallback := dockerExecutorWith(t, execution.DockerExecutorConfig{
...
func newDockerExecutor(t *testing.T, nodeID string) *execution.DockerExecutor {
	t.Helper()
	return dockerExecutorWith(t, execution.DockerExecutorConfig{
		NodeID: nodeID, Runtime: "runc", AllowedRegistries: []string{"docker.io"},
		AllowedPolicyRefs: []string{"trusted"}, StopTimeout: 2 * time.Second,
	})
}

func dockerExecutorWith(t *testing.T, config execution.DockerExecutorConfig) *execution.DockerExecutor {
	t.Helper()
	config.AllowInsecureRemote = true
	executor, err := execution.NewDockerExecutor(config)
	require.NoError(t, err)
	return executor
}
...
func waitForDockerStopped(t *testing.T, executor *execution.DockerExecutor, workloadID string) execution.Instance {
	t.Helper()
...
func cleanupDockerInstance(t *testing.T, executor *execution.DockerExecutor, instance execution.Instance) {
	t.Helper()
	t.Cleanup(func() {
