...
func TestConnectRPCNetworkPublicSurfaceMatchesLocalTruth(t *testing.T) {
	...
	localRouteCandidates, localRoute, err := localNode.ListRouteCandidates(nodeapi.ListRouteCandidatesQuery{Service: "echo"})
...
func TestConnectRPCDataTransferSurfaceMatchesLocalTruth(t *testing.T) {
	...
	var localTransfers []dataapi.TransferSnapshot
	...
	testkit.WaitForCondition(t, 5*time.Second, "transfer snapshots are visible through local and connectrpc surfaces", func() (bool, string) {
		localTransfers = rt.Data.ListTransfers()
	...
	localTransfer, ok := findLocalTransferByResource(localTransfers, stored.ID)
	require.True(t, ok)
	localGetTransfer, err := rt.Data.GetTransfer(localTransfer.ID)
	require.NoError(t, err)
...
func TestConnectRPCDataSurfaceMarksStaleRemoteSourceUnusable(t *testing.T) {
	...
	rt := testkit.StartRuntime(t, runtimeinfra.Config{
		...
		Boot: runtimeinfra.BootConfig{Sources: []string{"local://bootstrap"}},
		Data: runtimeinfra.NodeDataConfig{Dir: dataDir},
	})
	client := testkit.NewArdentsClient(t, rt.Runtime)
...
func findHostedServiceSnapshot(t *testing.T, services []hostingapi.HostedServiceSnapshot, id string) hostingapi.HostedServiceSnapshot {
	t.Helper()
...
func findLocalTransferByResource(transfers []dataapi.TransferSnapshot, resourceID string) (dataapi.TransferSnapshot, bool) {
...
func findLocalBlobSourceByTransport(sources []dataapi.BlobSourceSnapshot, transport string) (dataapi.BlobSourceSnapshot, bool) {
	for _, item := range sources {
		if item.Transport == transport {
			return item, true
		}
	}
	return dataapi.BlobSourceSnapshot{}, false
}
