...
func TestLocalControlReadyHelper(t *testing.T) {
	if os.Getenv("ARDENTS_LOCAL_CONTROL_READY_HELPER") != "1" {
		return
	}
	generation := os.Getenv("ARDENTS_WORKLOAD_GENERATION")
	server := &http.Server{Addr: os.Getenv("ARDENTS_LOCAL_CONTROL_READY_ADDRESS"), Handler: http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
...
func localControlReadyFixture(t *testing.T) (string, string, string) {
	...
	require.NoError(t, err)
	raw, err := json.Marshal(map[string]any{"command": executable, "args": []string{"-test.run=TestLocalControlReadyHelper"},
		"env": map[string]string{"ARDENTS_LOCAL_CONTROL_READY_HELPER": "1", "ARDENTS_LOCAL_CONTROL_READY_ADDRESS": fmt.Sprintf("0.0.0.0:%d", port)}})
	require.NoError(t, err)
