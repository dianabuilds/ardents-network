...
func TestCLINetworkPublicSurfaceReflectsLocalTruth(t *testing.T) {
	...
	routes, route, err := localNode.ListRouteCandidates(nodeapi.ListRouteCandidatesQuery{Service: "echo"})
...
func TestCLIDataTransferSurfaceReflectsFetchRuntimeTruth(t *testing.T) {
	...
	transfer, ok := findLocalTransferByResource(transfers, stored.ID)
	require.True(t, ok)
	detail, err := rt.Data.GetTransfer(transfer.ID)
	require.NoError(t, err)
	inventory := rt.Data.DataInventory()
	...
	require.Contains(t, transfersOut.stdout, "transfer: "+transfer.ID)
	require.Contains(t, transfersOut.stdout, "  state: "+transfer.State)
	require.Contains(t, transfersOut.stdout, "  resource: "+transfer.ResourceID)

	transferOut := cliHarness.run(t, "data", "transfers", "get", transfer.ID)
...
func newCLIHarness(t *testing.T, runtime runtimeprocess.NodeRuntime) cliHarness {
	t.Helper()
