...
func TestConnectRPCProjectsTCPWSSProfileTruth(t *testing.T) {
	...
	rt := testkit.StartRuntime(t, runtimeinfra.Config{
		...
		Boot:        runtimeinfra.BootConfig{Sources: []string{"local://bootstrap"}},
		Data:        runtimeinfra.NodeDataConfig{Dir: t.TempDir()},
		Transport: runtimeinfra.NodeTransportConfig{
...
func TestConnectRPCReadRequiresExactAction(t *testing.T) {
	...
	path, handler, err := rpcadapter.NewHandler(testkit.ConnectDependencies(rt.Runtime), rpcadapter.AuthConfig{
		Token:        "limited-token",
		SubjectID:    "connect-test",
		Capabilities: []string{"node.start"},
	})
	require.NoError(t, err)
	...
	_, err = client.GetNodeStatus(context.Background(), req)
	require.Error(t, err)
...
func TestConnectRPCDataRoundTripAndErrors(t *testing.T) {
	...
	rt := testkit.StartRuntime(t, runtimeinfra.Config{
		Name: "connect",
		Boot: runtimeinfra.BootConfig{Sources: []string{"local://bootstrap"}},
		Data: runtimeinfra.NodeDataConfig{
			Dir:                      t.TempDir(),
	...
	_, apiErr := rt.Node.GetObject("missing")
	...
	_, err = client.GetNodeStatus(context.Background(), connect.NewRequest(&ardentsv1.GetNodeStatusRequest{}))
	require.Error(t, err)
	require.True(t, errors.As(err, &connectErr))
	require.Equal(t, connect.CodeUnauthenticated, connectErr.Code())

	bareTokenReq := connect.NewRequest(&ardentsv1.GetNodeStatusRequest{})
	bareTokenReq.Header().Set("Authorization", testkit.ConnectAuthConfig().Token)
	_, err = client.GetNodeStatus(context.Background(), bareTokenReq)
	require.Error(t, err)
	require.True(t, errors.As(err, &connectErr))
	require.Equal(t, connect.CodeUnauthenticated, connectErr.Code())

	_, objErr := rt.Node.GetObject("missing")
	require.NotNil(t, objErr)
	require.Contains(t, objErr.Error(), "not found")
}
