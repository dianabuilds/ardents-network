...
func TestPublicReachabilityGatesAndWithdrawsNodeAdvertisement(t *testing.T) {
	...
	scenario.Precondition("start a public-direct node with an unverified advertised address", func(t *testing.T) {
		...
		testingNode := runtime.Runtime.(interface{ NodeForTesting() any }).NodeForTesting()
		node = testingNode.(*runtimeprocess.Node)
...
func localNodeEndpoints(t *testing.T, runtime runtimeprocess.NodeRuntime) []string {
	t.Helper()
