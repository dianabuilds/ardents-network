...
func TestHostedServicePublicTruthRequiresActualWorkloadListener(t *testing.T) {
	...
	runtime := testkit.StartRuntime(t, runtimeprocess.Config{Name: "hosted-readiness-runtime",
		Boot: runtimeprocess.BootConfig{Sources: []string{"local://bootstrap"}}, Data: runtimeprocess.NodeDataConfig{Dir: t.TempDir()},
		Workload: []runtimeprocess.NodeWorkloadConfig{{ID: "work.ready", Kind: "service", Owner: "node", Desired: "running",
			Config: readinessHelperConfig(t, address), Services: []runtimeprocess.NodeServiceConfig{{ID: "svc.ready", Type: "http",
	...
	require.NoError(t, runtime.Workload.StopWorkloadContext(context.Background(), "work.ready"))
...
func TestHostedServiceReadinessTracksRealGenerationBoundListener(t *testing.T) {
	...
	now := time.Now().UTC()
	backing := registry.Backing{Spec: hostingservice.Spec{ID: "svc.integration", Type: "http", Owner: "work.integration",
