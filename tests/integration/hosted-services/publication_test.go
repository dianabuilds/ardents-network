...
func TestPublishedServiceResolvesAndConnectsAcrossRealWakuNodes(t *testing.T) {
	...
	remote := testkit.StartNode(t, runtimeprocess.Config{
		...
		Transport: runtimeprocess.NodeTransportConfig{BindAddress: "127.0.0.1", ReachabilityMode: networkapi.ReachabilityPrivateLAN},
		Data:      runtimeprocess.NodeDataConfig{Dir: t.TempDir()}, Privacy: privacy.Sender, DiscoveryRefreshInterval: 50 * time.Millisecond,
		Workload: []runtimeprocess.NodeWorkloadConfig{{ID: "work.published", Kind: "service", Owner: "node", Desired: "running",
			Config: readinessHelperConfig(t, fmt.Sprintf("0.0.0.0:%d", port)), Services: []runtimeprocess.NodeServiceConfig{{
	...
	testkit.WaitForCondition(t, 5*time.Second, "current ready service is locally published", func() (bool, string) {
		...
		hosted, hostedErr := remote.GetHostedService("svc.published")
		publication, publicationErr := remote.GetServicePublicationStatus("svc.published")
	...
	local := testkit.StartNode(t, runtimeprocess.Config{
		...
		Transport: runtimeprocess.NodeTransportConfig{BindAddress: "127.0.0.1", ReachabilityMode: networkapi.ReachabilityPrivateLAN},
		Data:      runtimeprocess.NodeDataConfig{Dir: t.TempDir()}, Privacy: privacy.Receiver,
	...
	require.NoError(t, remote.StopWorkload("work.published"))
...
func nodeRecordEndpoints(records []discoveryapi.DiscoveryRecord) []string {
