...
func TestDataAvailabilityRepairsCorruptReplicaToDifferentWakuPeer(t *testing.T) {
	...
	blob, err := source.PublishBlob(dataapi.BlobSnapshot{
		MediaType: "application/octet-stream", Payload: []byte("repairable ciphertext"),
		Encrypted: true, Cipher: datapayload.AES256GCMCipher, KeyID: "key-redacted",
	})
	require.NoError(t, err)
	root, err := source.PublishManifest(dataapi.ManifestSnapshot{
		Kind: "blob-set", Owner: sourceIdentity.Principal, Encrypted: true, Retention: "durable",
		Refs: []dataapi.RefSnapshot{{Kind: "blob", ID: blob.ID}},
	})
	require.NoError(t, err)
	sourceNode := source.NodeForTesting().(*runtimeprocess.Node)
	_, err = source.SetReplicaIntent(dataapi.ReplicaIntentSnapshot{
	...
	require.Error(t, err)
	require.Equal(t, dataplacement.CommitmentCorrupt, corrupt.State)
	...
	events, _ := source.ListRecentEvents(100, "")
	...
	require.NoError(t, err)
	require.NoError(t, runtimeprocess.StopTransportForIntegrationTest(targetOne.NodeForTesting().(*runtimeprocess.Node), context.Background()))
	...
	retainedWhileOwnerOffline, err := targetTwo.GetBlob(blob.ID)
	require.NoError(t, err)
...
func importRepairNodeRecords(t *testing.T, source, targetOne, targetTwo runtimeprocess.NodeRuntime) {
	t.Helper()
...
func hasActiveCommitmentForPeer(state dataplacement.State, peer string) bool {
	for _, commitment := range state.Commitments {
		if commitment.PeerID == peer && commitment.State == dataplacement.CommitmentActive {
			return true
		}
	}
	return false
