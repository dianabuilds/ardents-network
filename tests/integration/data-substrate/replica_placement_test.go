...
func TestDataReplicaPlacementCommitsEncryptedCopyOverPrivateWaku(t *testing.T) {
	...
	testkit.ImportRecordsFromNode(t, target, source, "replica-source", func(record discoveryapi.DiscoveryRecord) bool { return record.Kind == "node" })
	...
	blob, err := source.PublishBlob(dataapi.BlobSnapshot{
		MediaType: "application/octet-stream", Payload: ciphertext,
	...
	targetBlob, err := target.GetBlob(blob.ID)
	require.NoError(t, err)
	require.True(t, targetBlob.Encrypted)
	require.Equal(t, "relay-temporary", targetBlob.Retention)

	targetState := runtimeprocess.ReplicaPlacementStateForIntegrationTest(target.NodeForTesting().(*runtimeprocess.Node))
