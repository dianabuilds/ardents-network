...
func TestDataSubstrateObjectAndBlobPersistAcrossRestart(t *testing.T) {
	...
	object, err := first.PublishObject(dataapi.ObjectSnapshot{
		Type:  "chat.message",
		Owner: "node",
	...
	blob, err := first.PublishBlob(dataapi.BlobSnapshot{
	...
	storedObject, err := second.GetObject(object.ID)
	require.NoErrorf(t, err, "get object: %v", err)
	require.Falsef(t, storedObject.Body["text"] != "hello", "text = %v, want hello", storedObject.Body["text"])

	storedBlob, err := second.GetBlob(blob.ID)
	require.NoErrorf(t, err, "get blob: %v", err)
...
func TestDataSubstrateOperationsEmitDiagnosticsEvents(t *testing.T) {
	...
	blob, err := n.PublishBlob(dataapi.BlobSnapshot{MediaType: "text/plain"})
	require.NoErrorf(t, err, "publish blob: %v", err)
	{

		_, err := n.PublishManifest(dataapi.ManifestSnapshot{
...
func TestDataSubstrateRejectsPlaintextRemoteReserve(t *testing.T) {
	...
	requester := testkit.StartNode(t, runtimeinfra.Config{
		...
		Data:    runtimeinfra.NodeDataConfig{Dir: requesterDir},
...
func TestDataSubstrateBlobResponseRequiresDiscoveredRequester(t *testing.T) {
	...
	exchange := datatransfer.NewPrivateExchange(privacy.Receiver, attacker)
