...
func TestDataSubstrateRestartReconcilesExpiredRetention(t *testing.T) {
	...
	published, err := first.PublishBlob(dataapi.BlobSnapshot{
		MediaType: "text/plain",
		Payload:   []byte("expire on restart"),
	})
	...
	blob, err := second.GetBlob(published.ID)
...
func TestDataSubstrateSnapshotExplainsBlobPayloadLoss(t *testing.T) {
	...
	blob, err := n.PublishBlob(dataapi.BlobSnapshot{
		MediaType: "text/plain",
		Payload:   []byte("payload disappears"),
	})
	require.NoErrorf(t, err, "publish blob: %v", err)
...
func TestDataSubstrateSnapshotExplainsBrokenObjectRefsAfterRestart(t *testing.T) {
	...
	blob, err := first.PublishBlob(dataapi.BlobSnapshot{
		MediaType: "text/plain",
		Payload:   []byte("object ref payload"),
	})
	require.NoErrorf(t, err, "publish blob: %v", err)
	{

		_, err := first.PublishObject(dataapi.ObjectSnapshot{
			Type:  "chat.message",
			Owner: "principal.local",
			BlobRefs: []dataapi.RefSnapshot{{
				Kind: "blob",
				ID:   blob.ID,
			}},
		})
