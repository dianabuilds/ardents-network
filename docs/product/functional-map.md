...
| NET-05 | The V1 Application Interface exposes one live, bidirectional, reliable, ordered byte stream without message boundaries. Closure or failure is explicit; datagrams, offline retention, exactly-once delivery, and automatic replay are not provided. | fixed | R-002 P1-D2 fixes the primitive and P1-D5 fixes its bounded failure contract. Exact resource bounds remain open. |
