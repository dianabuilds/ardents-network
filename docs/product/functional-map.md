...
| NET-05 | The V1 Application Interface exposes one live, bidirectional, reliable, ordered byte stream without message boundaries. Closure or failure is explicit; datagrams, offline retention, exactly-once delivery, and automatic replay are not provided. | fixed | R-002 P1-D2 fixes the primitive. Exact connect, accept, backpressure, partial-write, close, and error behavior remains to be specified. |
...
| Authorization | Separate local access to the Connection Interface and privileged Service Administration Interface, plus optional future restricted-discovery hooks. | Who may read, write, join, administer, or invoke an operation inside the Service protocol. |
