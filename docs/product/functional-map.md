...
| NET-14E | Each required Windows and Linux client reference endpoint supports at least `128` concurrently open outbound Service Connections, including at least `32` simultaneously carrying the declared active-transfer workload. These are minimum total client capacities, not hard maxima or per-Service, publisher, or infrastructure-Node limits. | fixed | R-023 P3-D3b1 fixes the floor. P3-D6 defines duration and traffic mix; P3-D3b2 sets publisher and Node capacity; P3-D3c sets aggregate goodput, CPU, memory, overhead, and fairness. Exhaustion returns an explicit local resource-limit result without crash, deadlock, unbounded queue, silent eviction, cross-context reuse, or downgrade. |
