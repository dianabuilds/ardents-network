...
### 2.5 Источник Истины Должен Быть Однозначным

Для каждого наблюдаемого состояния система должна иметь один авторитетный
источник и явно отличать факт от проекции, кэша и desired state. Этот принцип не
назначает владельцев среди будущих пакетов или модулей.
...
- набор параллельных противоречащих друг другу control surfaces;
- metadata-only substitutes вместо реального обязательного поведения.
