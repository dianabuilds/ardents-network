...
`docs/product/data-substrate-requirements.md` and is owned by `Data Substrate`.

This contract describes target product behavior. Metadata-only source
announcements and an `available-remote` state do not satisfy the replica
...
Availability diagnostics include bounded state, reason, copy counts, intent
version, and repair ordinal. Payload bytes, content keys, capability material,
private selectors, and raw routes are excluded.
