...
| A5 | H4-6A control: cached restart plus two fresh enrolled Endpoint state roots obtain the same catalog/Release/Network verdict | `make qualification-h4-3b-docker` | Historical controlled-fixture evidence passed locally and was repeated in A7 on 2026-08-27 for `reference-c2-linux-amd64` SHA-256 `ad0c405a81ae312459566f35445bf40b561c91eec8019d52a5a72b551927cf7b`; it gives cached-restart and two-fresh-root agreement. **Pending for this candidate:** repeat against the exact `70bf425e` Endpoint/control companions and future concrete component identities. |
| A6 | Dynamic H4-3B: all four HTTP/1.1 success/failure terminal cases, exact no-fallback, and Linux alpha-corpus floor; companion alpha-origin resource cell enforces `16 KiB` request/response heads, `1 MiB` request/response bodies, `1 s` header read, and `5 s` browser keep-alive idle ceiling | `make qualification-h4-3b-docker` | Historical evidence passed locally and was repeated in A7 on 2026-08-27 at 1 vCPU/1 GiB/128 PIDs. The C-2 cells covered normal dynamic HTTP/1.1, withdrawal, Publisher Application reset, and Publisher Endpoint loss; separate local-origin cells covered known/chunked body, oversized-head, header-timeout, and idle keep-alive enforcement. **Pending for this candidate:** repeat with its exact artifact and control input set. The resource cell is not a second two-Endpoint journey. |
| A7 | Constrained VPS Docker repetition of A5/A6 with exact uploaded byte digests, host envelope, and residue check | `make qualification-h4-3b-vps` with `ARDENTS_H4_3B_VPS` and `ARDENTS_H4_3B_SSH_KEY` | Historical behavior evidence passed on 2026-08-27 at `8d5c88513bf77d891c4b2ff109323d0f837ce272`: Linux `5.15.0-185-generic`, `x86_64`, 4 vCPU, Docker `29.4.1`, `golang:1.26.6` image `sha256:0d1d3a794be25f809dd2cb3160d8c73276c4056a9f8242a138e908ddeee7b6b6`; all four C-2 cases, 13 HTTP-limit cases, and two H4-6A reader cases passed, followed by a clean residue check. **Pending for this candidate:** repeat with the exact `70bf425e` Endpoint artifact before calling A7 green. |
| A8 | Full H4-3B two-host journey: Publisher and User endpoints on distinct hosts, with exact Target, dynamic workload, withdrawal and one declared loss case | `make qualification-h4-3b-multihost` and the topology in `tests/qualification/h4-3b-multihost/README.md` | Historical behavior evidence passed on 2026-08-27: Windows User side and independent Linux VPS Publisher side completed dynamic HTTP, withdrawal, Publisher Application reset, and Publisher Endpoint loss in 116.5 s total, followed by a clean residue check. The stage was built from `f41f4def33e0bfecd9ed9342137d3784fb985e1f`. **Pending for this candidate:** repeat with the exact `70bf425e` Endpoint artifact before calling A8 green. |
| A9 | Selected browser/platform: browser observes the dynamic workload and failure state at the candidate's loopback origin | Windows + Firefox `154.0.1`, clean-profile runner | Historical behavior evidence passed on 2026-08-27 at `8d5c88513bf77d891c4b2ff109323d0f837ce272`: endpoint observation, dynamic Firefox C-2 flow, and following no-Firefox C-2 leg passed. **Pending for this candidate:** repeat with the exact `70bf425e` Endpoint artifact before calling A9 green. This selection remains browser-observation only; it does not claim a Windows H4-1 lifecycle or participant Browser Entry. |
