...
| Field | Value |
|---|---|
| Source revision | `2eafb1101990837a8f4d6f5aeeb13e13793eadc5` |
...
| A5 | H4-6A control: cached restart plus two fresh enrolled Endpoint state roots obtain the same catalog/Release/Network verdict | `make qualification-h4-3b-docker` | Passed locally on 2026-08-27 for synthetic current-byte evidence; real component identities remain A4/A7 inputs |
| A6 | Dynamic H4-3B: all four HTTP/1.1 success/failure terminal cases, exact no-fallback, and Linux alpha-corpus floor | `make qualification-h4-3b-docker` | Passed locally on 2026-08-27 at 1 vCPU/1 GiB/128 PIDs |
| A7 | Constrained VPS Docker repetition of A5/A6 with exact uploaded byte digests, host envelope, and residue check | `make qualification-h4-3b-vps` with `ARDENTS_H4_3B_VPS` and `ARDENTS_H4_3B_SSH_KEY` | Active; awaiting a recorded run on the declared VPS |
| A8 | Full H4-3B two-host journey: Publisher and User endpoints on distinct hosts, with exact Target, dynamic workload, withdrawal and one declared loss case | purpose-named two-host C-2 runner plus selected topology | Inactive: current C-2 harness is local-process only; remote role orchestration is required |
| A9 | Selected browser/platform: browser observes the dynamic workload and failure state at the candidate's loopback origin | selected Endpoint desktop platform, browser/version, clean-profile runner | Inactive: Product Owner has not selected a participant browser/platform; retained Firefox Browser Entry evidence is non-gating compatibility only |
