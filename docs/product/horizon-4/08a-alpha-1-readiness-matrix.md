...
| A8 | Full H4-3B two-host journey: Publisher and User endpoints on distinct hosts, with exact Target, dynamic workload, withdrawal and one declared loss case | purpose-named two-host C-2 runner and the topology in `tests/qualification/h4-3b-multihost/README.md` | Inactive: the topology is now bounded, but its remote orchestrator and declared VPS execution are still required |
