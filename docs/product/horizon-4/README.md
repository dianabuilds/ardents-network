...
| H4-3A / H4-3B | The separate-process Publisher-to-User C-2 journey carries one exact Target Link through private reachability and the bounded loopback Reference Site. H4-3A passed local, clean Linux Docker, the constrained project-VPS Docker profile, and the selected Firefox 154 compatibility run. H4-3B preserves POST/body/header/cookie, redirect, cookie follow-up, and chunked response semantics; explicitly unpublishes the exact Target/generation after drain; classifies a Publisher Application reset and hard Publisher Endpoint loss; and refuses fallback to the prior/another alpha name or ordinary Internet. Its exact cross-built product bytes now pass those four terminal cases plus cached/two-fresh-root H4-6A control observation in a constrained local Linux Docker cell. | H4-3A is complete only as bounded Reference evidence. H4-3B still needs the declared project-VPS Docker repetition, selected platform/browser, actual multi-host, and declared HTTP/1.1-limit qualification. H4-4 Browser Entry is optional compatibility evidence rather than an H4-3B gate; H4-1 release/first-run qualification, capacity, public deployment, and H4-7 browser claims remain separate. |
