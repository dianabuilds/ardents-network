...
Status: **accepted Horizon 4 delivery briefs. H4-1A, H4-2, H4-3A, and H4-6A
have selected maintained slices; the current H4-4 alpha slice is closed with
its Firefox route retained as compatibility evidence only; later slices retain
their own gates.**
...
| H4-8 | [Qualification and promotion](08-qualification-promotion.md) | Evidence determines whether a live network may make the corresponding beta claim. |
...
|---|---|---|
| H4-1 | Ubuntu Portable enrollment, foreground authenticated replacement, custody export/locked-restore/confirmed-purge, and the direct Ubuntu `.deb` package lifecycle are maintained. Portable and replacement have native non-lingering Ubuntu user-session qualification; the package has Linux `dpkg` process qualification. | A declared immutable published Ubuntu artifact and its independent first-contact enrollment; native Desktop package qualification; Windows; package repositories/signing/automatic repair; and every public-release claim. |
...
| H4-3A / H4-3B | The separate-process Publisher-to-User C-2 journey carries one exact Target Link through private reachability and the bounded loopback Reference Site. It passed local, clean Linux Docker, the constrained project-VPS Docker profile, and the selected Firefox 154 compatibility run. The local H4-3B dynamic path preserves POST/body/header/cookie, redirect, cookie follow-up, and chunked response semantics; explicitly unpublishes the exact Target/generation after drain; classifies a Publisher Application reset and hard Publisher Endpoint loss; and refuses fallback to the prior/another alpha name or ordinary Internet. | H4-3A is complete only as bounded Reference evidence. H4-3B is locally qualification-ready; its exact candidate still needs Docker/VPS, selected platform/browser, actual multi-host, and declared HTTP/1.1-limit qualification. H4-4 Browser Entry is optional compatibility evidence rather than an H4-3B gate; H4-1 release/first-run qualification, capacity, public deployment, and H4-7 browser claims remain separate. |
...
| H4-6A | The alpha-control reader and signed component-catalog verification are maintained. | Concrete release/component identities and two fresh enrolled Endpoint observations. |
| H4-8 | Qualification and closure rules are defined. | One exact H4-1–H4-3 alpha build and topology to qualify. |
