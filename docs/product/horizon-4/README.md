...
| H4-3A / H4-3B | The separate-process Publisher-to-User C-2 journey carries one exact Target Link through private reachability and the bounded loopback Reference Site. H4-3B preserves POST/body/header/cookie, redirect, cookie follow-up, and chunked response semantics; explicitly unpublishes the exact Target/generation after drain; classifies a Publisher Application reset and hard Publisher Endpoint loss; and refuses fallback to the prior/another alpha name or ordinary Internet. On 2026-08-27, the exact Linux bytes passed all four terminal cases, 13 declared HTTP/1.1-limit cases, and cached/two-fresh-root H4-6A control observation locally and in the constrained project VPS Docker profile. The full Windows-to-independent-Linux-VPS journey passed all four cases; Windows + Firefox 154.0.1 also passed the selected loopback browser-observation cell. | H4-3A is complete only as bounded Reference evidence. H4-3B needs an immutable H4-1 artifact and real first-run enrollment before it can be used as a release journey, plus the declared soak/fault campaign. H4-4 Browser Entry remains optional compatibility evidence rather than an H4-3B gate; capacity, public deployment, and H4-7 browser claims remain separate. |
...
| H4-6A | The alpha-control reader and signed component-catalog verification are maintained; controlled cached restart and two-fresh-root agreement passed locally and in the constrained VPS Docker cell. | Concrete published release/component identities and fresh enrolled Endpoint observations over the real artifact. |
