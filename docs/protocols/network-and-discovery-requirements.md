...
Trust должен существовать как обязательная функция проверки, допустимости и
пригодности сетевого знания.
...
semantics are fixed in `docs/protocols/network-privacy-requirements.md`.
Cross-surface request/response/error contract discipline is fixed separately in
`docs/protocols/communication-contracts.md`.
Transport-variant requirements are fixed in
`docs/protocols/network-transport-variants-requirements.md`.
...
Trust evaluation is mandatory for network and discovery outcomes. This document
does not prescribe its package ownership or repository placement.
