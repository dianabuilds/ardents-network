...
- `tcp_wss` is implemented only when the daemon receives a complete explicit
  ...
  pair;
- `tcp_quic` is recognized only so explicit selection can fail closed under
  DEC-STB-003;
- a separate operational mode axis may narrow participation without becoming a
  second transport profile.

For the current loop, the initial allowed operational mode set is:

- `steady`
- `restricted_defense`

`restricted_defense` is a narrowed operational mode for attack, instability, or
policy-driven surface reduction. It may initially reuse the same TCP-only
endpoint-family set as the current baseline if that is the only truthful
runtime shape available.

The current implementation exposes the automatic mode, switch reason, and
recovery state but does not yet remove live Waku protocol services. It must
therefore be described as observability-only, not as an enforced defense
capability, until STB-306 supplies and tests the reduction.

In that Phase 2 form, the mode is valid only if:
...
The current root implementation starts from an explicit transport-profile model
whose default steady-state profile is still constrained `tcp_only`.

Implemented runtime truth in the current repository is:

- `tcp_only` as the default steady-state profile;
- `tcp_wss` as an implemented expansion profile when secure websocket
  certificate and key material are configured explicitly;
- `restricted_defense` as an implemented degraded/recovery operational mode
  selected by controller logic above the active profile;
- `tcp_quic` remains an allowed profile target, but is not yet implemented.

The current default runtime surface remains:

- `internal/transport/startup.go` disables default libp2p transports and
  enables TCP explicitly;
- QUIC, websocket-secured transport exposure, WebTransport, and WebRTC-family
  endpoints are absent from the default `tcp_only` runtime exposure;
- tests already assert that only TCP-backed endpoints are published.

Any transport-variant work must therefore begin from this known baseline rather
than from an assumed multi-transport runtime.
