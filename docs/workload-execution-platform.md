...
- the executor injects the reserved non-secret
  ...
  defined in `docs/hosted-service-probe-model.md`;
...
Security enforcement is completed in `STB-403`, but the lifecycle contract may
not create a product container that lacks its required bounds. The minimum
creation posture is:
...
boundary are defined in `docs/workload-security-policy.md`.
...
`STB-402` implements lifecycle and recovery through the Docker adapter.
`STB-403` completes admission, image provenance, security and resource
enforcement. Neither task may claim production support until real Linux tests
prove the applicable preflight, isolation, failure, and recovery paths.
