...
| DA-05 | Which Network/Namespace persistence and commitment representation is shared, if any? | F020/F021 expose cross-domain Store import and `network/epoch/merkle` placement without a selected shared foundation. | Proposed [R-060](../research/records/r-060-domain-owned-persistence-and-commitments.md) selects domain-owned representations; Product Owner acceptance; ADR only if a shared storage/commitment foundation is later selected. | Do not fold either domain into the other or introduce a generic shared package. |
