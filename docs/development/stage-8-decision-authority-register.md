...
| DA-02 | What is the single Network State missing-current recovery rule? | F014 contradicts the accepted R-027/R-029 recovery interpretation; D01/W01 repairs have no safe meaning until reconciled. | Proposed [R-059](../research/records/r-059-network-state-missing-current-recovery.md) reconciles R-027/R-029 with a failure/restart transition table; Product Owner acceptance; ADR only if persistence/protocol policy changes. | Do not alter missing-pointer repair, pointer format, or recovery oracle. |
...
| DA-05 | Which Network/Namespace persistence and commitment representation is shared, if any? | F020/F021 expose cross-domain Store import and `network/epoch/merkle` placement without a selected shared foundation. | S8.3 representation alternatives plus compatibility analysis; Product Owner acceptance; ADR if a shared storage/commitment foundation locks the model. | Do not fold either domain into the other or introduce a generic shared package. |
