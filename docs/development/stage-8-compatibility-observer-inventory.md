...
|---|---|---|
| `cmd/ardents`, `cmd/ardents-node` | Root `README.md` calls them current maintained commands. | Retained product journey, but syntax/plan/result compatibility is not established. M13 makes a C0 event-schema cutover to `ardents-source-event-v1` and `ardents-node-event-v1`; no H3 event-schema reader or support window remains. A current command reference is still required before a support promise exists. |
