...
| `cmd/ardents-route`, `cmd/ardents-service`, `cmd/ardents-bridge`, `cmd/ardents-stream-app`, `cmd/ardents-publish-app`, `cmd/ardents-name` | Root `README.md` lists them as current Stage 1-6 tracer commands. | A current repository statement exists, but it names no external caller, version, or support window. M13 may replace the statement and command only after Product Owner observer disposition. |
