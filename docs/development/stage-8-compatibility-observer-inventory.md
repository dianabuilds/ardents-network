...
| `cmd/ardents-release` | Package map and completed S7.2 briefs reference offline import/apply behavior. | No current product/operations support promise found. R-088/ADR-0030 classify its V0 input/result as unobserved test representation: M13 must complete the C0 V2 fixture cutover and retain exact V0 vectors only through a C4 verifier, then delete the command. It is not a bounded C2 operator route. |
