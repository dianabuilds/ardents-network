...
| `cmd/ardents-stream-app`, `cmd/ardents-publish-app` | Root `README.md` lists them as current Stage 1-6 tracer commands. | A current repository statement exists, but it names no external caller, version, or support window. M13 still decides their command shapes. |
