...
| `internal/custody` | Retain the M12 owner for canonical envelope admission, encrypted Vault records, explicit bounded secret use, and public header inspection. It neither releases root material nor allows Release/Update/Endpoint to mutate custody state. | M12 has Bundle export/test restore, quarantine/reconciliation, and custody-derived Name control signing. Its old V0 `custody_notice` is now an explicitly bounded C2 tracer field, while Authority-lifecycle revocation and platform qualification remain open. |
