...
| `internal/custody` | Vault/Recovery Bundle, unlock/export/restore/reconcile, revocation, and signing watermark. | New Module; secrets never enter Release/Update/diagnostics. The first M12 slice owns canonical envelope admission, independent encrypted Vault-record create/verify, distinct-password Bundle export/test restore to a new destination, encrypted authority-locked quarantine restore into an empty Vault, and an exact local Authority floor that advances only after encrypted record publication; it returns no root material. | ADR-0021; DA-08 and DA-09 remain required for platform and full lifecycle qualification. |
