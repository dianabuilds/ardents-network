...
| Service identity (16) | Retain one Instance root; the old decryptor was retired by ADR-0094. | F-42 closed by ADR-0102: Credential v3 request/response grammars; pre-v3 and phase-less roots meet the typed `ErrLegacyRoot` refusal. |
...
| Fifteen production files in `internal/naming/resolution` | Resolved by ADR-0100 on F-51's first-candidate disposition: the package, its tests, and the whole OHTTP dependency closure are removed; the command refusal's zero-effect oracle now stands on a bounded durable-root fixture. The Namespace-side Gateway/verifier views remain pending the Namespace root disposition. |
...
| Portable profile scaffold (`ConfigHome/grants`, `StateHome/vault`, `StateHome/diagnostics`, `CacheHome`) | The test-only `Run` facade was removed by ADR-0099; no non-test Go consumer uses these created directories. Review external operator assumptions, then contract the profile around the actual first-enrollment and current-program owners without deleting existing state (F-26). |
...
closed by ADR-0102 with the typed-refusal disposition. The remaining Route
v2-era obligation is exactly one data disposition: the duty spend-ledger
migration-or-refusal decision (F-53).
...
| 2. Repair observed authority and outcome gaps | The accepted four-domain Epoch-to-closed-profile mapping is enforced in the State owner and a signed mismatch is refused before durable profile acceptance and listener start (F-45, ADR-0103). Keep State root/role close failures in the terminal result, stop emitting an accepted Source-wave event for `--resume`, and give Issuer roots a later close owner after bounded drain timeout. Account for control-admission Hosting release results and the shared handle's late-child lifetime (F-62). Retain Catalog, Release and Network inspection close outcomes (F-64), join Endpoint replacement writer-lock release failures to their public result (F-68), and join Reachability and Instance startup failures with their exclusive-lease cleanup failures (F-71/F-72). | The full-Epoch valid-signature mismatch refusal case landed with ADR-0103; focused combined-error, resume/actual-wave, release-failure, inspection-close, replacement-lock-close, Reachability/Instance startup lease-release, and delayed-child/late-close tests remain. Unchanged signed bytes and one accepting closed Route version. These corrections precede package moves. |
...
| Closed Entry set | `cmd/ardents entry recipient/import` opens the older Invite root and can still write it. `route.OpenEntryAttachment` (ADR-0093) and the uncalled attachment machinery — `Acquire`, `Contact`, attempt-journal writers, guarded carrier, cleanup leases — are retired by ADR-0095; the durable attempt/contact journal schema remains decodable. The closed-alpha issuance surface — exported `entry.Issue` and `Verify` with their `Authorization`/reservation policy — is rejected and removed by ADR-0096; only the internal `validateInvite` classifier remains in the import/reopen path (F-08). | Endpoint opens `entry.OpenClosedSets` for its current State-selected members. The Invite command does not feed this root or the protected participant. Both roots share filesystem helpers but reject each other's marker. | Choose the existing Invite roots' recovery/refusal and accepted operator-contract disposition, then stop the old writer; its uncalled attachment machinery is already retired (ADR-0095), and the retained attempt/contact journal schema is decided with that disposition. Keep the internal verifier required to read retained Invite records until their data disposition is closed. A temporary reader is not another supported runtime or a reason to move two files into a premature subpackage. |
