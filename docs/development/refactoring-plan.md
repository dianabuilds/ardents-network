...
| **M2 — Update lifecycle** | Input: `internal/updatetransaction`, Release interface, `cmd/ardents-release`. Owner: `update`. Objective: own the bounded H3 technical-tracer stage, stop/drain, activation, self-test, journal, recovery, rollback, and cleanup. | DA-01, R-064, and R-087 are mandatory. D07 retains C1 forward-repair/safe-rollback behavior without selecting a supported lifecycle. | Checkpoint/fault/recovery/idempotence/rollback/residue, V1→V2 conversion/non-resurrection, and C4 V0 verifier tests. Real supported filesystem/activator evidence is explicitly out of scope. Delete `internal/updatetransaction/` and caller-authored transaction choreography. |
...
the R-064 `cmd/ardents-release` tracer renders the old key. R-087 selects the
M13 C1 V1→V2 root conversion, C4 V0 verifier, and command deletion; no second
