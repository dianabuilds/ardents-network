...
| **M1 — Release trust** | Input: `internal/releasedecision`, `cmd/ardents-release`. Owner: `release`. Objective: own verified metadata, root/floor/archive, lease, and opaque update authorization. | DA-01 is mandatory. D06 uses C1 security-forward migration: roots/floors never decrease; no staging/store export survives. | **Complete, 2026-08-23.** `release` owns the D06 root/floor/archive and yields only opaque Update authorization; `internal/releasedecision` is deleted. |
| **M2 — Update lifecycle** | Input: `internal/updatetransaction`, Release interface, `cmd/ardents-release`. Owner: `update`. Objective: own the bounded H3 technical-tracer stage, stop/drain, activation, self-test, journal, recovery, rollback, and cleanup. | DA-01, R-064, and R-088 are mandatory. D07 retains its current forward-repair/safe-rollback behavior without selecting a supported lifecycle. | **Complete, 2026-08-23.** `update` owns the bounded offline tracer and C4 V0 verifier; no supported installer, activator, or Custody lifecycle is claimed. |
| **M3 — authenticated Network State** | Input: `internal/network/{epoch,epoch/assignment,epoch/merkle,framing,source,state,store}`, `internal/namestore`, `cmd/ardents`, e2e network-source. Owner: `network/state`; `network/source` is its acquisition port. | DA-02 and DA-05 are closed by R-059/R-060/R-061. **R-061 prerequisite:** first transfer only the currently used Namespace root/pointer and proof mechanics to its existing `namestore` owner, preserve its bytes and one writer, and remove both Network imports. D01 is C1 with one current/pending/control writer; W01 is C3 or C0 only after its observer decision. | **Complete, 2026-08-22.** State owns the one current/pending/control writer; obsolete Network State subpackages and `namestore` are deleted. |
| **M4 — Duty and Resource** | Input: `internal/localroles`, `internal/resource`, callers in State/Node/Route/Bridge. Owners: `network/duty`, then `resource`. Objective: one monotonic duty writer and one finite resource coordinator. | D02 is C1 with no generation/watermark reset. Resource platform scope must be explicit; unsupported platform fails closed. | **Complete, 2026-08-22.** Duty preserves its compatible root with one writer; Resource is Linux-only and fails closed elsewhere. |
| **M5 — Namespace** | Input: `internal/{nameadmission,nameauthority,nameclaim,namelease,namerecovery,namestore,naming}`, including the R-061-owned local persistence/proof mechanics, name command, Stage 6 fixtures. Owner: `naming/namespace`. | DA-03/DA-04/DA-07 are closed for this wave by R-065/R-066/R-067; DA-05 is closed by R-060/R-061. Retain only R-067 profile facts. The target is the R-066 127-record technical tracer, not a product scale selection; D04/W03 internal tracer bytes are C0 unless a later observer decision says otherwise. | **Complete, 2026-08-24.** The six former Namespace-state packages are nested cohesive modules; the retained root owns only opaque Resolution composition views. Historical C4 fixtures import their concrete module owner directly. |
| **M6 — private Resolution** | Input: retained private-resolution behavior, current name command and e2e role fixtures. Owner: `naming/resolution`. | DA-03, DA-04, and DA-07 are mandatory. C3 applies to retained OHTTP/wire behavior; no plaintext fallback or shared implementation view. | **Complete, 2026-08-23.** Resolution uses opaque Namespace/State views and its separate Gateway/Relay process evidence; `internal/nameresolution` is deleted. |
...
| **M13 — command and operator consolidation** | Input: all retained product commands, plans/config/results, README/current operator routes. Owners: thin `cmd/ardents`, `cmd/ardents-node`, conditional bootstrap. | DA-10 governs every observed command/config/result. Each gets C0, C1, or C2 with a named observer and expiry; plans never become authority. | **Complete for Stage 8, 2026-08-24.** Current commands, owner-scoped inputs, bounded receipts, current command reference, and process tests replace the retired tracer commands. Unexposed future Custody lifecycle operations do not constitute a missing command route. |

**M13 complete for Stage 8, 2026-08-24.** The sole repository-visible observer for
...
reader for their former H3 event schemas. Their State, source, and Node
lifecycle JSON events therefore make one C0 cutover to
`ardents-state-event-v1`, `ardents-source-event-v1`, and
`ardents-node-event-v1`. Command and process tests assert those exact schemas;
...
on Go `go1.26.6 windows/amd64`, M1 used the closed DA-01 recovery boundary.
...
closed by DA-01 and the bounded H3 technical-tracer scope, M2 moved the
...
former V0 command/result is not a C2 observer. ADR-0030 completed the C0 V2
fixture cutover, independent C4 verifier,
...
composition seam. The former broad alias façade is deleted: historical verifier
tests now import their current technical module owner directly.
