...
| M09. Service authentication и logical stream | `internal/endpoint/text_service_binding.go`, `protected_service_tls.go:secureProtectedServiceClient/secureProtectedServicePublisher`, `text_service_stream.go:openTextServiceStreamWithRecovery` → `internal/service/connection/stream_lifecycle.go`, `stream_bounded.go:RunBounded`, `stream_recovery.go`; повтор пути: `text_service_route_recovery.go` | `service/connection` — один owner offsets/ACK/EOF/replay/terminal; Endpoint даёт проверенный Attachment и trusted bounds. Exact Service authentication сохраняем | Новые initial opening/continuation/overlap заменяют соответствующие машины состояний и records вместе с caller. Не переносим экспериментальный helper в production как готовый owner |
...
| N17 | Подключить выбранный защищённый Name consumer к обычной команде | N16; ADR-0090 сохранён | Человек использует Service Name без fallback; старые командные adapters уже удалены. Retained `naming/resolution`/OHTTP удалены по ADR-0100; новый consumer требует заново выбранного wire и dependency review, без повторного #99 |
...
`internal/naming/resolution` был сохранён ADR-0090 до отдельного exact-consumer
решения; этим решением стал ADR-0100 — пакет, его тесты и OHTTP-зависимости
удалены, история сохранена в Git, а zero-effect oracle отказов стоит на
bounded-фикстуре.
