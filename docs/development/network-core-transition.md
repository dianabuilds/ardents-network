...
|---|---|---|
| `internal/endpoint/alpha_control.go`, `alpha_control_test.go`; у `AcceptAlphaCorpusControl` найдены только dedicated test callers | Историческое чтение/inspection и floors остаются у naming/alpha и control inspection; новый intake снят | #232: перепроверить call closure, удалить один невызванный adapter и exclusive test |
...
| Make target `prepare-native-rendezvous-host`, `tests/qualification/native-rendezvous-host`, contributor reference | Current contributor lifecycle и generic install helpers с действующими callers; существующие установки не меняются | #239: снять один старый entrypoint и exclusive script closure |
| `experiments/multi-agent-real-2026-09-04` | Нужные факты у current owner, исходники/результаты в pinned provenance | #240: одно дерево; незавершённая приёмка discontinued/not qualified |
| `experiments/long-running-simulation-2026-09-04` | Та же evidence дисциплина; непроведённые trials не объявляются принятыми | #241: отдельное дерево и его ссылки |
| `experiments/multi-node-network-2026-09-04` | Уникальные проверяемые факты и именованный current consumer, если найден | #242: отдельное дерево; нужный перенос до удаления |
