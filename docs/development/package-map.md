...
| `internal/qualification/recoverysmoke` | `package recoverysmoke` | Orchestrate the bounded local Stage 4 S4.1/S4.2 Docker campaign and own its current Docker/Compose host observer, external faults, Docker-specific public evidence projection, verifier invocation, and cleanup while that projection migrates toward the accepted technology-neutral observation contract. | `internal/qualification/byteio`, `internal/qualification/recovery`, `internal/qualification/routesmoke`, `internal/qualification/servicenegative`, `internal/route`, `internal/serviceconn`, `golang.org/x/sys/unix`, standard library |
