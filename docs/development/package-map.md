...
| `cmd/blocked-entry-lab` | `package main` | Prepare one external frozen S5.5 specification or parse one bounded Stage 5 campaign invocation and call the evidence-only harness without authoring a verdict. | `internal/lab/blockedentry`, standard library |
| `cmd/blocked-entry-verify-lab` | `package main` | Parse immutable Stage 5 evidence inputs, call the separately built independent verifier, and render its canonical result. | `internal/lab/blockedverify`, standard library |
...
| `internal/lab/blockedentry` | `package blockedentry` | Own S5.5 external specification preparation and the bounded Stage 5 hostile matrix, private canary and secret artifacts, immutable manifest, canonical observations, fault injection, and complete cleanup inventory behind `Run`; never author a verdict. | `internal/lab/sourceidentity`, `golang.org/x/sys/unix` on Linux, `golang.org/x/sys/windows` on Windows, standard library |
| `internal/lab/blockedverify` | `package blockedverify` | Independently recompute one canonical Stage 5 `pass`, `fail`, or `invalid` result from immutable manifests, observations, private canaries, secret commitments, and cleanup inventory behind `Verify`. | `golang.org/x/sys/unix` on Linux, `golang.org/x/sys/windows` on Windows, standard library |
