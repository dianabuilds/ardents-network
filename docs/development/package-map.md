...
| `cmd/ardents-name` | `package main` | Validate canonical Service Name/Record encodings or execute one S6.2 private resolution selected from a recovered authenticated Network State without a naming control operation or fallback. | `internal/nameauthority` in command behavior tests; `internal/naming`, `internal/namelease`, `internal/nameresolution`, `internal/network/state`, `internal/planfile`, standard library |
| `cmd/ardents-bridge` | `package main` | Import Bridge Invites or compose one authenticated Bridge-domain Node lifecycle with the selected candidate server and its manifest-bound next Initiator leg. | `internal/bridge`, `internal/camouflage`, `internal/localroles`, `internal/network/state`, `internal/node`, `internal/node/probe`, `internal/planfile`, `internal/routeplan`, standard library |
...
| `internal/localroles` | `package localroles` | Own the bounded, owner-only, durable Endpoint-local duty generation used for authenticated identity and family conflict decisions across maintained role producers. | `golang.org/x/sys/windows` on Windows, standard library |
...
| `internal/namelease` | `package namelease` | Own S6.1 Name Lease, Name Generation, canonical Name Record encoding, parent binding, and monotonic transition invariants. It does not authenticate authorities or select recovery/ordering mechanisms. | `internal/naming`, standard library |
| `internal/nameauthority` | `package nameauthority` | Own the accepted S6.2 Ed25519 Name Authority identity, domain/network-bound canonical Record transcript, strict signed-record container, signing, and independent verification without recovery or threshold semantics. | `internal/namelease`, standard library |
| `internal/nameresolution` | `package nameresolution` | Own S6.2 authenticated role selection, immutable signed Record chains, fixed-size OHTTP Client/Relay/Gateway exchange, finite replay rejection, observer-safe counters, and fail-closed local resolution without control operations or fallback. | `internal/nameauthority`, `internal/namelease`, `internal/naming`, `internal/network/state`, reviewed OHTTP/CIRCL closure, standard library |
...
| `internal/node` | `package node` | Own one bounded local Node identity credential, assignment admission, readiness, finite capacity, pressure reaction, durable Endpoint-local duty, drain, withdrawal, and terminal cleanup from narrow authenticated facts. | `internal/localroles`, `internal/node/probe`, `internal/planfile`, `internal/resource`, standard library |
