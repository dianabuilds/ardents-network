...
| `cmd/ardents-instance-fixture` | `package main` | Generate one publisher-host-owned Stage 3 Instance key into a new private file and disclose only its public key to the Authority fixture. | standard library |
...
| `cmd/ardents-qualify` | `package main` | Start bounded black-box qualification or a local H3 Stage 2/3/4 Docker development smoke, host its internal S4.1 fault-controller adapter, and render terminal S4.1/S4.2 machine results. | `internal/planfile`, `internal/qualification/state`, `internal/qualification/node`, `internal/qualification/node/fixture`, `internal/qualification/recoverysmoke`, `internal/qualification/routesmoke`, `internal/qualification/servicesmoke`, standard library |
| `cmd/ardents-recovery-qualify` | `package main` | Read one bounded Stage 4 recovery evidence value, call the independent recovery verifier, and render its exact terminal verdict. | `internal/qualification/recovery`, standard library |
| `cmd/ardents-route` | `package main` | Load one role-local H3 Stage 2/4 tracer sequence, run its Route actors, and render bounded carrier or raw-attachment evidence. | `internal/route`, `internal/routeplan`, standard library |
...
| `cmd/ardents-service` | `package main` | Adapt one role-local Stage 3/4 Endpoint process invocation to bounded JSON readiness and terminal output. | `internal/serviceendpoint`, standard library |
| `cmd/ardents-service-negative` | `package main` | Run the isolated Stage 3 negative Module and emit its bounded receipt. | `internal/qualification/servicenegative`, standard library |
| `cmd/ardents-service-qualify` | `package main` | Read one bounded Stage 3 evidence value, replay the mandatory public-seam negatives, call the independent Service Connection verifier, and render its terminal machine result. | `internal/qualification/service`, `internal/qualification/servicenegative`, standard library |
| `cmd/ardents-service-volume-init` | `package main` | Assign exact networkless Stage 3 qualification socket volumes to the non-root runtime identity. | standard library |
| `cmd/ardents-stream-app` | `package main` | Act as one external opaque-stream tracer Application using only its supplied local byte stream and classified Connection Result. | `internal/applicationipc`, standard library |
...
| `internal/route` | `package route` | Select one complete four-position Route from authenticated Network State, perform the bounded mutually authenticated sealed Introduction setup used by the Stage 4 tracer, and carry a bounded canary, legacy caller-owned stream, or endpoint-secured raw Route Attachment through role-local actors with separate bounded setup and active lifetimes, without receiving Service Instance material. | `internal/network/state`, standard library |
...
| `internal/qualification/campaign` | `package campaign` | Run one technology-neutral qualification cell through prepare, arm, release, observe, freeze, and cleanup; keep candidate, observation, and cleanup results independent; and durably retain one immutable attempt receipt. | `golang.org/x/sys/windows` on Windows, standard library |
...
| `internal/qualification/recovery` | `package recovery` | Independently recompute bounded Stage 4 S4.1 Carrier and S4.2 finite-candidate/leg/Rendezvous/sequential recovery, byte/canary, timing, continuity, resource, negative, and cleanup conjuncts as pass, fail, or invalid. | standard library |
| `internal/qualification/recoverysmoke` | `package recoverysmoke` | Adapt the bounded local Stage 4 Docker fixture to the project-wide campaign cell Interface, validate explicit prior-stage evidence, own its current Docker/Compose observations and external faults, and invoke the independent recovery verifier. | `internal/qualification/byteio`, `internal/qualification/campaign`, `internal/qualification/recovery`, `internal/qualification/service`, `internal/qualification/routesmoke`, `internal/qualification/servicenegative`, `internal/route`, `internal/serviceconn`, `golang.org/x/sys/unix`, standard library |
| `internal/qualification/routesmoke` | `package routesmoke` | Prepare and run the bounded local Docker H3 Stage 2 development campaign, supply its authenticated finite-candidate fixture to Stage 4, collect six-container evidence, invoke the independent verifier, and prove cleanup. | `internal/network/epoch/assignment`, `internal/network/state`, `internal/qualification/byteio`, `internal/qualification/epochfixture`, `internal/qualification/route`, `internal/route`, standard library |
| `internal/qualification/service` | `package service` | Independently verify Stage 3 Target, Credential, migration, principal, stream, process-separation, forbidden-path, resource, and cleanup evidence as pass, fail, or invalid. | standard library |
| `internal/qualification/servicenegative` | `package servicenegative` | Exercise distinct malformed/slow IPC, principal, session, credential, Target, recovery-terminal, migration, and privilege attacks through the public Service Connection seams. | `internal/applicationipc`, `internal/serviceconn`, standard library |
| `internal/qualification/servicesmoke` | `package servicesmoke` | Prepare, run, independently verify, and clean up the bounded local Docker H3 Stage 3 migration campaign while retaining public evidence only. | `internal/qualification/byteio`, `internal/qualification/routesmoke`, `internal/route`, `internal/serviceconn`, standard library |
