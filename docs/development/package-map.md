...
| `cmd/ardents-route` | `package main` | Parse one role-local H3 Stage 2/4 tracer plan, obtain authenticated Network State for Client selection, run one Route actor, and render bounded carrier or raw-attachment evidence. | `internal/network/state`, `internal/route`, standard library |
...
| `internal/route` | `package route` | Select one complete four-position Route from authenticated Network State and carry a bounded canary, legacy caller-owned stream, or endpoint-secured raw Route Attachment through role-local actors over a replaceable laboratory carrier without receiving Service Instance material. | `internal/network/state`, standard library |
...
| `internal/qualification/recovery` | `package recovery` | Independently recompute bounded Stage 4 directional recovery, byte/canary, timing, continuity, negative, and cleanup conjuncts as pass, fail, or invalid. | standard library |
| `internal/qualification/recoverysmoke` | `package recoverysmoke` | Prepare, run, observe, independently verify, fault, and clean up the bounded local Docker Stage 4 recovery campaign while retaining public-only evidence. | `internal/qualification/byteio`, `internal/qualification/recovery`, `internal/qualification/routesmoke`, `internal/qualification/servicenegative`, `internal/route`, `internal/serviceconn`, `golang.org/x/sys/unix`, standard library |
| `internal/qualification/routesmoke` | `package routesmoke` | Prepare and run the bounded local Docker H3 Stage 2 development campaign, collect six-container evidence, invoke the independent verifier, and prove cleanup. | `internal/network/epoch/assignment`, `internal/network/state`, `internal/qualification/byteio`, `internal/qualification/epochfixture`, `internal/qualification/route`, `internal/route`, standard library |
