...
| `cmd/blocked-entry-lab` | `package main` | Parse one bounded Stage 5 hostile-campaign invocation and call the evidence-only harness without authoring a verdict. | `internal/lab/blockedentry`, standard library |
...
| `internal/lab/blockedentry` | `package blockedentry` | Own the bounded Stage 5 hostile matrix, private canary and secret artifacts, immutable manifest, canonical observations, fault injection, and complete cleanup inventory behind `Run`; never author a verdict. | `golang.org/x/sys/unix` on Linux, `golang.org/x/sys/windows` on Windows, standard library |
