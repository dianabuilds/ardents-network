...
| `cmd/ardents-service` | `package main` | Parse one role-local Stage 3 Endpoint plan, bind scoped Unix sockets, call the Service Connection Module, and render its bounded result. | `internal/serviceconn`, standard library |
| `cmd/ardents-service-negative` | `package main` | Exercise the public Service Connection seam in an isolated process and emit the bounded mandatory-negative receipt. | `internal/serviceconn`, standard library |
...
| `cmd/ardents-stream-app` | `package main` | Act as one external opaque-stream tracer Application using only its supplied local byte stream. | standard library |
