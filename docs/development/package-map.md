...
| `internal/lab/blockedentry` | `package blockedentry` | Own S5.5 external specification preparation and the bounded Stage 5 hostile matrix, private canary and secret artifacts, immutable manifest, canonical observations, fault injection, and complete cleanup inventory behind `Run`; never author a verdict. | `golang.org/x/sys/unix` on Linux, `golang.org/x/sys/windows` on Windows, standard library |
