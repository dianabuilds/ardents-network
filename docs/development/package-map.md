...
| `internal/bridge` | `package bridge` | Own strict Bridge Invite validation, the two-slot single-replacement Entry Set, replay history, exclusive state-root lifetime, restart revalidation, and atomic bounded persistence. | `internal/network/state`, `golang.org/x/sys/windows` on Windows, standard library |
