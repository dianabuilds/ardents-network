...
| `internal/network/state` | `package state` | Orchestrate authenticated Network State acceptance, current and pending decisions, finite acquisition, clock confidence, and durable publication. | `internal/network/epoch`, `internal/network/epoch/merkle`, `internal/network/framing`, `internal/network/source`, `internal/network/store`, `internal/qualification/epochfixture`, `internal/resource`, standard library |
...
| `internal/qualification/state` | `package state` | Read bounded persisted Network State evidence, delegate canonical chain and decision verification to Network Epoch, and return the terminal verdict. | `internal/network/epoch`, `internal/qualification/byteio`, standard library |
