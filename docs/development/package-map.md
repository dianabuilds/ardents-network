...
| `internal/namelease` | `package namelease` | Own Stage 6 Name Lease, Name Generation, Name Record state contracts, and monotonic transition invariants that all naming operations consume. | standard library |
| `internal/nameauthority` | `package nameauthority` | Own Stage 6 authority-transition gates (claim/renew/release/rotation/transfer/recovery), privilege continuity, and recovery-policy enforcement checks. | `internal/namelease`, standard library |
| `internal/nameresolver` | `package nameresolver` | Own Stage 6 private-resolution role separation and fail-closed failure surfaces across query and location visibility paths. | `internal/namelease`, standard library |
