...
| `cmd/ardents-bridge` | `package main` | Open current authenticated Network State and Endpoint-local duty truth, parse one bounded local Bridge import plan, invoke the transport-neutral owner through the selected candidate validator, and render the classified result. | `internal/bridge`, `internal/camouflage`, `internal/localroles`, `internal/network/state`, `internal/planfile`, standard library |
...
| `internal/camouflage` | `package camouflage` | Validate the selected WebTunnel candidate envelope into one immutable candidate- and identity-bound configuration without network or process activity. | standard library |
...
| `internal/node` | `package node` | Own one local Node identity's assignment admission, readiness, finite capacity, pressure reaction, durable Endpoint-local duty, drain, withdrawal, and terminal cleanup from narrow authenticated facts. | `internal/localroles`, `internal/node/probe`, `internal/resource`, standard library |
