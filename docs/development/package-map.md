...
| `cmd/reference-site-lab` | `package main` | Parse fixed Gate C commands, derive one experiment identity, call `siteexperiment`, and translate its result to an exit code. | `internal/experimentrun`, `internal/siteexperiment`, standard library |
...
| `internal/siteexperiment` | `package siteexperiment` | Own the bounded Gate C Named Unlisted Site scenario, role processes, security decisions, evidence, and cleanup. | `internal/experimentidentity`, `internal/experimentrun`, `internal/nativecircuit`, standard library; reviewed OHTTP closure |
