...
| `cmd/ardents-service` | `package main` | Parse one role-local Stage 3 Endpoint plan, bind scoped Unix sockets, call the Service Connection Module, and render its bounded result. | `internal/applicationipc`, `internal/planfile`, `internal/serviceconn`, standard library |
...
| `internal/route` | `package route` | Select one complete four-position Route from authenticated Network State and carry a bounded canary or caller-owned opaque stream through role-local actors over a replaceable laboratory carrier. | `internal/network/state`, standard library |
| `internal/serviceconn` | `package serviceconn` | Own bounded local admission, current Service publication, exact Target/Instance authentication, migration generation, and Application-visible Service Connection byte semantics. | standard library |
...
| `internal/qualification/servicenegative` | `package servicenegative` | Exercise distinct malformed/slow IPC, principal, session, credential, Target, migration, and privilege attacks through the public Stage 3 seams. | `internal/applicationipc`, `internal/serviceconn`, standard library |
| `internal/qualification/servicesmoke` | `package servicesmoke` | Prepare, run, and clean up the bounded local Docker H3 Stage 3 migration campaign while retaining public evidence only. | `internal/qualification/byteio`, `internal/qualification/routesmoke`, `internal/route`, `internal/serviceconn`, standard library |
