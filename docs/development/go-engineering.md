...
local pre-commit gate. CI independently runs `make check`; its selected Linux
`package-e2e` profile alone elevates to root to exercise `dpkg` and `setpriv`.
A missing privilege invalidates that profile rather than skipping it. The hook
