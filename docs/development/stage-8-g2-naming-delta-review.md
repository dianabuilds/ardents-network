...
| F024 | **Substantively repaired in M5 (`0c88d99`, `ee2769d`, `b781093`); open pending production composition and evidence replacement.** R-070/ADR-0023 require a Gateway decision to derive and verify the exact Authority-signed successor before atomically appending its opaque submission to the immutable pending journal. A threshold materialization may select only the durable pending successor prefix, persists its cursor in the generation integrity binding, and durable `OpenControl` restores the unapplied chain after restart. The real Gateway still needs to be composed with that durable authority rather than historical/test adapters; Stage 6 remains provenance, not proof of the new transaction. |
