...
| F028 | **Partially repaired in M5; open pending Epoch-ingestion cutover.** Durable Gateway control rejects `claim` before admission or journal mutation, so an authenticated current-state submission can no longer consume a late root-claim challenge. The historical Stage 6 `NewControl.Apply` route remains C4 provenance. Namespace still needs the target typed Epoch-input fact that verifies commit-bound admission once and supplies the global close/winner without rerunning Claim Order. |
