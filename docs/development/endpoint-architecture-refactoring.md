...
| Protected Service Connection | `textServiceBinding`, `textServiceStream`, `protected_service_attachment.go` | Context admits the exact Job and publication; the binding rechecks immutable authority. The Attachment owner authenticates the first transport and each replacement, returning the Publisher lease to stream cleanup and closing replacement transport on failure. The stream owns the logical Connection, Application half-close, and terminal join. These lifetimes still use Context and Publisher publication ownership, so a package split would expose those internals. |
