...
| Introduction delivery dispatch | `textIntroductionDispatch`, `textIntroductionRecoveryOwner` | Dispatch owns context-local waiter slots, one consumer gate and recovery slots under the Context lock. Recovery owns its buffered capsule, deadline refusal and waiter/retirement handoff; Context checks job authority and joins claimed deliveries. |
