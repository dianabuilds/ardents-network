...
| Protected Service Connection | `textServiceBinding`, `textServiceStream`, `protected_service_recovery_attachment.go` | Context admits the exact Job and publication; the binding rechecks immutable authority. The recovery Attachment operation owns one replacement raw transport through TLS and native Attachment construction, including failure cleanup. The stream owns the logical Connection, Application half-close, and terminal join. These lifetimes still use Context and Publisher publication ownership, so a package split would expose those internals. |
