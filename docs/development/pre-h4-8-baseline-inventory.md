...
| `experiments/r-118-credential-relay/`, `r-118-entry-carrier/`, and `r-118-private-transit-issuance/` | Retained decision evidence | **Retain pending maintained-test crosswalk.** R-118 explicitly requires a cleanup decision on whether its distinct negative transcript cases are duplicated by maintained tests. |
| `experiments/r-092-*`, `r-094-*`, `r-095-*`, `r-096-browser-loopback/`, `r-098-alpha-control-catalog/`, `r-101-endpoint-state-attachment-profile/`, and `r-102-endpoint-liveness-lock/` | Decided research candidates | **Pending per-directory fact migration.** Each README declares disposable status, but retirement still requires an exact maintained-test/technical-owner crosswalk for its measured counterexamples. |
