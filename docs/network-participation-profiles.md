...
source of truth for configuration validation until later Phase 3 slices add
new product paths.
...
`tcp_quic`, WebTransport, and WebRTC remain unsupported under DEC-STB-003.
...
be assessed together with STB-304 and STB-306 rather than silently widening the
current TCP/WSS contract.
...
itself proof of external reachability; STB-304 owns that observation.
...
## 7. Remaining Phase 3 Gates

- STB-305: real constrained Filter/Lightpush product paths and capability
  reporting;
- STB-306: enforced resource/abuse controls and real restricted-defense
  reduction are implemented; multi-host transition proof remains STB-307;
- STB-307: multi-host recovery proof across the supported matrix.
