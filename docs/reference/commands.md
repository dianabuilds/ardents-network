...
| `endpoint open <application-socket> <target-link> <input-file> <output-file>` | Return `endpoint open is retired` before opening either file, dialing the local socket, or causing Endpoint/Network work. The former accepting AAI2 file client is absent; no generic AAI3 translation or fallback is selected. |
