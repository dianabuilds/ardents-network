...
| `inspect-public-control` | `--evidence PATH --at RFC3339 --audit-floor-generation N --expected-predecessor DIGEST` | H4-6C readiness diagnostic. It reads one bounded regular `ardents-public-control-evidence-v1` manifest and emits only its content-addressed declared custody, Candidate View, package, builder, auditor, and transition findings. The floor and predecessor are explicit external audit inputs, not locally retained authority. It identifies malformed, stale, replayed, revoked, conflicting, unavailable, and declared-boundary-collision evidence. `qualified` is always false and `external-evidence-required` is always present: this command cannot treat project keys, VPS, CI, Docker, or a Product Owner walkthrough as independent custody/build/audit evidence. It neither accepts an Epoch/package nor writes any floor. |
| `simulate-public-control` | no arguments | Runs one local H4-6C mechanics simulation with fresh in-memory project-controlled keys: routine `3-of-5`, expiring disable-only emergency `4-of-5`, bidirectional lifecycle rotation, two full Candidate View reconstructions, two retained package attestations, and named rejection cells including the reader matrix. Its report is marked `simulation: true` and `qualified: false`; it creates no authority or proof of independent operation. |
