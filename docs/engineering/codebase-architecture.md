...
| `deployment` | strict bounded multi-host topology manifest admission, deterministic redacted host-local plan compilation, abstract three-Node observation workflow, conservative clock-skew preflight, bounded status projection, exact Authority recovery coordination and deterministic Authority-last/Authority-first order projection | SSH, signer/session adapters, host or network mutation, Authority/repository truth or administration, archive extraction, rollout/fence journals or real-host qualification |
...
| `cli` | operator command model, pin-validated host-local observation adapters, human/JSON presentation, shell and TUI UX | product state transitions, RPC server behaviour, topology admission/status policy or real-host qualification |
