...
| W3-D002 | Application Messaging addressing and delivery model | DR-01 | accepted | Opaque authority-backed conversations with Operator-owned membership, five bounded Application operations, durable local send acceptance protected by an independent Messaging checkpoint, same-membership Authority rotation for delivery-Node rebind, bounded close by tombstone/non-renewal/expiry, recipient-local cursor order, at-least-once transport, Node receipts distinct from Application consumption, and Content References for large immutable data | ADR-0015 | Messaging implementation |
...
| W3-D004 | First claimed multi-host release topology and reachability | DR-04 | accepted | Exactly three operator-owned Linux amd64 hosts with `private_lan` and `public_direct` variants, at least two bootstrap/Store Nodes, one designated authority slot, and bounded workstation-side topology operations including monotonic fence/rejoin | ADR-0013 | Future multi-host DR-06 scope; excluded from independent stabilization release |
