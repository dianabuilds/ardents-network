...
- `route.EntryBinding` authenticates an exact User-to-Initiator attempt, and
  the Initiator listener then accepts only `RelaySetup` with next role
  Rendezvous under ADR-0033.
...
- **Sourced fact:** the current Initiator duty reads only `RelaySetup` and
  requires its next role to be Rendezvous.
...
Choose H1's `ResolutionRelaySetup` candidate if the Product Owner accepts its
new Route wire and finite Initiator duty. It keeps endpoint-adjacent exposure
...
the new Route wire/duty surface; its acceptance must precede implementation.

## Disposition

**Decided — H1 accepted by the Product Owner on 2026-08-25.** ADR-0037 fixes
the new closed `ResolutionRelaySetup` operation. It unblocks implementation of
the Endpoint-private carrier; it does not yet qualify the descriptor, Store,
Gateway, or browser path for alpha use.
