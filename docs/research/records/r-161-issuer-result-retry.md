...
- `internal/route/closed_issuance_operation.go`, `closed_admission_channel.go`, `closed_source_control.go` and the private-admission owner: one OPERATION and RESULT are each 16,384 bytes; class-1 child has 64 KiB per-direction credit and at most 30 seconds. The forward parent has its own 32 MiB reserve and original deadline. Today `exchangeControl` computes a fresh up-to-30-second child end on every attempt, so the proposed shared 30-second window is not already implemented.
...
**Sourced fact, result identity audit:** `closed_token_issuer_ledger.go` persists a reservation, not status or signature bytes. `ClosedTokenIssuer.issue` revalidates current profile, permission and key before that ledger lookup, then recomputes blind signatures. `closed_token_issuer_test.go` verifies byte-equal issued signatures and a byte-equal terminal body after issuer restart **with the same terminal nonce**. Actual `ClosedSourcePrefix.ExchangeIssuer` samples a fresh terminal nonce on each transport attempt, and `issueTerminalOperation` echoes it in RESULT. Thus the exact invariant for a real retry is identical signed batch/request ID/digest/kind, one issuer debit and identical issued credential payload/signatures; the terminal RESULT has a new matching nonce and is not byte-identical as a whole. An expired permission or unavailable current profile can return unavailable despite the durable reservation. A failure after reservation but before successful signing is not proof that issued RESULT bytes were committed. The original lost-RESULT oracle must inject loss **after** successful result generation and before Endpoint receipt.
...
| Child traffic ceiling | Existing 64 KiB per direction per child; at most 128 KiB per direction across two children, charged to the existing parent |
| Issuer duty reservation | Two channel slots and 2 × 16 KiB control queue reserve, subject to the existing 1,024-channel/64 MiB duty ceilings |
| Issuer host reserve inputs | Two class-1 reservations: each requests AdmissionTraffic Tx/Rx = 128 KiB and TerminationTraffic Tx/Rx = 16 KiB; pair inputs total 256 KiB admission plus 32 KiB termination in each direction |
...
The first admission must reserve or otherwise prove payable the additional restricted child and a full 30-second window before promising recovery. If the original authority or parent has less time, or host/duty/parent capacity cannot pay both children, refuse before issuer debit rather than leave a committed batch with an illusory recovery guarantee. The 15+15 split guarantees a scheduled retry opportunity after a first RESULT loss only while the exact Source parent, State/profile, permission and issuer stay live; it does not guarantee delivery against an adversary, parent failure or outage. Losing the parent cannot silently buy a new bootstrap batch or Route under this result, so that case must end unavailable. The current issuer host input comes from `internal/node/closed_hosting.go`; the duty control reservation comes from `internal/route/closed_duty_limits.go`. These are reserve inputs, not measured physical traffic or a finding that the host can pay them. The terminal grammar has no delivery acknowledgement, so a recovery promise must retain its future capacity until the fixed recovery end even if the first RESULT was delivered. Issuer restart must reconstruct unexpired promises before admitting competing work; Endpoint process crash still burns its volatile blinding state. Whether the recovery child can be pre-reserved and reconstructed under current host accounting is a separate capacity proof obligation; #84/R-160 is not accepted evidence for it. A provisional duplicate presentation may parse only the fixed request needed to prove the committed exact reservation, under the pre-admission rate/concurrency/byte cap. It may not expose a general tokenless issuer API. The original token bytes stay volatile with the pending batch and are erased on success, terminal exhaustion, revocation or process crash. A crash loses the ability to unblind, as the current contract already states.

A numeric falsification oracle: one-token stock, 32-token pending batch, first admission and committed ledger record, first RESULT lost, second restricted admission using the same token material, then the same 32 issued signatures under the retry nonce with **one** issuer reservation, **one** receiver Spend, **two** counted child admissions, no extra permission allocation, and no more than the original deadline. Repeat with a second RESULT lost: no third admission. Change one batch byte or request kind: no old RESULT and no new issuer debit through recovery. Repeat over TCP/TLS and QUIC, and with cancellation/revocation at every durable boundary.
