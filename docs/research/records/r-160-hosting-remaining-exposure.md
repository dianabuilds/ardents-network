...
**Separate lifecycle finding:** The actual-work Carrier pool retains an idle Carrier for up to 120 seconds after the last useful child lease; QUIC-v2 config enables a 1-second keepalive and Node reaps the pool periodically. The parent's hosting Release may already have run, while the retained pool entry can still emit traffic. This is a distinct future-cost/cleanup owner, recorded as [#262](https://github.com/dianabuilds/ardents-network/issues/262). Its required reserve or early-close policy is not silently charged to every #84 parent refill or treated as a proved property of the current build. A fixed-profile warm Carrier qualification must resolve #262 independently.
...
After a valid ADMIT is debited, suppose the selected directional W(D,context) costs 180 and existing termination still covers finish. One refill adds 180, so R=580 and F=250. A concurrent different parent's attempted increment of 160 must refuse because it would leave F=90<L; in a separate branch, a 140-byte increment may commit first and leave F=110, after which the 160-byte contender still refuses. The main-path numbers below assume no competing reservation committed. Each decision runs under the shared period lease and observes the latest committed R. If another measured 80 bytes arrives, U=250, R=580, F=170. After the parent and all children join, Release makes R=0 while U remains 250, so F=750. If measurement or storage becomes uncertain before that Release, the debit remains and no inferred refund occurs. Reserving the 100-byte termination envelope again at refill would change R to 680 and falsely deny an otherwise payable competing increment; it does not buy a second finish.
