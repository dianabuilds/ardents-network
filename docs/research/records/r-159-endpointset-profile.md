...
**Sourced fact (profile coupling):** State's epoch_profile.go uses ardents-route-v3 as the closed Epoch profile, while Route's closed_node_carrier.go uses the same exact literal for TLS ALPN. Route bootstrap and terminal recipient checks compare the State snapshot profile with that Route constant, and State's ClosedProfile acceptance requires the closed Epoch profile. Thus a new Epoch identifier alone makes current callers refuse; retaining the old TLS ALPN under a new Epoch identifier would need an explicit authenticated mapping and owner amendment. The generation-3 HELLO binds a profile digest, but that does not itself select such a mapping.

**Inference (bounded profile split):** The signed Epoch contains its profile string and the generation-3 HELLO carries exact State and ClosedProfile digests. A separate successor **State acceptance** profile could therefore be mapped explicitly to the unchanged Carrier-v2 TLS ALPN and HELLO wire, rather than implicitly renaming that Carrier. This is a candidate only: State, ClosedProfile and Route snapshot checks currently require ardents-route-v3, and no mixed old/new peer test has proved the mapping. Its mandatory falsification case is a v3 peer and a successor peer with individually valid but different State/profile digests: both must refuse before forwarding or Application effects. If this cannot be proved, a separate ALPN/Carrier profile decision is required outside this format-only change. Reusing the v3 ALPN would never permit a v3 State consumer to accept schema 3.
...
Sign the exact prefix from ARNR through NodePublic, including schema, length and tuple order. No second set signature. The State authority must still authenticate Network membership, key, time, role/profile, committed View and exact record digest; self-signature alone grants none of them. One Node/key/generation contributes one capacity, regardless of tuple count. Current epoch_candidate_projection.go appends Endpoint and Carrier to parallel arrays. A successor State projection must expose each selected tuple atomically with the same verified Node, key, generation, exact signed-record digest and profile; callers must not independently index or combine endpoint and Carrier arrays. That projection obligation is a future implementation oracle, not something these record vectors prove. New cross-Node endpoint collision checks compare canonical IP/port across each set, conservatively preserving current endpoint uniqueness even across TCP and QUIC; one Node may use the same IP/port for its own TCP and QUIC tuples.
...
The exact successor Epoch/Carrier profile identifier and interaction with the selected ardents-route-v3 ALPN/HELLO are consequential owner decisions. The suggested name ardents-route-v4 in the external proposal is **not selected here**. The proposed exact State refusal matrix is:

| Epoch/Node consumer | Signed Node Record | Decision before any dial |
|---|---|---|
| Current ardents-route-v3 | Schema 2, one TCP/TLS-v2 or QUIC-v2 Carrier | May accept only after all current authority, time, View and ClosedProfile checks. |
| Current ardents-route-v3 | Schema 1 or schema 2 with Carrier-v1/unknown | Refuse for generation-3 duty under current contract. |
| Current ardents-route-v3 | Schema 3 / EndpointSet | Refuse unknown schema; do not parse as text or partially accept. |
| Explicit successor profile (identifier pending) | Schema 3 / EndpointSet v1 with known mapping | May accept only after all new authority, time, View and ClosedProfile checks. |
| Explicit successor profile (identifier pending) | Schema 1/2 or unknown set version/mapping | Refuse for successor duty even when signature and key are valid. |

Old signed records remain historical evidence; a new verifier must not auto-convert them into new duty. A shared TLS ALPN, if separately selected, must not override these State decisions. Distinct Network/trust context for a clean dev successor is a deployment proposal, not permission to erase retained roots, keys or floors. No automatic conversion or active Attachment mutation follows.
