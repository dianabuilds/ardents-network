...
Choose H2. After Product Owner acceptance, one bounded Naming implementation card should delete the complete internal/naming/resolution directory, including its module-only tests, and remove only references that became false from the owning Naming, package-map, repository-layout and dependency documents. The historical ADR/research records remain provenance. No new package, accepting fake, successor OHTTP exchange or Name migration is introduced.
