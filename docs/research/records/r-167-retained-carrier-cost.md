...
- **Sourced fact:** quic-go `Conn.CloseWithError` waits for its connection context, and the local-close path creates a closed packet handler that can answer later peer packets while the handler exists. `DialAddr` uses a single-use UDP transport that also closes after `conn.run` returns. The exact ordering and possible post-return bytes for this adapter were not established by this source review; treating `CloseWithError` return alone as a zero-future-byte proof would be unsupported.
...
2. **Close on last useful lease under its still-live parent coverage.** Removes the 120-second warm Carrier reuse path. It needs a joined physical-close/tail boundary before the last covering reservation is released, plus no late session-reader invalidation of a replacement. Cold and warm workload gates must pass on both Carriers; no such result is yet available.
