...
| [R-161](records/r-161-issuer-result-retry.md) / [#93](https://github.com/dianabuilds/ardents-network/issues/93) | What finite already-paid retry budget retrieves one durably reserved issuer RESULT after loss, including when its request used the holder's last usable token? | **Prepared; awaiting Product Owner admission choice.** Draft PR #263 proves the current last-token gap, proposes one loss/30-second/two-admission budget, separates issued signatures from the fresh terminal nonce, and identifies conditional persisted/admission decision #264. No current owner, ADR or runtime accepts this proposal; #94 is not Ready. The selected research slot moves to independent #90. |
