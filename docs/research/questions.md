...
| [R-084](records/r-084-publication-generation-owner.md) | What durable publication/generation owner gives M9 one crash-atomic Instance hand-off without retaining the H3 Endpoint action union? | Decide a domain-owned C1 publication root, its immutable generation/current pointer/recovery rule, Instance-private-material lifetime, and supersede/drain behavior. It must not select Custody, local IPC, or a generic storage package. | open |
