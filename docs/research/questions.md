...
| [R-003](records/r-003-service-name-contract.md) | How does an exact Service Name bind to a Service Target, resolve without becoming a directory, survive accepted rotation, and handle registration, expiry, recovery, conflict, enumeration, query privacy, and Control Plane capture? | **Active:** P4-D1 fixes a distinct Name Authority independently of Service Authority. P4-D2a fixes one canonical network-wide Namespace. P4-D2b fixes first-valid deterministic permissionless Name Leases, hierarchical subclaims, explicit conflicts, and no mandatory account, wallet, token balance, or registrar. P4-D2c fixes lowercase ASCII dot hierarchy, rightmost-parent semantics, an explicit `ardents://` Service Link, no Unicode canonical form, and no DNS behavior. P4-D3 fixes Active, Grace, Released, renew/reclaim, Name Generation, monotonic record, cache-freshness, and parent-bounded descendant behavior while leaving numeric durations and convergence open. Authority lifecycle, resolution privacy, governance, and the anti-abuse mechanism remain open; no registry technology is selected. | active |
