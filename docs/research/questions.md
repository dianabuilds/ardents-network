...
| R-165 / [#237](https://github.com/dianabuilds/ardents-network/issues/237) | Where should current TCP/TLS and QUIC Carrier authentication/lifetime adapters live relative to Route lane grammar and Node ownership? | **Selected C0 question, 2026-09-23.** Falsify the hypothesis that extracting a Carrier package creates a smaller, cohesive Interface without import cycles or weakening the current exact State selection, TLS/QUIC, cancellation and Join contracts. Trace actual Node/Route/credential callers and choose an exact import direction; no runtime change is authorized. |
