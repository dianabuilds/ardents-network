...
| R-042 | Which authenticated deterministic claim ordering and ordered-collision proof distinguishes ordered collision from unresolved Conflict, partition, and rule Fork in S6.0, and what is the minimum evidence an honest endpoint must produce to name a deterministic loser? | **Open:** epoch plus claim digest sorts only a locally known set and does not prove inclusion/completeness or prevent copying, grinding, withholding, and partition divergence. Run the R-042 claim-ordering experiment and choose an authenticated inclusion mechanism before S6.5. See [r-042-claim-ordering.md](records/r-042-claim-ordering.md). | open |
| R-043 | Which exact persistence, restart, rollback, and cache-proof ownership boundary lets the Stage 6 Name Lease/Generation/Record state survive restart, fail closed on tamper, and remain replay-bounded without selecting a storage engine, consensus mechanism, or trust root by inertia? | **Decided (semantic storage boundary):** classify durable, restart-derived, and cache-bounded state; fail closed on tamper/stale epoch; use atomic commits. S6.4 may define a naming-owned interface and test an `internal/network/store` adapter, but no generic interface or adapter is claimed to exist yet. See [r-043-persistence-restart-rollback.md](records/r-043-persistence-restart-rollback.md). | decided |
| R-044 | Which maintained mechanism and complete protocol implement the accepted distinct `t`-of-`n` Recovery Authority trust model? | **Open for S6.4:** ADR-0013 is withdrawn. The named BLS package does not exist and aggregation did not implement participant setup, authentication, threshold custody, or replay-safe recovery. R-047 separately owns the narrow S6.2 Ed25519/OHTTP decision. Compare and measure maintained recovery candidates before a recovery ADR. See [r-044-cryptographic-suite.md](records/r-044-cryptographic-suite.md). | open |
| R-045 | Which R-010-compatible finite, measurable Anonymous Cost and local admission profile protects name claim, renewal, resolution, policy, and recovery surfaces from observation copying, front-running, flooding, rollback, and equivocation without money, account, IP reputation, stable identity, wallet, token, or personhood claim? | **Open:** the former rates, work factors, capacities, and TTL were unmeasured hypotheses. Run the R-045 experiment on the R-023 host and a weaker client before freezing a profile. See [r-045-anonymous-cost.md](records/r-045-anonymous-cost.md). | open |
...
| R-055 | Which canonical manifest, observation, cleanup, and verdict serialization lets a separately built verifier establish Stage 6 development completion without trusting runner summaries? | **Open, decision-ready:** S6E1 proposes strict canonical Go-struct JSON indexes, ordinal-derived stream paths, SHA-256 commitments, disjoint roots, one deterministic episode for every A0-D6 cell, and independent mutation verification. Product Owner acceptance and implementation evidence remain pending. See [r-055-stage-6-evidence-serialization.md](records/r-055-stage-6-evidence-serialization.md). | open |
| [R-048](records/r-048-h3-stage-7-contract.md) | What exact bounded contract and decision order makes H3 Stage 7 implementable after Stage 6 without selecting installer, updater, custody, IPC, or isolation foundations by inertia? | **Review:** choose shared Release Decision, Update Transaction, Install Lifecycle, Authority Custody, Application Broker, and Application Isolation Modules with Ubuntu/Windows Adapters; accept the complete documentation set together, keep maintained coding closed, and use the readiness checklist as the sole entry gate. | review |
| [R-049](records/r-049-stage-7-release-verifier.md) | Which maintained TUF-compatible Go verifier and exact bounded H3 metadata profile meet ADR-0006 without first-party update cryptography or a distributor authority? | **Review:** select `go-tuf/v2 v2.4.2` behind one Release Decision Module with raised `x/crypto`/`x/sys`/`x/term`, no cache/network/repository/signing/multi-repository/delegation surface, and the measured finite profile; reject the decayed legacy fork. | review |
| [R-050](records/r-050-stage-7-install-update-adapters.md) | Which Ubuntu and Windows package, stable-bootstrap, immutable-layout, atomic-activation, crash-durability, repair, uninstall, and purge Adapters meet the release-activation decision proposal? | **Open:** freeze exact supported images/filesystems/APIs/privilege and interrupt every transition; reject in-place mutation, unsafe cross-volume behavior, hidden residue, or platform-specific weaker outcomes. | open |
| [R-051](records/r-051-stage-7-application-principal.md) | Which Ubuntu and Windows local-channel and process-tree facts securely bind a launcher-born Application Principal rather than a PID, path, desktop user, or bearer? | **Open:** compare Unix peer credentials and Windows pipe/token/process/job facts under sibling attachment, PID reuse, substitution, failed identity query, restart, revoke, and resource pressure. | open |
| [R-052](records/r-052-stage-7-application-isolation.md) | Which stable Ubuntu and Windows mechanisms can deny ordinary networking for a complete Application/helper process tree with scoped Ardents IPC and bounded privilege? | **Open:** falsify candidates with DNS/listener/fetch/callback/WebSocket/WebRTC/QUIC/socket, child/breakaway, inherited-handle, storage, restart, resource, and cleanup probes; experimental-only APIs and broad privileged daemons do not qualify. | open |
| [R-053](records/r-053-stage-7-authority-recovery.md) | Which maintained cryptographic and platform-storage profile protects Authority Vault and Recovery Bundle state across update, export, test restore, reconciliation, and purge? | **Open:** freeze format/KDF/encryption/dependencies/bounds and prove tamper/wrong-secret/wrong-environment/rollback/locked/export-only/strictly-higher reconciliation without restoring Grants or runtime keys. | open |
| [R-054](records/r-054-stage-7-evidence-profile.md) | Which canonical Stage 7 manifest, evidence, cleanup, and verdict profile independently proves install/update/principal/isolation behavior on both platforms? | **Open:** freeze schemas, paths, clocks, identities, resources, episode counts, mutation corpus, the complete A–H cell inventory, campaign identity, and deterministic `pass|fail|invalid` predicates before implementation evidence. | open |
