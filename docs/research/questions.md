...
| [R-023](records/r-023-interactive-route-performance-budget.md) | What end-to-end performance budget makes the V1 Interactive Route and Named Unlisted Site useful without weakening the accepted security contract? | **Active:** P3-D1 through P3-D4 fix platforms, normal performance, endpoint capacity, resources, fairness, queues, overhead, impaired paths, and recovery. P3-D5a protects the full `256/64` publisher workload during 10 minutes of `1,000` incomplete attempts/s capped at `20 Mbit/s` on a `100 Mbit/s` link: all established streams survive, active aggregate remains `>= 32 Mbit/s`, each remains `>= 400 kbit/s` with no gap over `5 s`, and publisher RSS/CPU remain within `1 GiB`/one core. Honest new admission and established hostile work remain P3-D5b/c; the measurement gate remains open, while role-specific Node capacity and cost follow R-004 evidence. | active |
