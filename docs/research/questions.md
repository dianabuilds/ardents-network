...
development completed on 2026-08-19; Stage 6 coding gate remains closed**
...
| [R-039](records/r-039-h3-private-naming-lifecycle.md) | What exact bounded production slice for Service Name lifecycle is required to prepare Stage 6 and what evidence/verifier split is mandatory before code coupling? | **Accepted scope, coding gate closed:** encoding, lifecycle, delegation, authority/recovery, Target continuity, private resolution and control-operation separation, conflicts/forks, abuse bounds, and independent `pass|fail|invalid` verdicting are required. Stage 5 development has advanced; corrected-document acceptance and the S6.0 freeze for limits, ordering, persistence, cryptography, Anonymous Cost, and role fields remain mandatory before implementation. | accepted |
| [R-040](records/r-040-h3-stabilization-closure.md) | How does Horizon 3 become one clean, technically documented, reproducibly verified baseline before Horizon 4? | **Accepted:** add mandatory Stage 9 after functional integration. It removes or promotes laboratory/transitional code, converts completed-stage development documents into a small active technical set, freezes maintained test/qualification tiers and infrastructure, and independently qualifies the cleaned source before H4. | accepted |
| R-041 | What exact canonical Service Name label length, total length, hierarchy depth, canonical encoding, and schema version freeze the parser, encoder, and Service Link behavior in S6.0 without introducing Unicode, IDNA, Punycode, public-TLD lookup, or DNS fallback? | **Decided (Option B, ASCII strict):** label 1–63 ASCII, total ≤ 253 ASCII, depth ≤ 127, charset `[a-z0-9-]`, delimiter `.`, parent on the right, no leading/trailing/empty label, no leading/trailing or consecutive hyphen, no all-numeric top label, wire encoding length-prefixed, `schema_version = 1` (uint16 big-endian). The product-level rule (R-039 § Fixed product contract) is fixed; this record freezes only the numeric profile and version. S6.1 (encoding/lifecycle) may consume these values. Any future change requires a new `schema_version` and a new record. See [r-041-canonical-name-limits.md](records/r-041-canonical-name-limits.md). | decided |
| R-042 | Which authenticated deterministic claim ordering and ordered-collision proof distinguishes ordered collision from unresolved Conflict, partition, and rule Fork in S6.0, and what is the minimum evidence an honest endpoint must produce to name a deterministic loser? | **Decided (Option A, `(network_id, epoch_number, claim_digest)`):** order key = `(network_id, epoch_number, SHA-256(canonical claim payload))` reusing the R-029 Network Epoch. Proof structure: signed claim payload, current authenticated `(network_id, epoch_number, epoch_digest)`, parent proof, R-041 schema_version=1, R-046 role visibility. Five-state classification with explicit fail-closed: `ordered` (smaller order key wins, larger is `claim-loses-ordered`; winner gets Lease, loser gets nothing), `conflict` (`state-conflict`, no Lease mutation), `fork` (`fork-unresolved`, no Lease mutation, different `network_id`), `unavailable` (`state-unavailable`, no Lease mutation, no authenticated Epoch), `partition` (treated as `state-conflict`, no Lease mutation). Coverage map assigns all eight brief scenarios (observation copying, front-running, withholding, flooding, partition, rollback, equivocation, rule fork) into the five states. S6.5 (concurrency/fork/abuse) may implement the classifier. See [r-042-claim-ordering.md](records/r-042-claim-ordering.md). | decided |
| R-043 | Which exact persistence, restart, rollback, and cache-proof ownership boundary lets the Stage 6 Name Lease/Generation/Record state survive restart, fail closed on tamper, and remain replay-bounded without selecting a storage engine, consensus mechanism, or trust root by inertia? | **Decided (Option B, `Storage` interface + `internal/network/store` default):** Go `Storage` interface in `internal/namelease/store.go` (`Put`, `Get`, `Delete`, `Iterate`, `AtomicWrite`, `Sync`); default implementation is `internal/network/store` (existing H3 supply, no new technology). Durable set: Name Lease state, Name Generation, Name Record (signed Ed25519 per R-044), Recovery Policy (BLS12-381 per R-044), capability counter (R-045). Restart-derived set: ephemeral handles, in-flight admission, resolver query state, uncommitted transaction buffers. Cache-bounded set: resolved Target with `epoch_number` + `epoch_digest` freshness proof, Recovery Policy activation countdown, recently-claimed order-key index. Tamper rule: signed entries, modification → `state-tampered` Connection Result, no read, no recovery. Replay-bound: every key includes `epoch_number`; read with `epoch_number < current` → `state-stale`. Atomic write: write-new → fsync → atomic rename → directory fsync, batched through `AtomicWrite`. S6.4 may implement through the interface. Future engine swap requires a new research record and a new ADR. See [r-043-persistence-restart-rollback.md](records/r-043-persistence-restart-rollback.md). | decided |
| R-044 | Which cryptographic mechanism or explicit replaceable interface for Name Authority signing, Service Instance binding, Recovery Policy threshold, and resolver query hiding is accepted through research (and where the selection creates lock-in, through an ADR) before any Stage 6 implementation slice may select a primitive locally? | **Decided (Option A, suite + interface boundary), ADR-0013:** Ed25519 (Name Authority, RFC 8032, Go `crypto/ed25519`); BLS12-381 (Recovery Policy threshold, `golang.org/x/crypto/bls12381`); OHTTP (resolver query hiding, R-026 supply `openpcc/ohttp v0.0.80` commit `79bec89d8042`); SHA-256 (canonical encodings, order key, commitments); HKDF-SHA-256 (connection-only keying material). Replaceable Go interface boundary: `Signer`, `ThresholdVerifier`, `QueryHider` in `internal/nameauthority/sig.go`. No local primitive. S6.2, S6.4, and S6.5 may import the suite through the interface; any other import requires a new research record and a superseding ADR. See [r-044-cryptographic-suite.md](records/r-044-cryptographic-suite.md) and [ADR-0013](../adr/0013-stage-6-cryptographic-suite.md). | decided |
| R-045 | Which R-010-compatible finite, measurable Anonymous Cost and local admission profile protects name claim, renewal, resolution, policy, and recovery surfaces from observation copying, front-running, flooding, rollback, and equivocation without money, account, IP reputation, stable identity, wallet, token, or personhood claim? | **Decided (Option A, Hashcash PoW + rate limit + capability):** SHA-256 with `k` required leading zero bits; per-endpoint per-epoch rate limit; scoped short-lived capability (TTL ≤ 60 s, scoped to `(endpoint_pubkey, surface, target_epoch)`) for batched operations. Per-surface limits (calibrated to R-023 reference host): claim 1/20 bits/100 cap, renewal 10/16/1000, resolution 100/8/10000, policy 1/18/10, recovery 0/22/1 per generation. Admission order: epoch → role → schema → counter → PoW when required. Fail-closed: exhaustion returns `admission-denied` (R-002), no lease mutation, no override path. No identity coupling. S6.5 (concurrency/fork/abuse) may implement the admission path and verifier. See [r-045-anonymous-cost.md](records/r-045-anonymous-cost.md). | decided |
| R-046 | Which exact field-level role matrix freezes resolution and naming control-operation inputs, observations, stable identifiers, Role Domains, known-family exclusions, and Isolation Context boundaries so no one ordinary role receives both endpoint location and exact name/lookup view? | **Decided (Option A, five roles, per-role concrete types):** `endpoint-adjacent`, `naming-rendezvous`, `local-resolver`, `authority-operation`, `observer`. Each role is a distinct concrete type for request and observation; no superset object, no hidden fields by convention. Forbidden combined view: no role sees both `User/Endpoint location` and `exact Service Name / lookup value` at the same time. Stable identifiers (`query_id`, `session_id`, `nonce`, `ephemeral_handle`) are scoped to one role and one Isolation Context and never appear in evidence or long-term state. Role Domain per ADR-0005; identity/known-family exclusion per ADR-0005 + R-005 + R-035. Isolation Context per CONTEXT.md § Network-Isolated Application Boundary. Fail-closed on missing/invalid role proof: resolution stops, no Lease mutation, classified failure per R-002. S6.2 (role separation) may define per-role concrete types and a contract predicate; S6.5 (concurrency/fork) may reference the role set. See [r-046-role-matrix.md](records/r-046-role-matrix.md). | decided |
