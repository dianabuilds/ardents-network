...
| [R-023](records/r-023-interactive-route-performance-budget.md) | What end-to-end performance budget makes the V1 Interactive Route and Named Unlisted Site useful without weakening the accepted security contract? | **Active:** P3-D1 and P3-D2 fix platforms and latency. P3-D3 fixes goodput, endpoint capacity, resources, fair progress, overhead, queues, and qualified scale-up. P3-D4a requires same-connection recovery after one eligible failure within `p95 <= 5 s` and by `15 s`; P3-D4b1 requires three sequential recoveries. P3-D4b2a fixes 10-minute useful progress at `300 ms` base RTT, `5%` bidirectional loss, and `100 ms` `p95` jitter. P3-D4b2b requires one overlapping pair to recover within `p95 <= 8 s` or terminate by `15 s`, both measured from the first failure without timer reset. Degraded-path and recovery cost, hostile load, and the measurement gate remain open; role-specific Node capacity follows R-004 evidence. | active |
