...
| [R-023](records/r-023-interactive-route-performance-budget.md) | What end-to-end performance budget makes the V1 Interactive Route and Named Unlisted Site useful without weakening the accepted security contract? | **Active:** P3-D1 requires Windows and Linux desktop/laptop User and Developer endpoints and a modest Linux server/VPS infrastructure Node as V1 benchmark and release classes; macOS and mobile are later targets. P3-D2a caps exact-name authenticated connection establishment at `p95 <= 3 s` cold and `p95 <= 1 s` warm. P3-D2b caps installed-process network readiness at `p95 <= 5 s` on routine restart and `p95 <= 15 s` on clean first start. Useful-data latency, throughput, tail behavior, resources, hostile load, and the measurement gate remain open. | active |
