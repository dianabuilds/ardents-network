...
- V1 performance and release gates cover Windows and Linux desktop/laptop
  endpoints for Users and Developers and a modest Linux server/VPS for an
  infrastructure Node; macOS and mobile remain later targets;
...
| [R-023](records/r-023-interactive-route-performance-budget.md) | What end-to-end performance budget makes the V1 Interactive Route and Named Unlisted Site useful without weakening the accepted security contract? | **Active:** P3-D1 fixes Windows/Linux endpoint and Linux/VPS infrastructure classes. P3-D2 fixes connection, endpoint-readiness, and controlled-Site first-byte latency. P3-D3a requires single-connection `p05` goodput of at least `min(10 Mbit/s, 50% of direct baseline)` in each direction. P3-D3b1 requires each client reference endpoint to support at least `128` open outbound connections, including `32` simultaneously active; these are minimum client capacity, not server maxima. Publisher/Node capacity, resources, fairness, degradation, hostile load, and the measurement gate remain open. | active |
