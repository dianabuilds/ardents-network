...
development evidence passed; Stage 4 recovery development authorized**
...
| [R-033](records/r-033-h3-stage-5-research-map.md) | What is the exact Horizon 3 Stage 5 scope and decision order? | **Recommendation ready for review:** limit H3 to authenticated offline/file Invite import, one finite Bridge Entry Set for one selected adjacent Role Domain plus cross-domain negatives, one bounded ordinary-to-Bridge regime transition, one Camouflage Adapter seam tested by exactly two feasibility candidates before one maintained selection, and one controlled blocked-entry campaign. R-034 is accepted and assigns Bridge-specific capacity to Stage 5. R-033 acceptance, R-035 acceptance, R-036 Adapter/supply, R-037 evidence, and an explicit implementation brief remain required before code. Public distribution, general routing, all-leg camouflage, Shielded traffic analysis, production UX, and public claims are excluded. Stage 4 development/local evidence is complete; no Stage 5 code is authorized. | review |
| [R-035](records/r-035-h3-bridge-state.md) | What exact H3 Bridge Invite, finite Bridge Entry Set, exposure, regime-change, restart, expiry, replacement, and one-Role-Domain state machine preserves the R-009/ADR-0005 contract? | **Recommendation ready for review:** use one two-slot Initiator-domain Bridge Entry Set, at most one explicit replacement per slot/Epoch, one owner-approved or precommitted ordinary-to-Bridge transition, at most two contacts per member, one durable non-resetting attempt deadline, and fail-closed restart with retained exposure. R-036 owns candidate configuration; R-037 owns numeric clocks and evidence. | review |
...
| [R-036](records/r-036-h3-camouflage-adapter.md) | Which replaceable Camouflage Adapter interface and pinned maintained implementation can carry the same H3 Bridge workload without selecting a production transport by inertia? | **Blocked after source/contract precommit:** the exact PT seam, pinned Lyrebird obfs4 `0.8.1` and standalone WebTunnel `v0.0.6` sources, offline-build recipe, no-DNS rule, structural resource bounds, shutdown, useful work, and falsifiers are ready for review. No candidate is selected: R-033 requires both feasibility candidates to run before selection, while the controlling Stage 5 gate forbids an Adapter experiment until R-036 is already `decided` and a brief is accepted. Product Owner must explicitly change that ordering; no code, binary, dependency, or experiment is authorized. | blocked |
