...
| [R-037](records/r-037-h3-blocked-entry-evidence.md) | Which controlled address-blocking, protocol-allow-list, active-probe, Invite-replay, withholding, role-conflict, restart, resource, and cleanup cells determine Stage 5 `pass|fail|invalid`? | **Ready for Product Owner review:** proposed `h3-s5-b1-v1` freezes one Ubuntu topology, seven censor/probe profiles, a non-resetting `64 s` Bridge attempt, `15 s` contacts, exact sample floors, four concurrent reference Bridge units, sustained/resource/pressure gates, complete helper accounting, secret/public evidence separation, and an independent verdict. No implementation brief or maintained Stage 5 code is authorized until this record is explicitly accepted. | active |
