...
| [R-023](records/r-023-interactive-route-performance-budget.md) | What end-to-end performance budget makes the V1 Interactive Route and Named Unlisted Site useful without weakening the accepted security contract? | **Active:** P3-D1 fixes Windows/Linux endpoint and Linux/VPS infrastructure classes. P3-D2 fixes separate `p95` gates: connection `3 s` cold / `1 s` warm, endpoint readiness `5 s` restart / `15 s` clean start, and controlled Site first byte `4 s` cold / `2 s` warm. P3-D3a requires single-connection 60-second `p05` goodput of at least `min(10 Mbit/s, 50% of direct baseline)` separately in each direction. Concurrency, resources, tail behavior, hostile load, and the measurement gate remain open. | active |
