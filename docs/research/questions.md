...
| [R-159](records/r-159-endpointset-profile.md) / [#218](https://github.com/dianabuilds/ardents-network/issues/218) | Which one signed EndpointSet record format and incompatible Node profile binds each advertised endpoint to the same current Node identity without importing producer or destination policy? | **Selected C0 question, 2026-09-23.** Compare the prepared EndpointSet candidate with the current State Node Record, exact signature bytes, duplicate/order/limit rejection, and old/new profile refusal. A record accepted with an endpoint or Carrier from a different signed record falsifies the proposed binding. No runtime, producer workflow, destination allowlist or public authority is selected. |
