...
| R-163 / [#233](https://github.com/dianabuilds/ardents-network/issues/233) | Which parts of the retired `OpenServiceAdministration` → Publisher → Transit chain have no maintained non-test consumer, and which state, floor, cleanup or Interface obligations must survive? | **Prepared in draft PR #266; awaiting Product Owner retirement/compatibility choice.** Falsify the hypothesis that the entire chain is exclusive legacy code by tracing production callers, import closure and current text Administration/Publisher use. Produce an exact keep/remove map and owner-separated removal cards; no runtime deletion or AAI3 change is authorized by this row. |
