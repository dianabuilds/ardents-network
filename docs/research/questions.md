...
| [R-160](records/r-160-hosting-remaining-exposure.md) / [#84](https://github.com/dianabuilds/ardents-network/issues/84) | What remaining-exposure formula counts spent bytes, still-payable admitted work and a real termination reserve exactly once across initial admission, refill and close? | **Selected C0 question, 2026-09-23.** Falsify the candidate if a valid admitted obligation exceeds available reserved resources, if a concurrent refill double counts or releases exposure, or if unknown aggregate host delta becomes free capacity. Compare current ledger, allowance, sample and Node caller with numeric initial→refill→close oracles. No runtime/storage/tariff change is selected by this row. |
