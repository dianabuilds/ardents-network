...
| [R-004](records/r-004-routing-rendezvous-families.md) | Which routing and rendezvous families can meet the accepted Interactive Route claim and R-023 performance budget under churn, malicious Nodes, and realistic client devices? | **Active:** P5-D1 fixes the Route Module Seam. Split-leg refinement rejects a combined Service Entry/Introduction role and a stored invitation mailbox, advances Rendezvous-forwarded sealed introduction as the lowest-cost hypothesis, and retains an independent introduction route as the security fallback. A blocking claim issue remains: a Node can independently act as a User and learn a public exact-name Service, so R-001 can forbid one Node from linking Target to either endpoint origin but cannot truthfully guarantee absolute Target ignorance at an Introduction role without new cryptographic evidence. No claim, route, library, cryptography, hop count, or language is selected. | active |
