...
| [R-003](records/r-003-service-name-contract.md) | How does an exact Service Name bind to a Service Target, resolve without becoming a directory, survive accepted rotation, and handle registration, expiry, recovery, conflict, enumeration, query privacy, and Control Plane capture? | **Active:** P4-D1 fixes a distinct Name Authority controlling the Name Record independently of Service Authority. P4-D2a fixes one canonical network-wide Namespace with hierarchical delegation, explicit conflict failure, and no silent local or external fallback. Syntax and allocation, record and authority lifecycles, resolution privacy, and governance remain open; no registry technology is selected. | active |
