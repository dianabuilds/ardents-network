...
| [R-002](records/r-002-live-application-interface.md) | What is the smallest live Application Interface that lets an existing local application publish and consume a Service safely? | **Active:** P1-D1 accepts a local socket/proxy-style boundary and optional convenience-only SDKs. P1-D2 accepts one live reliable ordered byte stream. P1-D3 separates privileges. P1-D4 accepts both destination forms. P1-D5 accepts honest failure classes. P1-D6 assigns every Application a safe local default Isolation Context with optional subdivisions and no network identity. The remaining contract must define local authorization and resource/backpressure bounds. | active |
