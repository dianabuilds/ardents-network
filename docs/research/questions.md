...
| R-005 | Which routing families can satisfy the Shielded Route metadata contract at an affordable cover budget? | Asynchronous carrier architecture | Anonymity simulation, latency/bandwidth budgets, active attack analysis, operator diversity assumptions | open |
| R-006 | How do Recovery Root, Devices, Personas, relationships, forward secrecy, revocation, and multi-device state coexist? | Identity and messaging state model | State-machine scenarios, compromise/recovery analysis, existing protocol comparison, usability testing | open |
| R-007 | What retention, replication, polling, deletion, and recovery semantics make Mailboxes and Site Bundles available without stable metadata beacons? | Replica/storage contract | Failure simulation, privacy analysis, capacity model, deletion and seizure behavior | open |
...
| ID | Question | Decision unlocked | Required evidence | State |
|---|---|---|---|---|
| R-013 | Which existing cryptographic protocols and maintained implementations cover each accepted identity, messaging, naming, and routing primitive? | Build/adopt map | Security reviews, API fit, maintenance history, license, interoperability and misuse analysis | open |
...
| R-017 | Is Named Private Site + Anonymous Mailbox the smallest slice that demonstrates differentiated value? | Architecture-tracer scope; later V1 candidate | Structured Product Owner walkthrough, incumbent comparison, traced failure/recovery; later external comprehension | review |
