...
| [R-161](records/r-161-issuer-result-retry.md) / [#93](https://github.com/dianabuilds/ardents-network/issues/93) | What finite already-paid retry budget retrieves one durably reserved issuer RESULT after loss, including when its request used the holder's last usable token? | **Selected C0 question, 2026-09-23.** Trace real stock/refill → prepare/present → issuer durable reservation → lost RESULT → exact retry. Falsify any scheme that needs a new free token after the final debit, spends a second token for the same result, extends the original authority/parent deadline, or makes another request claim the retained result. #95 owns Job-cancel pending policy; no runtime or authority redesign is selected. |
