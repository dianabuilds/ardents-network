...
| [R-002](records/r-002-live-application-interface.md) | What is the smallest live Application Interface that lets an existing local application publish and consume a Service safely? | **Active:** P1-D1 accepts a local socket/proxy-style boundary; SDKs are optional convenience wrappers with no separate network behavior. The remaining contract must define the data primitive, control surface, destination inputs, target authentication, failures, resource bounds, and Isolation Context. | active |
