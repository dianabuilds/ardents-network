...
| [R-023](records/r-023-interactive-route-performance-budget.md) | What end-to-end performance budget makes the V1 Interactive Route and Named Unlisted Site useful without weakening the accepted security contract? | **Active:** P3-D1 and P3-D2 fix platforms and latency. P3-D3 fixes single-connection goodput plus client `64/16` and publisher `256/64` minimum concurrency. P3-D3b3 requires every selected infrastructure role to be useful on a Linux `2 vCPU`, `2 GiB RAM`, symmetric `100 Mbit/s` reference VPS; this is not a ceiling or trust grant. Scale-up, resources, fairness, degradation, hostile load, and the measurement gate remain open; role-specific Node capacity follows R-004 evidence. | active |
