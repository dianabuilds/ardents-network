...
| [R-079](records/r-079-entry-binding-v1.md) | How does a User authenticate one User-to-Initiator Route leg without a User State identity or an unverified Entry capability? | **Accepted:** an R-077 Invite plus fresh mTLS attempt-key digest forms one atomically consumed EntryBinding; User is neither a synthetic State Node nor a durable identity. ADR-0027 corrects unannounced R-078 kind assignments. | decided |
| [R-080](records/r-080-stage-5-historical-provenance.md) | What Stage 5 material remains after native Entry supersedes Bridge and WebTunnel? | **Accepted:** retain Stage-5 records and independent evidence readers as C4 provenance, but delete Bridge/WebTunnel runtime and its active live runners. Historical results are not native-profile Qualification evidence. | decided |
| [R-081](records/r-081-native-route-resource-boundary.md) | What resource admission profile may the unannounced native Route use before Node integration? | **Accepted:** native M8 uses an explicit resource-admission port for behavior; it does not inherit H3 limits or announce a peer runtime. M11 must select and measure Node admission. | decided |
| [R-082](records/r-082-m9-compatibility-disposition.md) | Which M9 Service Connection and local publication bytes require compatibility support after the DA-10 inventory? | **Accepted:** no repository-visible external observer requires H3 `AS*`, H3 domain tags/exporter labels, local publication text, or the `serviceconn` action/result unions. M9 performs a coordinated C0 break for those bytes while preserving semantic authority separation and one classified terminal outcome. Commands remain M13 decisions. | decided |
| [R-083](records/r-083-native-service-connection-framing.md) | What exact native endpoint-to-endpoint connection framing preserves authenticated Target/Instance binding and bounded attachment recovery after H3 Service Connection bytes are retired? | **Accepted:** closed `ardents-service-connection-v1` endpoint records after exact-Instance-pinned TLS bind immutable Connection Context, attachment continuity, data/ack/terminal semantics, and strict retirement. Publication and local IPC remain separate M9/M10 decisions. ADR-0028 records the format. | decided |
| [R-084](records/r-084-publication-generation-owner.md) | What durable publication/generation owner gives M9 one crash-atomic Instance hand-off without retaining the H3 Endpoint action union? | **Accepted:** `service/publication` owns a C1 immutable-generation/current-pointer root; it migrates only the old monotonic floor, never restores Instance private material, and drains/erases it on withdrawal. No Custody or generic storage selection is implied. | decided |
| [R-085](records/r-085-m10-generic-broker-scope.md) | What M10 Application operating profile can transfer local admission without asserting qualified Isolation? | **Accepted:** M10 may build an explicit generic/unqualified Broker and isolation observation contract. It does not select a sandbox, platform-supported host, or Application Location Privacy claim; a qualified Linux/Windows profile remains a separate decision and ADR gate. | decided |
| [R-086](records/r-086-custody-authority-revocation.md) | How can Authority Custody fail closed when R-044 threshold recovery replaces an active Name Authority, without adding plaintext Name state, a foreign authority, or a generic current-state claim to the Vault? | **Open:** decide whether a bounded opaque replacement proof and D08 migration can safely demote the local Vault record; R-044 already revokes the effective Namespace authority, while Broker Grant revocation is separate. | open |
