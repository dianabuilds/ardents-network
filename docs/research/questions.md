...
| [R-023](records/r-023-interactive-route-performance-budget.md) | What end-to-end performance budget makes the V1 Interactive Route and Named Unlisted Site useful without weakening the accepted security contract? | **Active:** P3-D1 through P3-D4 fix platforms, normal performance, endpoint capacity, resources, fairness, queues, overhead, impaired paths, and recovery. P3-D5a protects established work during incomplete-attempt flood, and P3-D5b admits honest anonymous clients while `16` slots remain with `>= 95%` success, connection `p95 <= 8 s`, explicit result by `15 s`, and bounded identity-free client cost. P3-D5c fills all `256` slots with `128` honest and `128` valid admitted hostile connections: unread hostile streams reach bounded backpressure while the `64` active honest streams, `64` canaries, queue caps, and `1 GiB`/one-core publisher limits remain intact. Full capacity permits an explicit capacity result by `15 s`; per-person or Sybil-resistant new-admission fairness is not claimed. The P3-D6 measurement gate remains open, while role-specific Node capacity and cost follow R-004 evidence. | active |
