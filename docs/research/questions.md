...
|---|---|---|
| R-157 / [#78](https://github.com/dianabuilds/ardents-network/issues/78) | What receive-credit and shared DATA/control reservation makes every admitted Source/forwarding lane promise payable under the existing prefix, nested-parent and receiving-duty ceilings without silently reducing the selected workload? | **Prepared; awaiting Product Owner capacity choice.** [Draft research PR #256](https://github.com/dianabuilds/ardents-network/pull/256) found the existing 64 KiB-per-lane initial credit is not fully backed by the shared queue at maximum admitted fan-out. No #79 runtime change or capacity maximum is selected by this row. |
| [R-159](records/r-159-endpointset-profile.md) / [#218](https://github.com/dianabuilds/ardents-network/issues/218) | Which one signed EndpointSet record format and incompatible Node profile binds each advertised endpoint to the same current Node identity without importing producer or destination policy? | **Prepared; awaiting Product Owner profile direction.** [Draft PR #260](https://github.com/dianabuilds/ardents-network/pull/260) compares current State/Route owners with signed schema-3 EndpointSet, includes positive/negative Ed25519 record vectors and exact old/new refusal. The separate successor State profile and its ALPN/HELLO/ClosedProfile mapping are not accepted; no runtime, producer workflow, destination allowlist or public authority is selected. |
