...
| [R-026](records/r-026-private-resolution-adapter.md) | Can an existing RFC 9458 OHTTP implementation support the bounded two-role Gate C Private Resolution Adapter without joining endpoint origin, exact Name/Target lookup, or Gateway identity? | **Active Gate C dependency gate:** evaluate exact `openpcc/ohttp` and `chris-wood/ohttp-go` revisions outside the root module. Select only a license- and vulnerability-reviewed, offline-buildable implementation that preserves the Relay/Gateway knowledge split and lets the fixture detect modification, staleness, replay, nonce, and binding failures. If neither candidate passes, Gate C terminates with `stop`; no local-table, direct, DNS, HTTP, alternate-namespace, cached-success, or first-party OHTTP fallback is permitted. | active |
