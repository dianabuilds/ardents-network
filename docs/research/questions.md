...
|---|---|---|
| R-157 / [#78](https://github.com/dianabuilds/ardents-network/issues/78) | What receive-credit and shared DATA/control reservation makes every admitted Source/forwarding lane promise payable under the existing prefix, nested-parent and receiving-duty ceilings without silently reducing the selected workload? | **Prepared; awaiting Product Owner admission choice in draft PR #256.** Hypothesis tested: the current 64 KiB initial credit for every admitted lane is fully backed by current shared queue capacity. Compare initial, simultaneous lawful burst, delivered-but-credit-pending, cancel/EOF/close and nested parents in both directions; an admitted legal frame refused solely for aggregate queue exhaustion falsifies the hypothesis. Choose bounded internal reservation or an explicit paired wire change in #78 before #79; no runtime change is authorized by this row. |
| R-162 / [#90](https://github.com/dianabuilds/ardents-network/issues/90) | Which prefix-retirement outcomes require only the affected Context to stop, and which require the Endpoint text generation to stop? | **Prepared in draft PR #265; awaiting Product Owner stop-scope choice.** Falsify the hypothesis that a completed, proven local join permits sibling Contexts to remain live: trace the reservation owner and join evidence for network terminal failure, joined cleanup failure, unjoined work, and unknown refund through `retireTextPrefixLocked` and `failTextContexts`. Shared authority loss remains Endpoint-wide. Produce one outcome-to-stop-boundary table and positive/negative oracles for #91; no runtime change or general error redesign is authorized. |
