...
| [R-023](records/r-023-interactive-route-performance-budget.md) | What end-to-end performance budget makes the V1 Interactive Route and Named Unlisted Site useful without weakening the accepted security contract? | **Active:** P3-D1 through P3-D4 fix platforms, normal performance, endpoint capacity, resources, fairness, queues, overhead, impaired paths, and recovery. P3-D5a protects the full `256/64` established publisher workload during the accepted incomplete-attempt flood. P3-D5b starts with `240` established connections and `16` free slots during the same flood: at least `95%` of `600` honest anonymous attempts connect to the authenticated exact target and pass a canary, connection `p95 <= 8 s`, and every attempt resolves by `15 s`, while P3-D5a work and resource floors remain. Any required client admission check is capped at one CPU-second, `64 MiB`, and `1 MiB`, without money, account, IP reputation, or stable identity. Established hostile work remains P3-D5c; the measurement gate remains open, while role-specific Node capacity and cost follow R-004 evidence. | active |
...
| R-010 | Which local, anonymous, and bounded costs protect connection, discovery, rendezvous, and naming capacity from flooding and Sybil capture without a global User identity? | Resource-by-resource attack economics and admission controls, including accessibility and unlinkability costs. Publisher connection admission must meet the accepted P3-D5a established-work and P3-D5b honest-admission gates. Any network-mandated honest-client check adds at most one logical-core CPU-second, `64 MiB` peak memory, and `1 MiB` traffic, and cannot require money, an account, IP or source reputation, a stable network-generated identity, or cross-context linking. The concrete mechanism remains open. | open |
