...
| [R-003](records/r-003-service-name-contract.md) | How does an exact Service Name bind to a Service Target, resolve without becoming a directory, survive accepted rotation, and handle registration, expiry, recovery, conflict, enumeration, query privacy, and Control Plane capture? | **Active:** P4-D1 fixes a distinct Name Authority controlling the Name Record independently of Service Authority. It is unnecessary for ordinary publication or resolution and supplies the truthful rebinding path after target loss or compromise. Namespace allocation, record and authority lifecycles, resolution privacy, and governance remain open; no registry technology is selected. | active |
