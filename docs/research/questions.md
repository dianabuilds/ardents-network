...
| [R-023](records/r-023-interactive-route-performance-budget.md) | What end-to-end performance budget makes the V1 Interactive Route and Named Unlisted Site useful without weakening the accepted security contract? | **Active:** P3-D1 through P3-D3 fix platforms, latency, goodput, endpoint capacity, resources, fairness, queues, overhead, and scale-up. P3-D4a through P3-D4b2b fix same-connection recovery after one, sequential, and overlapping failures plus useful impaired-live progress. P3-D4b2c1 caps each endpoint in every 10-minute impaired/recovery run at `512 MiB` `p95` RSS, `50%` mean CPU and `100%` `p95` one-second CPU of one logical core, retains the `256 KiB` directional connection queue cap, and forbids accumulated recovery state. Degraded/recovery carrier cost, hostile load, and the measurement gate remain open; role-specific Node capacity and cost follow R-004 evidence. | active |
