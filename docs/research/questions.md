...
| [R-023](records/r-023-interactive-route-performance-budget.md) | What end-to-end performance budget makes the V1 Interactive Route and Named Unlisted Site useful without weakening the accepted security contract? | **Active:** P3-D1 through P3-D5 fix platforms, useful performance, finite resources, recovery, incomplete-attempt admission, and established hostile-work isolation, including the explicit full-capacity Sybil non-claim. P3-D6a makes qualification conjunctive across every mandatory platform, endpoint side, direction, and scenario: no cross-cell averaging, all applicable metrics pass together, a valid security or privacy guardrail violation hard-fails, and failures or timeouts remain evidence. Only a confirmed retained harness invalidation permits replacement; a failed candidate may remain research but cannot claim V1 Route Qualification. P3-D6b/c still define the reference matrix, sampling, artifacts, and regression rules, while role-specific Node capacity and cost follow R-004 evidence. | active |
