...
| R-164 / [#80](https://github.com/dianabuilds/ardents-network/issues/80) | What finite current/previous publication-registration transition applies when one parent expires, including pending ACK, old readers and no replacement parent? | **Selected C0 question, 2026-09-23.** Falsify the hypothesis that an ACK under the next parent can preserve every already admitted old reader without extending its original deadline or unboundedly retaining resources. Trace pair, refresh, registration and prefix owners before proposing exact boundary outcomes for #81; no runtime change is authorized. |
