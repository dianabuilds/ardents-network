...
| [R-004](records/r-004-routing-rendezvous-families.md) | Which routing and rendezvous families can meet the accepted Interactive Route claim and R-023 performance budget under churn, malicious Nodes, and realistic client devices? | **Active:** P5-D1 fixes the Route Module Seam and P5-D2 fixes the Introduction boundary: an operator may independently know a public Target, but the role receives neither endpoint origin and any Target-to-origin link fails qualification. Split-leg refinement rejects a combined Service Entry/Introduction role and a stored invitation mailbox, advances Rendezvous-forwarded sealed introduction as the lowest-cost hypothesis, and retains an independent introduction route as the security fallback. No route, library, cryptography, hop count, or language is selected. | active |
