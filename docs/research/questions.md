...
| [R-062](records/r-062-resource-platform-scope.md) | Which supported-platform resource adapters can truthfully enforce and measure the retained H3 profiles, and how must every unsupported platform fail closed? | Compare the current Linux cgroup-v2/rlimit adapter with a deliberately unsupported result and any later native Adapter; decide the current support matrix before M4 changes resource behavior. | open |
