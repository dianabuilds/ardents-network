...
| [R-004](records/r-004-routing-rendezvous-families.md) | Which routing and rendezvous families can meet the accepted Interactive Route claim and R-023 performance budget under churn, malicious Nodes, and realistic client devices? | **Active:** P5-D1 fixes the Route Module Seam and P5-D2 fixes the Introduction boundary. Position review finds that the original three-position split route fails the accepted no-full-Route rule because Rendezvous sees every carrier Node; four positions are asymmetric. A symmetric five-position route is the smallest recommended candidate and converges to the Tor-shaped data-path reference, so no custom forwarding advantage is assumed. Compare Tor-style separate introduction with C1 Rendezvous-forwarded introduction and retain tunnel pools as the structurally different alternative. C-3 remains only an unqualified performance control. No route, library, cryptography, hop count, or language is selected. | active |
