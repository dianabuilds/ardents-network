...
| R-165 / [#237](https://github.com/dianabuilds/ardents-network/issues/237) | Where should current TCP/TLS and QUIC Carrier authentication/lifetime adapters live relative to Route lane grammar and Node ownership? | **Prepared; awaiting Product Owner architecture choice in draft PR #271.** [R-165](records/r-165-carrier-adapter-seam.md) proposes retaining current physical Carrier adapters as an in-package route module with Node and route/credential importing route. A separate package would move protocol/profile and TLS/admission types or add aliases without a current new consumer. #250 old-listener deletion remains separate; no runtime change is authorized. |
