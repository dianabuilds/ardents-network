...
| [R-004](records/r-004-routing-rendezvous-families.md) | Which routing and rendezvous families can meet the accepted Interactive Route claim and R-023 performance budget under churn, malicious Nodes, and realistic client devices? | **Active:** P5-D1 fixes the Route Module Seam, P5-D2 fixes the Introduction boundary, and P5-D3 fixes the symmetric five-position baseline. C-3 remains only an unqualified performance control and C-4 is rejected. The shape converges to the Tor data-path reference, so no custom forwarding advantage is assumed. Compare Tor-style separate introduction with C1 Rendezvous-forwarded introduction and retain tunnel pools as the structurally different alternative. No routing family, introduction mechanism, library, cryptography, wire protocol, or language is selected. | active |
