...
| [R-023](records/r-023-interactive-route-performance-budget.md) | What end-to-end performance budget makes the V1 Interactive Route and Named Unlisted Site useful without weakening the accepted security contract? | **Active:** P3-D1 and P3-D2 fix platforms and latency. P3-D3 fixes goodput, client `64/16` and publisher `256/64` floors, and a Linux `2 vCPU` / `2 GiB` / symmetric `100 Mbit/s` infrastructure class. P3-D3c1 allows bounded local scale-up; P3-D3c2a-b set idle client resources and a secondary traffic guardrail. P3-D3c2c1-2 cap active client and publisher process trees: respectively 16 connections / `10 Mbit/s` / `512 MiB` / `50%` core and 64 connections / `40 Mbit/s` / `1 GiB` / one core over 10 minutes. Active overhead, fairness, combined workloads, degradation, hostile load, and the measurement gate remain open; role-specific Node capacity follows R-004 evidence. | active |
