...
|---|---|---|
| R-157 / [#78](https://github.com/dianabuilds/ardents-network/issues/78) | What receive-credit and shared DATA/control reservation makes every admitted Source/forwarding lane promise payable under the existing prefix, nested-parent and receiving-duty ceilings without silently reducing the selected workload? | **Prepared; awaiting Product Owner choice of retained capacity maxima.** Hypothesis to falsify: the current 64 KiB initial credit for every admitted lane is fully backed by current shared queue capacity. Compare initial, simultaneous lawful burst, delivered-but-credit-pending, cancel/EOF/close and nested parents in both directions; an admitted legal frame refused solely for aggregate queue exhaustion falsifies the hypothesis. Choose bounded internal reservation or an explicit paired wire change in #78 before #79; no runtime change is authorized by this row. |
| R-158 / [#214](https://github.com/dianabuilds/ardents-network/issues/214) | Which bounded canonical Service Connection frame encoding and incompatible profile can replace current v2 records without weakening pre-effect authentication or silently reusing retained identities? | **Prepared; awaiting Product Owner wire choice, 2026-09-23.** [R-158](records/r-158-connection-wire-encoding.md) compares the published CBOR/ALPN candidate with v2 bytes, exact callers, parser limits, old/new refusal and retained-state boundaries. Its separate-profile recommendation and syntax vectors are evidence, not an accepted grammar or runtime authorization. #214 remains open pending the chosen technical-owner/ADR decision. |
