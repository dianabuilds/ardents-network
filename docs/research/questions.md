...
| [R-083](records/r-083-native-service-connection-framing.md) | What exact native endpoint-to-endpoint connection framing preserves authenticated Target/Instance binding and bounded attachment recovery after H3 Service Connection bytes are retired? | Decide one closed authenticated endpoint record grammar, attachment-continuity binding, data/ack/terminal semantics, and conformance corpus without exposing Service material to a Node or retaining a legacy reader. Publication representation and local Application IPC are separate M9/M10 decisions. | open |
