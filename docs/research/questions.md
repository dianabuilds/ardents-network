...
| [R-023](records/r-023-interactive-route-performance-budget.md) | What end-to-end performance budget makes the V1 Interactive Route and Named Unlisted Site useful without weakening the accepted security contract? | **Active:** P3-D1 and P3-D2 fix platforms and latency. P3-D3 fixes goodput, client `64/16` and publisher `256/64` floors, reference resources, fair progress, carrier overhead, combined load, queues, and qualified scale-up with `20%` resource headroom. P3-D4a makes seamless continuation of the same logical Service Connection mandatory after one eligible ordinary-Node or Carrier Channel failure: ordered delivery resumes within `p95 <= 5 s`, or the connection terminates explicitly by `15 s`. Loss, jitter, repeated churn, hostile load, and the measurement gate remain open; role-specific Node capacity follows R-004 evidence. | active |
