...
| R-164 / [#80](https://github.com/dianabuilds/ardents-network/issues/80) | What finite current/previous publication-registration transition applies when one parent expires, including pending ACK, old readers and no replacement parent? | **Prepared; awaiting Product Owner decision in draft PR #270.** [R-164](records/r-164-publication-parent-expiry.md) proposes explicit unavailability at original parent/registration expiry: current refresh has one Introduction parent and rotates only Registration under it. Seamless second-parent overlap would need a separate admission/cleanup decision. #81 remains conditional; no runtime change is authorized. |
