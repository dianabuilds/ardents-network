...
| R-166 / [#234](https://github.com/dianabuilds/ardents-network/issues/234) | Does the uncomposed Namespace OHTTP resolution module have a current consumer or compatibility obligation after the Name network-command refusal? | **Selected C0 question, 2026-09-23.** Falsify the hypothesis that its only external caller is the retirement test fixture, while canonical Names, Namespace/custody roots and current Target reachability have independent owners. Trace production/test imports, the zero-effect fixture and actual OHTTP dependency users before one keep/remove decision; no runtime change is authorized. |
