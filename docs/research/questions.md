...
| [R-023](records/r-023-interactive-route-performance-budget.md) | What end-to-end performance budget makes the V1 Interactive Route and Named Unlisted Site useful without weakening the accepted security contract? | **Active:** P3-D1 fixes platform classes and P3-D2 fixes latency. P3-D3a requires single-connection `p05` goodput of at least `min(10 Mbit/s, 50% of direct baseline)` in each direction. P3-D3b1 revises the client minimum to `64` open / `16` active outbound connections; P3-D3b2 separately requires a publisher minimum of `256` open / `64` active incoming connections. Both are floors, not maxima. Node capacity, resources, fairness, degradation, hostile load, and the measurement gate remain open. | active |
