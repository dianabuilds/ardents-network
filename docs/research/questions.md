...
| [R-004](records/r-004-routing-rendezvous-families.md) | Which routing and rendezvous families can meet the accepted Interactive Route claim and R-023 performance budget under churn, malicious Nodes, and realistic client devices? | **Active:** primary-source comparison retains full Tor-shaped circuits as the security reference, I2P-shaped unidirectional tunnel pools as the principal alternative, and advances a three-position Ardents split-leg rendezvous to an information-flow model and bounded prototype. A delayed Nym-shaped mixnet is deferred to R-005. No route, library, cryptography, hop count, or language is selected. | active |
