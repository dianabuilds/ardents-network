...
| [R-036](records/r-036-h3-camouflage-adapter.md) | Which replaceable Camouflage Adapter interface and pinned maintained implementation can carry the same H3 Bridge workload without selecting a production transport by inertia? | **Active comparison:** reproducible offline builds and input-bound manifests passed; both candidates carried the same exact `512-byte` nonce-bearing HTTP request and `64 KiB` response, and the live-work snapshot observed zero capabilities and descendants. WebTunnel's supply is clean and materially smaller; Lyrebird's linked binary has six reachable advisories, but their metadata does not establish the frozen high/critical rejection threshold. Complete DNS/resource observation, forced shutdown rungs, and post-cleanup residual verification remain before selection. No candidate, fallback, root dependency, or maintained Stage 5 code is authorized. | active |
