...
| ID | Exact question | Decision and required result | State |
|---|---|---|---|
| [R-013](records/r-013-carrier-lab-technology-candidates.md) | Which maintained protocols and implementations fit each accepted addressing, Carrier Channel, routing, discovery, naming, and bootstrap contract? | **Candidate implemented for development smoke:** native C-5/C2 over TCP with separate Introduction, telescoped and end-to-end TLS 1.3, HPKE invitation, real shaping/capture, image receipt, role evidence, process-failure handling, and cleanup are implemented. The frozen comparative workload, official Ubuntu run, Direct/C-3 measurements, full timed process-failure condition, and Tor/Chutney reference remain required before a Route verdict. | active |
