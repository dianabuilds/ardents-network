...
| [R-001](records/r-001-interactive-route-claim.md) | Which endpoint, local-observer, relay-collusion, and Broad Traffic Observer capabilities must the Interactive Route resist, and what does it deliberately expose? | **Active:** P2-D1 accepts endpoint-location and single-intermediary protection but no Broad Traffic Observer correlation-resistance claim. Local-observer visibility, intermediary knowledge, collusion, malicious endpoints, active attacks, conditions, and falsification remain to be fixed. | active |
