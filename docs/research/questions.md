...
| [R-023](records/r-023-interactive-route-performance-budget.md) | What end-to-end performance budget makes the V1 Interactive Route and Named Unlisted Site useful without weakening the accepted security contract? | **Active:** P3-D1 through P3-D3 fix platforms, normal performance, endpoint capacity, resources, fairness, queues, overhead, and scale-up. P3-D4 fixes same-connection operation through impaired paths and single, sequential, and overlapping failures. Each impaired/recovery endpoint is capped at `512 MiB` `p95` RSS, `50%` mean CPU, `100%` `p95` one-second CPU, and existing queues; impaired-live carrier ratio is `<= 2.0`, recovery excess is `<= 8 MiB` per endpoint and episode, and directional `p95` one-second carrier bitrate is `<= min(25 Mbit/s, 80% usable link budget)`. Hostile load and the measurement gate remain open; role-specific Node capacity and cost follow R-004 evidence. | active |
