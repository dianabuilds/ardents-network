...
| R-046 | Which exact field-level role matrix freezes resolution and naming control-operation inputs, observations, stable identifiers, Role Domains, known-family exclusions, and Isolation Context boundaries so no one ordinary role receives both endpoint location and exact name/lookup view? | **Open:** five role names are candidate vocabulary, not a field matrix. The next record must list every request, response, log, error, cache, retry, and evidence field per operation and pass forbidden-field mutation tests. See [r-046-role-matrix.md](records/r-046-role-matrix.md). | open |
