...
Status: **accepted cross-horizon decision registry; all Stage 4 development
authorized; Stage 5 research active under the accepted exact scope**
...
| [R-037](records/r-037-h3-blocked-entry-evidence.md) | Which controlled address-blocking, protocol-allow-list, active-probe, Invite-replay, withholding, role-conflict, restart, resource, and cleanup cells determine Stage 5 `pass|fail|invalid`? | **Decided:** the Product Owner accepted option O1 and `h3-s5-b1-v1` on 2026-08-16. The profile freezes the Ubuntu topology, C0-C6, a non-resetting `64 s` attempt, `15 s` contacts, exact sample floors, reference/stronger capacity, sustained/resource/pressure gates, complete helper accounting, secret/public evidence separation, and an independent verdict. The Stage 5 implementation brief is prepared for separate acceptance; maintained code remains unauthorized until then. | decided |
| [R-039](records/r-039-h3-private-naming-lifecycle.md) | What exact bounded production slice for Service Name lifecycle is required to prepare Stage 6 and what evidence/verifier split is mandatory before code coupling? | **Accepted scope, coding gate closed:** encoding, lifecycle, delegation, authority/recovery, Target continuity, private resolution and control-operation separation, conflicts/forks, abuse bounds, and independent `pass|fail|invalid` verdicting are required. Stage 5 advance, corrected-document acceptance, and the S6.0 freeze for limits, ordering, persistence, cryptography, Anonymous Cost, and role fields remain mandatory before implementation. | accepted |
