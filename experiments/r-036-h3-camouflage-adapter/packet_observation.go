...
func isObserverControl(packet []byte, allowTCP bool) bool {
	offset, protocol, ambiguous, ok := packetTransport(packet)
	...
	return bytes.Equal(packet[offset+8:offset+length], observerControlPayload)
}

func isAmbiguousPacket(packet []byte) bool {
	_, _, ambiguous, ok := packetTransport(packet)
	return ok && ambiguous
}
...
func packetTransport(packet []byte) (int, byte, bool, bool) {
	...
	if protocol == 0x8100 && len(packet) >= 18 {
		protocol = binary.BigEndian.Uint16(packet[16:18])
		offset = 18
	}
	switch protocol {
	case 0x0800:
