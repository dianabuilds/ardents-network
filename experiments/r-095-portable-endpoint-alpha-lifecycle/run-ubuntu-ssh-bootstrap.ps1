...
$artifact = Join-Path $temporaryRoot 'ardents-r095-bootstrap-linux'
$source = Join-Path $PSScriptRoot 'lifecycle_main.go'
$runner = Join-Path $PSScriptRoot 'run-ubuntu-ssh-bootstrap.sh'
...
try {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw 'bootstrap fixture build failed'
        }
    } finally {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Ubuntu SSH bootstrap runner exited $LASTEXITCODE"
    }
} finally {
