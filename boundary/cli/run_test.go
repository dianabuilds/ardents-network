...
func fetchRuntimeIdentity(t *testing.T, baseURL, token string) *ardentsv1.IdentitySnapshot {
	...
	service := ardentsv1connect.NewArdentsServiceClient(httpClient, baseURL)
...
func newCLIServer(t *testing.T, token string) *httptest.Server {
	...
	path, handler, err := rpcadapter.NewHandler(testkit.ConnectDependencies(n), rpcadapter.AuthConfig{
