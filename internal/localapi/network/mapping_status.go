...
func toDiscoveryStatusSnapshot(in discovery.StatusSnapshot) *ardentsv1.DiscoveryStatusSnapshot {
	return &ardentsv1.DiscoveryStatusSnapshot{
		...
		LastPublishAt:   rpc.Timestamp(in.LastPublishAt),
		LastRefreshAt:   rpc.Timestamp(in.LastRefreshAt),
	}
}

func toLocalPresenceSnapshot(in publication.LocalPresenceSnapshot) *ardentsv1.LocalPresenceSnapshot {
	return &ardentsv1.LocalPresenceSnapshot{
		Published:              in.Published,
		State:                  in.State,
		Reason:                 in.Reason,
		RecordId:               in.RecordID,
		PublishedAt:            rpc.Timestamp(in.PublishedAt),
		ExpiresAt:              rpc.Timestamp(in.ExpiresAt),
		OperatorActionRequired: in.OperatorActionRequired,
	}
}

func toRouteCandidateSnapshots(items []discovery.RouteCandidateSnapshot) []*ardentsv1.RouteCandidateSnapshot {
