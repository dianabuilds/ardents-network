// Package localapi owns authenticated local-control server composition.
// It does not own product decisions or area handler implementation.
package localapi
...
func newAccessInterceptor(auth localauth.Config, audit diagapi.EventWriter) *accessInterceptor {
...
func (i *accessInterceptor) authorize(procedure string, header http.Header) error {
	rule, ok := localauth.RuleForProcedure(procedure)
	...
	if !decision.Allowed {
		...
		return rpc.ToConnectError(accessRPCError(rule, decision))
	}
	return nil
}
...
func accessRPCError(rule localauth.Rule, decision identityapi.Decision) *rpc.Error {
	return &rpc.Error{
		Code: decision.Code, Category: rpc.ErrorCategory(decision.Code), Message: decision.Message,
...
func (i *accessInterceptor) record(rule localauth.Rule, outcome, reason, subject string) {
	if i.audit == nil {
		return
	}
