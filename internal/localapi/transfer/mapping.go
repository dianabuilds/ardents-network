...
func blobSnapshot(in domain.Blob) *ardentsv1.BlobSnapshot {
	return &ardentsv1.BlobSnapshot{Reference: in.Reference.String(), MediaType: in.MediaType, Size: in.Size, Hash: in.Hash,
		CreatedAt: rpc.Timestamp(in.CreatedAt), Cipher: in.Cipher, KeyId: in.KeyID,
...
func sourceSnapshots(items []domain.BlobSourceRecord) []*ardentsv1.BlobSourceSnapshot {
	...
	for _, item := range items {
		out = append(out, &ardentsv1.BlobSourceSnapshot{ContentReference: item.ContentReference.String(), NodeId: item.NodeID, ServiceId: item.ServiceID,
