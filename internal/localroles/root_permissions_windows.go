...
func validateOwnerOnlyRoot(root string, _ os.FileInfo) error {
	...
	if err != nil {
		return errors.New("read local role owner SID")
	}
	if err := setOwnerOnlyDACL(root, user.User.Sid, windows.SUB_CONTAINERS_AND_OBJECTS_INHERIT); err != nil {
		return errors.New("local role root owner-only DACL could not be enforced")
	}
	entries, err := os.ReadDir(root)
	if err != nil {
		return errors.New("read local role root for DACL enforcement")
	}
	for _, entry := range entries {
		if entry.IsDir() || setOwnerOnlyDACL(filepath.Join(root, entry.Name()), user.User.Sid, windows.NO_INHERITANCE) != nil {
			return errors.New("local role file owner-only DACL could not be enforced")
		}
	}
	return nil
}
