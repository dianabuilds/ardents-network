...
func assertQualityWiring(t *testing.T, root string) {
	...
	for _, required := range []string{"contents: read", "go-version: " + goVersion, "cache: true", "go run ./scripts/select-pr-checks.go ./scripts/select-pr-check-registry.go --base", "fail-fast: false", "-timeout=180s", "ARDENTS_E2E_COMMAND_ROOT", "./cmd/ardents-custody", "-buildvcs=false", "fetch-depth: 0", "qualification-scripts:", "Management.Automation.Language.Parser", "needs.select.outputs.powershell == 'true'", "workflow_dispatch:", "push:", "branches: [main]", "run: make check"} {
