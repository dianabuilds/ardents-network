...
func signingDigest(header envelopeHeader, message *wire.PrivateMessageV1) ([]byte, error) {
	unsigned := proto.Clone(message).(*wire.PrivateMessageV1)
...
func marshalPadded(message *wire.PrivateMessageV1) ([]byte, error) {
	message.Padding = nil
...
func validateDecodedInner(raw []byte, message *wire.PrivateMessageV1) error {
	...
	withoutPadding := proto.Clone(message).(*wire.PrivateMessageV1)
