...
func Publish(ctx context.Context, node *wakuNode.WakuNode, envelope network.Envelope) error {
	if node == nil {
	...
	msg := &wpb.WakuMessage{
		...
		Timestamp:    new(time.Now().UnixNano()),
	}
	_, err := node.Relay().Publish(ctx, msg, relay.WithPubSubTopic(pubsubTopic))
	return err
}

func Subscribe(ctx context.Context, node *wakuNode.WakuNode, pubsubTopic string, contentTopics ...string) (<-chan network.Envelope, error) {
	if node == nil {
...
func relaySubscriptionLoop(ctx context.Context, pubsubTopic string, subs []*relay.Subscription, out chan<- network.Envelope) {
...
func consumeRelaySubscription(ctx context.Context, wg *sync.WaitGroup, pubsubTopic string, sub *relay.Subscription, out chan<- network.Envelope) {
	defer wg.Done()
	for {
		select {
		case <-ctx.Done():
...
func relayEnvelope(pubsubTopic string, env *protocol.Envelope) (network.Envelope, bool) {
