...
func FetchEnvelopes(ctx context.Context, node *wakuNode.WakuNode, endpoints []string, maximumFetchedEnvelopes int, pubsubTopic string, contentTopics ...string) ([]Envelope, error) {
	...
	seen := make(map[[32]byte]Envelope)
	var lastErr error
...
func storedEnvelope(pubsubTopic string, contentTopics []string, message *wpb.WakuMessage) (Envelope, bool) {
...
func envelopeValues(items map[[32]byte]Envelope) []Envelope {
