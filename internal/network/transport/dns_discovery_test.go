...
func TestDiscoveryConfigRejectsUnsignedOrAmbiguousInputs(t *testing.T) {
	require.Error(t, validateDiscoveryConfig(Config{DNSDiscoveryURLs: []string{"https://nodes.example.org"}}))
	require.Error(t, validateDiscoveryConfig(Config{DNSDiscoveryNameServer: "1.1.1.1"}))
	require.Error(t, validateDiscoveryConfig(Config{
...
func TestDNSRefreshReplacesPeersAndClearsStaleResultsOnFailure(t *testing.T) {
	...
	svc := New(Config{DNSDiscoveryURLs: []string{signedTestTree}})
...
func TestWakuDNSDiscoveryVerifiesSignedTreeAndReturnsTCPPeers(t *testing.T) {
	...
	peers, err := (wakuDNSPeerDiscovery{resolver: resolver}).Retrieve(
		context.Background(), []string{signedTestTree, url}, "", networkreadiness.ProfileTCPOnly,
	)
	require.NoError(t, err)
...
func TestDiscoveredAddressFilterPreservesConfiguredCarrierFamilies(t *testing.T) {
	...
	require.True(t, allowedDiscoveredAddress(tcp, networkreadiness.ProfileTCPOnly))
	require.False(t, allowedDiscoveredAddress(wss, networkreadiness.ProfileTCPOnly))
	require.True(t, allowedDiscoveredAddress(wss, networkreadiness.ProfileTCPWSS))
	require.False(t, allowedDiscoveredAddress(plaintextWS, networkreadiness.ProfileTCPWSS))
	require.False(t, allowedDiscoveredAddress(quic, networkreadiness.ProfileTCPWSS))
}
