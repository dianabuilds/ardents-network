...
func TestNetworkOperationRejectsOversizedMessageAndReportsIt(t *testing.T) {
	svc := New(Config{Limits: Limits{MaxMessageBytes: 1024}})
...
func TestNetworkOperationAppliesRateLimit(t *testing.T) {
	svc := New(Config{Limits: Limits{OperationRate: 1, OperationBurst: 1}})
	done, err := svc.acquireNetworkOperation(0, "")
	require.NoError(t, err)
	done(nil)
...
func TestProviderPenaltyTemporarilyBansAndRecovers(t *testing.T) {
	...
	svc := New(Config{Limits: Limits{OperationRate: 1000, OperationBurst: 1000}})
...
func TestValidateLimitsRejectsNegativeAndUnsafeValues(t *testing.T) {
	require.ErrorContains(t, validateLimits(Limits{OperationRate: -1}), "cannot be negative")
	require.ErrorContains(t, validateLimits(Limits{MaxMessageBytes: 200 * 1024}), "maximum message")
	require.ErrorContains(t, validateLimits(Limits{MaxPeerConnections: 4, MaxConnectionsPerIP: 5}), "connection limits")
}
