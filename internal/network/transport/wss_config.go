...
func ValidateConfig(cfg Config, now time.Time) error {
	...
	profile := networkreadiness.NormalizeProfile(cfg.Profile)
	if profile != networkreadiness.ProfileTCPWSS {
		if hasWSSConfig(cfg) {
...
func validateWSSMaterial(cfg Config, host string, now time.Time) error {
...
func hasWSSConfig(cfg Config) bool {
