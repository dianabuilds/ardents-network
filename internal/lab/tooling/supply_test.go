...
func TestFilesystemPathComparisonUsesPlatformSemantics(t *testing.T) {
	t.Parallel()
	left := filepath.Join("root", "lab", "carrier", "tools.lock")
	right := filepath.Join("ROOT", "LAB", "CARRIER", "TOOLS.LOCK")
...
func newToolBundleFixture(t *testing.T) toolBundleFixture {
	...
	carrierLabDirectory := filepath.Join(repositoryRoot, "lab", "carrier")
	if err := os.MkdirAll(carrierLabDirectory, 0o700); err != nil {
		t.Fatal(err)
	}
