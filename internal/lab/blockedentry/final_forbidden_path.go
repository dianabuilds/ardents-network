...
func validFinalForbiddenPathReceipt(raw []byte, variant string) bool {
	...
	wantSources := map[string]string{"dns": "host-alias", "environment-proxy": "proxy-environment",
	...
	wantComponents := map[string]string{"dns": "adapter-resolver", "environment-proxy": "adapter-process",
	...
	wantTerminal := "bridge-attempt-exhausted"
	if variant == "deadline-exposure-reset" {
		wantTerminal = "bridge-deadline-exceeded"
	}
	return value.Schema == "ardents-h3-g7-receipt-v2" &&
		value.Variant == variant && value.Source == wantSources[variant] && value.Component == wantComponents[variant] &&
		validFinalG7Component(value, variant, wantComponents[variant]) && value.Calls > 0 &&
		value.Calls <= 2 && value.ContactStarts == value.Calls && value.Terminal == wantTerminal &&
...
func finalG7EnvelopeTarget(raw []byte) string {
	...
	if nameLength != len("front.example") || len(raw) != offset+nameLength+32 ||
		string(raw[offset:offset+nameLength]) != "front.example" {
		return ""
	}
	if bytes.Equal(raw[len(raw)-32:], make([]byte, 32)) {
		return ""
	}
