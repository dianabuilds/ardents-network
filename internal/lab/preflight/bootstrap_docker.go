...
func (run *bootstrapRun) buildCandidateImage() error {
	sourceSHA256, err := sourceidentity.SourceSHA256(run.repositoryRoot)
	if err != nil {
	...
	return run.runCommand([]string{"DOCKER_BUILDKIT=1"}, "docker",
		...
		"--file", filepath.Join(run.repositoryRoot, "lab", "carrier", "Dockerfile"),
		"--target", "application",
...
func (run *bootstrapRun) runPinnedVerifier(expectedBinarySHA256 string) error {
	containerSession := containerSessionPath(run.sessionRoot)
	...
	arguments := []string{
		...
		containerSession, containerTemporaryRoot, run.runID, expectedBinarySHA256,
	}
...
func (run *bootstrapRun) runPinnedFinalizer(resources OwnedResources) error {
	...
	arguments := []string{
		...
		"--repository-root", "/repo", "--session-root", containerSession, "--temp-root", containerTemporaryRoot, "--run-id", run.runID,
...
const containerTemporaryRoot = "/carrier-lab"

func containerSessionPath(sessionRoot string) string {
	return containerTemporaryRoot + "/" + filepath.Base(sessionRoot)
}
