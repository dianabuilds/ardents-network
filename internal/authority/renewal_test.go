...
func TestAuthorityCreatesStrictlySeparatedChannelClasses(t *testing.T) {
	...
	member, principal, attestation := newMembershipMember(t, fixture, 0x91)
	...
	for index, channel := range fixture.store.state.Channels {
		...
		require.True(t, ok)
		ref, importErr := member.ImportGrant(grant)
	...
	for index, audit := range fixture.store.state.AuditLog[1:] {
		require.Equal(t, string(classes[index]), audit.ChannelClass)
		require.Equal(t, uint32(1), audit.Generation)
		require.Equal(
			t,
			InitialGenerationDeliveryResource(
				genesis.RealmID, "class-separation-"+string(rune('a'+index)),
			),
			audit.ResourceID,
		)
	}
	_, secondPrincipal, secondAttestation := newMembershipMember(t, fixture, 0x92)
	changed, err := fixture.service.ChangeChannelMembership(
		ctx, membershipCommand(genesis.RealmID, results[0].ChannelID),
		MembershipChangeRequest{
			Version: ContractVersion, RequestID: "class-separation-add-discovery",
			RealmID: genesis.RealmID, ChannelID: results[0].ChannelID,
			Change: MembershipChangeAdd, TargetPrincipal: secondPrincipal,
	...
	require.NoError(t, err)
	require.Equal(t, uint32(2), changed.PendingGeneration)
	require.Equal(t, identityapi.CapabilityRealmDiscovery, changed.ChannelClass)
	require.Equal(t, uint32(1), fixture.store.state.Channels[0].PendingGenerationCount)
	require.Len(t, fixture.store.state.Channels[0].PendingGrants, 2)
	for _, sibling := range fixture.store.state.Channels[1:] {
		...
		require.Len(t, sibling.CurrentGrants, 1)
		require.NotEqual(t, secondPrincipal, sibling.CurrentGrants[0].SubjectPrincipal)
	}
	membershipAudit := fixture.store.state.AuditLog[len(fixture.store.state.AuditLog)-1]
	require.Equal(t, string(identityapi.CapabilityRealmDiscovery), membershipAudit.ChannelClass)
	require.Equal(t, uint32(2), membershipAudit.Generation)
	require.Equal(t, secondPrincipal, membershipAudit.TargetPrincipal)
}
...
func TestExistingRealmMemberCanJoinSiblingChannelAtRealmCapacity(t *testing.T) {
	ctx := context.Background()
	fixture := newServiceFixture(t)
	fixture.service.random = rand.Reader
	genesis, err := fixture.service.CreateOrReopen(
		ctx, fixture.createCommand(),
		CreateRequest{
			Version: ContractVersion, RequestID: "sibling-capacity-genesis",
			RealmClass: RealmClassProduction,
		},
	)
	require.NoError(t, err)
	_, principalA, attestationA := newMembershipMember(t, fixture, 0xa1)
	_, _, attestationB := newMembershipMember(t, fixture, 0xa2)
	issue := func(requestID string, attestation identityapi.CapabilityDeliveryAttestation) InitialGenerationResult {
		result, issueErr := fixture.service.IssueInitialGeneration(
			ctx,
			Command{
				Actor: "operator", Effective: "operator", Action: ActionIssueDelivery,
	...
	issue("sibling-capacity-a", attestationA)
	sibling := issue("sibling-capacity-b", attestationB)
	for index := len(fixture.store.state.Members); index < MaxRealmMembers; index++ {
		fixture.store.state.Members = append(fixture.store.state.Members, MemberRecord{
			Version:   ContractVersion,
			Principal: "capacity-member-" + string(rune(0x1000+index)),
		})
	}
	require.Len(t, fixture.store.state.Members, MaxRealmMembers)

	changed, err := fixture.service.ChangeChannelMembership(
		ctx, membershipCommand(genesis.RealmID, sibling.ChannelID),
		MembershipChangeRequest{
			Version: ContractVersion, RequestID: "sibling-capacity-add",
			RealmID: genesis.RealmID, ChannelID: sibling.ChannelID,
			Change: MembershipChangeAdd, TargetPrincipal: principalA,
			RecipientAttestations: []identityapi.CapabilityDeliveryAttestation{
	...
	require.Equal(t, uint32(2), changed.PendingGeneration)
	require.Len(t, fixture.store.state.Members, MaxRealmMembers)
	require.Len(t, fixture.store.state.Channels[1].PendingGrants, 2)
}
...
func TestOrdinaryRotationRetainsLegacyIdempotencyHash(t *testing.T) {
	request := RotationRequest{
		Version: ContractVersion, RequestID: "legacy-hash",
		RealmID:   "r1_00112233445566778899aabbccddeeff",
		ChannelID: [16]byte{0x31},
		RecipientAttestations: []identityapi.CapabilityDeliveryAttestation{{
			Version: ContractVersion, SubjectPrincipal: "member",
		}},
		ValidFor: time.Hour, DrainFor: 15 * time.Minute,
	}
	legacyRaw, err := json.Marshal(struct {
		...
		int64(request.DrainFor / time.Second), request.MembershipChange,
		request.TargetPrincipal,
	})
	require.NoError(t, err)
	legacySum := sha256.Sum256(legacyRaw)
	require.Equal(t, hex.EncodeToString(legacySum[:]), rotationPayloadHash(request))
	request.Renewal = true
	require.NotEqual(t, hex.EncodeToString(legacySum[:]), rotationPayloadHash(request))
}
