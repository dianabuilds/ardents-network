...
func TestDropOwnerBindingPreservesSharedThenReclaimsUnreferencedPayload(t *testing.T) {
	...
	_, err = service.DropBlobForOwner(alice, aliceBlob.ID)
	require.NoError(t, err)
	require.False(t, service.HasBlobOwner(alice, aliceBlob.ID))
	require.True(t, service.HasBlobOwner(bob, aliceBlob.ID))
	_, err = service.GetBlobPayloadForOwner(bob, aliceBlob.ID)
	require.NoError(t, err)

	dropped, err := service.DropBlobForOwner(bob, aliceBlob.ID)
	require.NoError(t, err)
	require.Equal(t, "deleted", dropped.State)
	_, statErr := os.Stat(service.payloadPath(aliceBlob.ID))
	require.ErrorIs(t, statErr, os.ErrNotExist)

	restarted := NewInDir(dir)
	require.NoError(t, restarted.Load())
	require.False(t, restarted.HasBlobOwner(alice, aliceBlob.ID))
	require.False(t, restarted.HasBlobOwner(bob, aliceBlob.ID))
}
...
func TestDropOwnerBindingKeepsReferencedOrIndependentlyRetainedPayload(t *testing.T) {
	...
	for name, retain := range map[string]func(*Service, string) error{
		...
		t.Run(name, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.NoError(t, retain(service, blob.ID))
			_, err = service.DropBlobForOwner(owner, blob.ID)
			require.NoError(t, err)
...
func TestDropOwnerBindingRollsBackOnCatalogueFailure(t *testing.T) {
	...
	_, err = service.DropBlobForOwner(owner, blob.ID)
	require.Error(t, err)
	require.True(t, service.HasBlobOwner(owner, blob.ID))
	raw, err := service.GetBlobPayloadForOwner(owner, blob.ID)
	require.NoError(t, err)
...
func TestRemoteFetchNeverCreatesOwnerBindingAndCanRefillExistingBinding(t *testing.T) {
	...
	_, err = withoutBinding.FetchBlob(remote.ID, origin)
	require.NoError(t, err)
	require.False(t, withoutBinding.HasBlobOwner(contentTestOwner(0x55), remote.ID))
	...
	require.NoError(t, err)
	_, err = withBinding.DropBlob(owned.ID)
	require.NoError(t, err)
	require.True(t, withBinding.HasBlobOwner(owner, owned.ID))
	_, err = withBinding.FetchBlob(owned.ID, origin)
	require.NoError(t, err)
	raw, err := withBinding.GetBlobPayloadForOwner(owner, owned.ID)
	require.NoError(t, err)
