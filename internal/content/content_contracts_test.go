...
func TestContentContractsPreservePersistedReferenceFields(t *testing.T) {
	owner := contentTestOwner(0x36)
	objectWire, err := json.Marshal(Object{ID: "object-1", Owner: owner, BlobRefs: []Ref{{Kind: "blob", ID: "blob-1"}}})
	...
	manifestWire, err := json.Marshal(Manifest{ID: "manifest-1", Owner: owner, Refs: []Ref{{Kind: "blob", ID: "blob-1"}}})
	require.NoError(t, err)
