...
func TestPrincipalServerStreamReauthenticatesOnceBeforeFirstEvent(t *testing.T) {
	...
	client, err := New(Config{BaseURL: "unix://" + filepath.ToSlash(socket), Timeout: 5 * time.Second, ExpectedPrincipal: node, Signer: signer})
	require.NoError(t, err)
...
func TestPrincipalSessionOverUnixAuthenticatesAndRetriesOnce(t *testing.T) {
	...
	client, err := New(Config{BaseURL: "unix://" + filepath.ToSlash(socket), Timeout: time.Second, ExpectedPrincipal: node, Signer: signer})
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, client.Close()) })
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
