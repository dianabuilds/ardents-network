...
func TestResolvedBindingAcceptsOnlyCurrentSameTargetLineage(t *testing.T) {
	...
	binding, _, err := namelease.ResolveBinding(record, now.Unix(), nil)
	...
	renewedBinding, _, err := namelease.ResolveBinding(renewed, now.Add(time.Minute).Unix(), nil)
	...
	cases := map[string]func(*namelease.Record){
		"recovery pending": func(value *namelease.Record) { value.Recovery = "recovery-pending" },
		"released": func(value *namelease.Record) {
	...
	for name, mutate := range cases {
		t.Run(name, func(t *testing.T) {
			changed := renewed
			mutate(&changed)
			if _, _, err := namelease.ResolveBinding(changed, now.Add(time.Minute).Unix(), nil); err == nil {
