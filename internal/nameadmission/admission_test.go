...
func TestAdmissionVerifyFreezesEverySurfaceAndFailsClosedAcrossRestart(t *testing.T) {
	...
	restarted, err := nameadmission.NewAdmission([32]byte{1}, [32]byte{2}, 3, [32]byte{9})
	if err != nil {
		t.Fatal(err)
	}
...
func admissionGate(t testing.TB) *nameadmission.Admission {
	t.Helper()
	gate, err := nameadmission.NewAdmission([32]byte{1}, [32]byte{2}, 3, [32]byte{4})
	if err != nil {
		t.Fatal(err)
	}
	return gate
...
func admissionProof(t testing.TB, gate *nameadmission.Admission, surface string, nonce byte) nameadmission.Proof {
	t.Helper()
