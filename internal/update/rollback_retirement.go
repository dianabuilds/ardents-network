...
func retireForNextGeneration(store *ownedStore, inspection rootInspection, generation uint64) (*ownedStore, rootInspection, error) {
	...
	next, _, err := acquireStore(store.root)
	if err != nil {
		return nil, rootInspection{}, err
	}
	inspection, err = next.inspect(generation, false)
	if err != nil {
		return nil, rootInspection{}, errors.Join(err, next.release())
	}
	return next, inspection, nil
}
