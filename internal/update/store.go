...
func acquireStore(root string) (*ownedStore, rootInspection, error) {
	...
	inspection, err = store.inspect(0, true)
	if err != nil {
...
// openTransaction admits one root, derives its next transaction generation
// from the inspected current selection when the caller has none, and then
// re-inspects the exact transaction candidate. It keeps storage admission and
// allowed transaction-directory inventory in the state owner.
func openTransaction(root string, requested uint64, artifact [sha256.Size]byte) (*ownedStore, rootInspection, uint64, error) {
	store, inspection, err := acquireStore(root)
	if err != nil {
		return nil, rootInspection{}, requested, err
	}
	generation := requested
	if generation == 0 {
		generation = ownedGeneration(inspection, artifact)
	}
	inspection, err = store.inspect(generation, false)
	if err != nil {
		return nil, rootInspection{}, generation, errors.Join(err, store.release())
	}
	return store, inspection, generation, nil
}
...
func (store *ownedStore) inspect(generation uint64, allowSuccessor bool) (rootInspection, error) {
	...
	seenTransactions := make(map[uint64]struct{})
	includeTransaction := func(transaction uint64) error {
		if transaction == 0 {
			return nil
		}
		if _, seen := seenTransactions[transaction]; seen {
			return nil
		}
		seenTransactions[transaction] = struct{}{}
		if _, err := os.Lstat(store.generationPath("transactions", transaction)); err == nil {
			transactionNames = append(transactionNames, strconv.FormatUint(transaction, 10))
			return nil
		} else if !errors.Is(err, os.ErrNotExist) {
			return err
		}
	...
	if err := includeTransaction(selection.Transaction); err != nil {
		return inspection, err
	}
	if err := includeTransaction(generation); err != nil {
		return inspection, err
	}
	if allowSuccessor && selection.Transaction < ^uint64(0) {
		if err := includeTransaction(selection.Transaction + 1); err != nil {
			return inspection, err
		}
	}
