...
func fixedNativeRoleConfigs(runID string, hops map[string]roleHop, slot handle, leafSHA256, payloadSeed string, payloadBytes int, fault string, workload *nativeWorkload) map[string]roleConfig {
	if workload != nil && workload.Profile == workloadDirect {
		return fixedDirectRoleConfigs(runID, slot, leafSHA256, payloadSeed, payloadBytes, workload)
	}
	if workload != nil && workload.Profile == workloadC3 {
		return fixedC3RoleConfigs(runID, hops, slot, leafSHA256, payloadSeed, payloadBytes, workload)
	}
	...
	configs["user"] = roleConfig{
		...
		ExpectedLeafSHA256: leafSHA256, PayloadSeed: payloadSeed, PayloadBytes: payloadBytes, Fault: fault,
	}
	...
	if workload != nil && workload.Kind == "stream" {
		user := configs["user"]
		user.PayloadSeed, user.PayloadBytes = "", 0
		user.StreamDirection, user.StreamSeed, user.StreamDuration = workload.Direction, workload.Seed, workload.DurationSeconds
		configs["user"] = user
		service := configs["service"]
		service.StreamDirection, service.StreamSeed, service.StreamDuration = workload.Direction, workload.Seed, workload.DurationSeconds
...
func prepareNativeToolConfigs(fixture *nativeFixture, runID string, topology nativeTopology) error {
	...
	if topology.profile == "c3" {
		captures = map[string][]fixtureToolLink{
			"user":               {{Name: "c3-user-entry", Peer: "user-entry"}},
			"user-entry":         {{Name: "c3-entry-rendezvous", Peer: "rendezvous"}, {Name: "c3-user-introduction", Peer: "introduction-node"}},
			"rendezvous":         {{Name: "c3-rendezvous-entry", Peer: "data-service-entry"}},
			"data-service-entry": {{Name: "c3-entry-service", Peer: "service"}, {Name: "c3-service-introduction", Peer: "introduction-node"}},
		}
	}
	if topology.profile == "direct" {
		captures = map[string][]fixtureToolLink{"user": {{Name: "direct-link", Peer: "service", CaptureAll: true}}}
	}
