...
func TestPendingGenerationRequiresSignedActivationBeforeItBecomesReady(t *testing.T) {
	...
	require.NoError(t, err)
	require.False(t, reopened.GenerationReadiness(next.ChannelID).Ready)
	active, err = reopened.ConfirmGenerationRuntimeAdoption(activation)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	thirdActive, err = reopened.ConfirmGenerationRuntimeAdoption(thirdActivation)
	require.NoError(t, err)
