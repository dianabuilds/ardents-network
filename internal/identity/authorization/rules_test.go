...
func TestRequireChecksAuthenticationAndCapabilities(t *testing.T) {
	unauthorized := Require(Subject{}, "node", AccessRead)
	require.False(t, unauthorized.Allowed)
	require.Equal(t, "unauthorized", unauthorized.Code)

	forbidden := Require(Subject{
		Authenticated: true,
		Capabilities:  []string{"node:write"},
	}, "node", AccessRead)
	require.False(t, forbidden.Allowed)
	require.Equal(t, "forbidden", forbidden.Code)

	allowed := Require(Subject{
		Authenticated: true,
		Capabilities:  []string{"node:read"},
	}, "node", AccessRead)
