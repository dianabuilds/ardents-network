...
func prepareNodeStorage(options NodeOptions) (protectedNodeStorage, error) {
	...
	key, err := loadOrCreateKey(storage.storeKeyPath, storage.capabilityStore)
	if err != nil {
		return protectedNodeStorage{}, err
	}
	if _, err := loadOrCreateKey(storage.replayKeyPath, storage.discoveryReplay); err != nil {
		return protectedNodeStorage{}, err
	}
...
func writePrivate(path string, raw []byte) error {
	temp, err := os.CreateTemp(filepath.Dir(path), ".ardents-provision-*")
	if err != nil {
		return err
	}
	name := temp.Name()
