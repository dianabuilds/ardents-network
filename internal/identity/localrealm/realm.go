...
func (a *Authority) ProvisionNode(options NodeOptions, admission identityapi.CapabilityAdmission) (NodeProvision, error) {
	...
	state := noderecovery.NewStoreInDir(options.DataDir)
	if err := state.Load(); err != nil {
		return NodeProvision{}, fmt.Errorf("load canonical node identity state: %w", err)
	}
	summary, _, err := identitylifecycle.New().EnsureNode(
		state, identitycontinuity.NewKeyStoreInDir(options.DataDir),
	)
	if err != nil {
...
func (a *Authority) provisionSubject(options NodeOptions, subject string, admission identityapi.CapabilityAdmission, now time.Time) (NodeProvision, error) {
	...
	storage, err := prepareNodeStorage(options)
	if err != nil {
		return NodeProvision{}, err
	}
	service, err := identitycapability.NewService(storage.capabilityStore, storage.storeKey, subject,
	...
	record, err := a.loadOrCreateNodeState(storage.recordPath, subject, issuer, now)
	if err != nil {
		return NodeProvision{}, err
	}
	...
	return NodeProvision{
		...
		DiscoveryRef: discoveryRef, DataRef: dataRef, StoreKeyPath: storage.storeKeyPath,
		ReplayKeyPath: storage.replayKeyPath, CapabilityStore: storage.capabilityStore,
		DiscoveryReplay: storage.discoveryReplay, DataReplay: storage.dataReplay,
	}, nil
}
...
func loadOrCreateKey(path, protectedState string) ([]byte, error) {
	...
	if err := writePrivate(path, []byte(base64.StdEncoding.EncodeToString(key))); err != nil {
		return nil, err
	}
	return key, nil
}
