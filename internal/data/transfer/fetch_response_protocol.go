...
func storeAcceptedBlobResponse(cfg ExchangeConfig, response blobFetchResponse, trustResult discovery.TrustResult) (model.Blob, string, int64, error) {
	...
	_, err = cfg.Data.ObserveBlobSource(stored.ID, appdata.BlobSourceRecord{
		NodeID:    response.Source,
		Trust:     appdata.SourceTrust{State: discovery.TrustStateForResult(trustResult), Outcome: trustResult.Outcome, Valid: trustResult.Valid, Trusted: trustResult.Trusted, Usable: trustResult.Usable},
	...
	if cfg.Diagnostics != nil {
		cfg.Diagnostics.RecordEvent("data", "blob_fetched", stored.ID, "blob fetched from trusted peer", "", map[string]any{"source": response.Source})
	}
