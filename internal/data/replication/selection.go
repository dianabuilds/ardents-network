...
func (s *Service) PlaceAvailable(ctx context.Context, blobID string, count int, intentVersion uint64) (PlacementOutcome, error) {
	...
	for _, candidate := range decision.Selected {
		commitment, err := s.PlaceBlob(ctx, blobID, candidate.NodeID, intentVersion)
		if err != nil {
			outcome.Decision.Denials = append(outcome.Decision.Denials, placement.Denial{NodeID: candidate.NodeID, Reason: safeReason(err)})
			continue
		}
		outcome.Commitments = append(outcome.Commitments, commitment)
	}
	if len(outcome.Commitments) < count {
		return outcome, placementUnsatisfiedError(outcome.Decision, len(outcome.Commitments), count)
	}
	return outcome, nil
}
...
func placementUnsatisfiedError(decision placement.SelectionDecision, committed, required int) error {
	...
	return fmt.Errorf("replica placement target count is unsatisfied: committed=%d required=%d denials=%s", committed, required, strings.Join(reasons, ","))
}

func (s *Service) existingReplicaPeers(blobID string, intentVersion uint64) map[string]bool {
	excluded := map[string]bool{}
	for _, commitment := range s.cfg.Data.ReplicaPlacementState().Commitments {
		if commitment.BlobID == blobID && commitment.IntentVersion == intentVersion && commitment.PeerID != "" {
			excluded[commitment.PeerID] = true
		}
	}
	return excluded
}
...
func (s *Service) candidateTargets() []string {
...
func (s *Service) observeCandidate(ctx context.Context, target string, blob appdata.Blob) placement.Candidate {
	candidate := placement.Candidate{NodeID: target, PolicyAllowed: s.cfg.Policy.AllowPeerBlobReserving(blobView(blob)) == nil}
...
func (s *Service) queryCapacity(ctx context.Context, target string, blob appdata.Blob) (capacityObservation, error) {
	...
	if result.Status != "available" || result.Capacity.NodeID != target || result.Capacity.ObservedAt.IsZero() {
