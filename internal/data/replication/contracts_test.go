...
func TestReplicaCapacityQueryRequiresBoundEncryptedBlobMetadata(t *testing.T) {
	valid := appdata.Blob{ID: "cid", CID: "cid", Hash: "hash", Cipher: datapayload.AES256GCMCipher, Size: 10, Encrypted: true}
	if !validReplicaBlobMetadata(valid) {
		t.Fatal("valid encrypted blob metadata was rejected")
	}
	for name, mutate := range map[string]func(*appdata.Blob){
		"plaintext":          func(blob *appdata.Blob) { blob.Encrypted = false },
		"wrong cid":          func(blob *appdata.Blob) { blob.CID = "other" },
		"missing hash":       func(blob *appdata.Blob) { blob.Hash = "" },
		"missing cipher":     func(blob *appdata.Blob) { blob.Cipher = "" },
		"unsupported cipher": func(blob *appdata.Blob) { blob.Cipher = "custom" },
		"empty size":         func(blob *appdata.Blob) { blob.Size = 0 },
	} {
...
func TestReplicaControlSignatureBindsRoutingAndBody(t *testing.T) {
	...
	source, err := identityapi.PrincipalFromPublicKey(encodedKey)
	if err != nil {
		t.Fatal(err)
	}
