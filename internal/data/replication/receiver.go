...
import (
	...
	privacy "ardents/internal/network/privacy"
)
...
func validReplicaBlobMetadata(blob appdata.Blob) bool {
...
func (s *Service) authorizePeer(peerID string, blob appdata.Blob, expiresAt time.Time) placement.PeerAuthorization {
...
func blobView(blob appdata.Blob) dataapi.BlobSnapshot {
	return dataapi.BlobSnapshot{
		ID: blob.ID, CID: blob.CID, MediaType: blob.MediaType, Size: blob.Size,
		Hash: blob.Hash, Cipher: blob.Cipher, KeyID: blob.KeyID, State: blob.State,
		Retention: blob.Retention, Encrypted: blob.Encrypted, ExpiresAt: blob.ExpiresAt,
		CreatedAt: blob.CreatedAt,
	}
}

func (s *Service) publishControl(ctx context.Context, action, operationID, target string, body any) error {
	identity := s.cfg.Identity.NodeSummary()
