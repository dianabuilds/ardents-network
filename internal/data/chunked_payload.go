...
func (s *Service) storeChunkStream(ctx context.Context, spec ChunkedPayloadSpec, reader io.Reader, key []byte) (chunkStoreOutcome, error) {
	...
	count, total, err := chunking.Stream(reader, func(_ int, plaintext []byte) error {
		if err := ctx.Err(); err != nil {
			return err
		}
		if len(outcome.IDs) >= chunking.MaxLeafRefs*chunking.MaxRootRefs {
...
func (s *Service) publishChunkManifests(stored chunkStoreOutcome, spec ChunkedPayloadSpec) (Manifest, []string, error) {
	plan, err := chunking.Plan(stored.IDs, chunking.ManifestSpec{
	...
	for _, leaf := range plan.Leaves {
		published, publishErr := s.PublishManifest(leaf)
		if published.ID != "" {
	...
	if plan.Root.ID != plan.Leaves[0].ID {
		published, publishErr := s.PublishManifest(plan.Root)
		if published.ID != "" {
			created = append(created, published.ID)
		}
		if publishErr != nil {
...
func (s *Service) storeStagedChunk(blob Blob, plaintext, key []byte, keyID string) (Blob, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	stored, err := blobpkg.StoreEncrypted(blob, plaintext, key, keyID, func(item Blob, ciphertext []byte) (Blob, error) {
		return blobpkg.Store(&s.blobs, item, ciphertext, s.nextID, s.writePayloadLocked)
	})
	if err != nil {
		return stored, err
	}
