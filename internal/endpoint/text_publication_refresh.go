...
// scheduled publication will consume. This uses the current admitted Source,
// existing allocation and retained receivers, never another bootstrap allowance.
func (owner *textContext) prepareTextRefreshSource(ctx context.Context, flight *textResolutionFlight) error {
	owner.mu.Lock()
	...
	if err != nil || ctx.Err() != nil || owner.resolution != flight || owner.prefix != flight.prefix || owner.permission == nil {
		owner.mu.Unlock()
		return errors.New("text refresh Source stock unavailable")
	}
