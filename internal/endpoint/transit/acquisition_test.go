package transit

import (
	"bytes"
	"context"
	"crypto/ed25519"
	"crypto/rand"
	"crypto/sha256"
	"crypto/tls"
	"encoding/json"
	"errors"
	"path/filepath"
	"testing"
	"time"

	"github.com/dianabuilds/ardents-network/internal/route"
	"github.com/dianabuilds/ardents-network/internal/route/credential"
)

func enrollAcquisitionForTest(raw []byte, certificate tls.Certificate) (func(), error) {
	grant, err := route.DecodeTransitGrant(raw)
	if err != nil {
		return nil, err
	}
	digest, err := route.ClientTLSKeyDigest(certificate.Leaf)
	if err != nil || digest != grant.ClientKeyDigest {
		return nil, errors.New("transit test TLS enrollment does not match its Grant")
	}
	return func() {}, nil
}

func (owner *Acquisition) stateForTest() transitAcquisitionState {
	owner.mu.Lock()
	defer owner.mu.Unlock()
	return owner.state
}

func TestTransitAcquisitionReconcilesAndBurnsAmbiguousPresentation(t *testing.T) {
	t.Parallel()
	now := time.Unix(2_000_000_000, 0).UTC()
	root := filepath.Join(t.TempDir(), "transit-acquisition")
	_, signer, err := ed25519.GenerateKey(rand.Reader)
	if err != nil {
		t.Fatal(err)
	}
	var signerPublic [32]byte
	copy(signerPublic[:], signer.Public().(ed25519.PublicKey))
	scope := Scope{NetworkID: acquisitionID(1), Digest: acquisitionID(2), Epoch: 3,
		IssuerNodeID: acquisitionID(4), IssuerPublicKey: acquisitionID(5), IssuerProfileDigest: acquisitionID(6),
		GrantSignerPublicKey: signerPublic, TransitNodeID: acquisitionID(7), TransitRole: route.IntroductionRole,
		NotAfter: now.Add(10 * time.Second)}

	owner, err := openAcquisition(Config{Root: root, Create: true, Clock: func() time.Time { return now }})
	if err != nil {
		t.Fatal(err)
	}
	pending, err := owner.begin(scope)
	if err != nil {
		t.Fatal(err)
	}
	if pending.Phase != transitPending || pending.Request.RequestID == [32]byte{} || pending.Request.AttachmentID == [32]byte{} ||
		pending.Request.ClientKeyDigest == [32]byte{} || pending.Certificate.PrivateKey == nil {
		t.Fatalf("pending acquisition = %+v", pending)
	}
	requestID, attachment, keyDigest := pending.Request.RequestID, pending.Request.AttachmentID, pending.Request.ClientKeyDigest
	private := append([]byte(nil), pending.Certificate.PrivateKey.(ed25519.PrivateKey)...)
	if err := owner.Close(); err != nil {
		t.Fatal(err)
	}

	owner, err = openAcquisition(Config{Root: root, Clock: func() time.Time { return now }})
	if err != nil {
		t.Fatal(err)
	}
	reconciled, err := owner.begin(scope)
	if err != nil {
		t.Fatal(err)
	}
	if reconciled.Request.RequestID != requestID || reconciled.Request.AttachmentID != attachment ||
		reconciled.Request.ClientKeyDigest != keyDigest || !bytes.Equal(reconciled.Certificate.PrivateKey.(ed25519.PrivateKey), private) {
		t.Fatalf("reconciled acquisition changed its one-use inputs: %+v", reconciled.Request)
	}
	grant := acquisitionGrant(t, scope, reconciled.Request, signer)
	if err := owner.commit(reconciled.Request.RequestID, credential.Result{Outcome: credential.Issued, Grant: grant}); err != nil {
		t.Fatal(err)
	}
	ready, err := owner.begin(scope)
	if err != nil || ready.Phase != transitReady || !bytes.Equal(ready.Grant, grant) ||
		!bytes.Equal(ready.Certificate.PrivateKey.(ed25519.PrivateKey), private) {
		t.Fatalf("ready acquisition = %+v, %v", ready, err)
	}
	if _, err := owner.present(reconciled.Request.RequestID, scope); err != nil {
		t.Fatal(err)
	}
	if err := owner.Close(); err != nil {
		t.Fatal(err)
	}

	owner, err = openAcquisition(Config{Root: root, Clock: func() time.Time { return now }})
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = owner.Close() })
	state := owner.stateForTest()
	if state.Phase != transitBurned || len(state.PrivateKey) != 0 || len(state.Certificate) != 0 || len(state.Grant) != 0 {
		t.Fatalf("ambiguous presentation recovery = %+v", state)
	}
}

func TestTransitCredentialLifecycleIgnoresStalePresentationCompletion(t *testing.T) {
	t.Parallel()
	now := time.Unix(2_000_001_000, 0).UTC()
	root := filepath.Join(t.TempDir(), "transit-acquisition")
	_, signer, err := ed25519.GenerateKey(rand.Reader)
	if err != nil {
		t.Fatal(err)
	}
	var signerPublic [32]byte
	copy(signerPublic[:], signer.Public().(ed25519.PublicKey))
	base := Scope{NetworkID: acquisitionID(61), Digest: acquisitionID(62), Epoch: 63,
		IssuerNodeID: acquisitionID(64), IssuerPublicKey: acquisitionID(65), IssuerProfileDigest: acquisitionID(66),
		GrantSignerPublicKey: signerPublic, TransitNodeID: acquisitionID(67), TransitRole: route.IntroductionRole,
		NotAfter: now.Add(10 * time.Second)}
	first := base
	first.AttachmentID = acquisitionID(68)
	second := base
	second.AttachmentID = acquisitionID(69)
	third := base
	third.AttachmentID = acquisitionID(70)

	owner, err := openAcquisition(Config{Root: root, Create: true, Clock: func() time.Time { return now }})
	if err != nil {
		t.Fatal(err)
	}
	defer owner.Close()
	acquire := func(scope Scope) (Lease, error) {
		return owner.Acquire(t.Context(), scope, func(_ context.Context, request credential.Request) (credential.Result, error) {
			return credential.Result{Outcome: credential.Issued, Grant: acquisitionGrant(t, scope, request, signer)}, nil
		}, enrollAcquisitionForTest)
	}

	acquiredFirst, err := acquire(first)
	if err != nil {
		t.Fatal(err)
	}
	if _, err := owner.begin(second); err == nil {
		t.Fatal("replacement acquisition did not invalidate the first attempt")
	}
	acquiredThird, err := acquire(third)
	if err != nil {
		t.Fatal(err)
	}
	thirdState := owner.stateForTest()
	if thirdState.Phase != transitPresenting || thirdState.RequestID != acquiredThird.Request.RequestID {
		t.Fatalf("third acquisition was not presenting: %+v", thirdState)
	}

	if err := acquiredFirst.Finish(true); err == nil {
		t.Fatal("stale first completion reported success")
	}
	if state := owner.stateForTest(); state.Phase != transitPresenting || state.RequestID != acquiredThird.Request.RequestID ||
		!bytes.Equal(state.Grant, acquiredThird.Grant) || len(state.PrivateKey) == 0 {
		t.Fatalf("stale completion changed the third acquisition: %+v", state)
	}
	if err := acquiredThird.Finish(true); err != nil {
		t.Fatalf("third completion failed: %v", err)
	}
	if state := owner.stateForTest(); state.Phase != transitSpent || state.RequestID != acquiredThird.Request.RequestID {
		t.Fatalf("third completion did not spend its own attempt: %+v", state)
	}
	if err := owner.Close(); err != nil {
		t.Fatal(err)
	}
	reopened, err := openAcquisition(Config{Root: root, Clock: func() time.Time { return now }})
	if err != nil {
		t.Fatal(err)
	}
	defer reopened.Close()
	if state := reopened.stateForTest(); state.Phase != transitSpent || state.RequestID != acquiredThird.Request.RequestID ||
		len(state.PrivateKey) != 0 || len(state.Grant) != 0 {
		t.Fatalf("reopened third completion = %+v", state)
	}
}

func TestTransitCredentialLifecycleIgnoresStaleIssuerError(t *testing.T) {
	t.Parallel()
	now := time.Unix(2_000_001_100, 0).UTC()
	root := filepath.Join(t.TempDir(), "transit-acquisition")
	_, signer, err := ed25519.GenerateKey(rand.Reader)
	if err != nil {
		t.Fatal(err)
	}
	var signerPublic [32]byte
	copy(signerPublic[:], signer.Public().(ed25519.PublicKey))
	base := Scope{NetworkID: acquisitionID(71), Digest: acquisitionID(72), Epoch: 73,
		IssuerNodeID: acquisitionID(74), IssuerPublicKey: acquisitionID(75), IssuerProfileDigest: acquisitionID(76),
		GrantSignerPublicKey: signerPublic, TransitNodeID: acquisitionID(77), TransitRole: route.IntroductionRole,
		NotAfter: now.Add(10 * time.Second)}
	first := base
	first.AttachmentID = acquisitionID(78)
	second := base
	second.AttachmentID = acquisitionID(79)
	third := base
	third.AttachmentID = acquisitionID(80)

	owner, err := openAcquisition(Config{Root: root, Create: true, Clock: func() time.Time { return now }})
	if err != nil {
		t.Fatal(err)
	}
	defer owner.Close()
	started := make(chan credential.Request, 1)
	release := make(chan struct{})
	firstResult := make(chan error, 1)
	go func() {
		_, err := owner.Acquire(t.Context(), first, func(_ context.Context, request credential.Request) (credential.Result, error) {
			started <- request
			<-release
			return credential.Result{}, errors.New("issuer stopped after the replacement")
		}, enrollAcquisitionForTest)
		firstResult <- err
	}()
	<-started
	if _, err := owner.begin(second); err == nil {
		t.Fatal("replacement acquisition did not invalidate the first attempt")
	}
	acquiredThird, err := owner.Acquire(t.Context(), third, func(_ context.Context, request credential.Request) (credential.Result, error) {
		return credential.Result{Outcome: credential.Issued, Grant: acquisitionGrant(t, third, request, signer)}, nil
	}, enrollAcquisitionForTest)
	if err != nil {
		t.Fatal(err)
	}
	close(release)
	if err := <-firstResult; !errors.Is(err, ErrStale) {
		t.Fatalf("stale issuer error = %v, want stale-attempt rejection", err)
	}
	if state := owner.stateForTest(); state.Phase != transitPresenting || state.RequestID != acquiredThird.Request.RequestID ||
		!bytes.Equal(state.Grant, acquiredThird.Grant) || len(state.PrivateKey) == 0 {
		t.Fatalf("stale issuer error changed the third acquisition: %+v", state)
	}
	if err := acquiredThird.Finish(true); err != nil {
		t.Fatalf("third completion failed: %v", err)
	}
}

func TestTransitAcquisitionPersistsFixedTerminalWithoutKey(t *testing.T) {
	t.Parallel()
	now := time.Unix(2_000_000_100, 0).UTC()
	root := filepath.Join(t.TempDir(), "transit-acquisition")
	public, _, err := ed25519.GenerateKey(rand.Reader)
	if err != nil {
		t.Fatal(err)
	}
	var signerPublic [32]byte
	copy(signerPublic[:], public)
	scope := Scope{NetworkID: acquisitionID(11), Digest: acquisitionID(12), Epoch: 13,
		IssuerNodeID: acquisitionID(14), IssuerPublicKey: acquisitionID(15), IssuerProfileDigest: acquisitionID(16),
		GrantSignerPublicKey: signerPublic, TransitNodeID: acquisitionID(17), TransitRole: route.IntroductionRole,
		NotAfter: now.Add(10 * time.Second)}
	owner, err := openAcquisition(Config{Root: root, Create: true, Clock: func() time.Time { return now }})
	if err != nil {
		t.Fatal(err)
	}
	pending, err := owner.begin(scope)
	if err != nil {
		t.Fatal(err)
	}
	if err := owner.commit(pending.Request.RequestID, credential.Result{Outcome: credential.Exhausted}); err != nil {
		t.Fatal(err)
	}
	if _, err := owner.begin(scope); !errors.Is(err, errTransitAcquisitionTerminal) {
		t.Fatalf("terminal acquisition begin error = %v", err)
	}
	state := owner.stateForTest()
	if state.Phase != transitExhausted || len(state.PrivateKey) != 0 || len(state.Certificate) != 0 || len(state.Grant) != 0 {
		t.Fatalf("terminal acquisition retained secret material: %+v", state)
	}
	successor := scope
	successor.Digest, successor.Epoch = acquisitionID(18), scope.Epoch+1
	if next, err := owner.begin(successor); err != nil || next.Phase != transitPending || next.Request.RequestID == state.RequestID {
		t.Fatalf("successor State acquisition = %+v, %v", next, err)
	}
	if err := owner.Close(); err != nil {
		t.Fatal(err)
	}
}

func TestTransitAcquisitionRootRequiresExplicitInitializationAndOneOwner(t *testing.T) {
	t.Parallel()
	root := filepath.Join(t.TempDir(), "transit-acquisition")
	clock := func() time.Time { return time.Unix(2_000_000_200, 0).UTC() }
	if _, err := openAcquisition(Config{Root: root, Clock: clock}); err == nil {
		t.Fatal("missing transit acquisition root was initialized implicitly")
	}
	owner, err := openAcquisition(Config{Root: root, Create: true, Clock: clock})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := openAcquisition(Config{Root: root, Clock: clock}); err == nil {
		t.Fatal("second transit acquisition owner opened the same root")
	}
	if err := owner.Close(); err != nil {
		t.Fatal(err)
	}
	reopened, err := openAcquisition(Config{Root: root, Clock: clock})
	if err != nil {
		t.Fatal(err)
	}
	if err := reopened.Close(); err != nil {
		t.Fatal(err)
	}
}

func TestTransitAcquisitionSetSeparatesRoleScopedLifecycles(t *testing.T) {
	t.Parallel()
	now := time.Unix(2_000_000_300, 0).UTC()
	_, signer, err := ed25519.GenerateKey(rand.Reader)
	if err != nil {
		t.Fatal(err)
	}
	var signerPublic [32]byte
	copy(signerPublic[:], signer.Public().(ed25519.PublicKey))
	base := Scope{NetworkID: acquisitionID(41), Digest: acquisitionID(42), Epoch: 43,
		IssuerNodeID: acquisitionID(44), IssuerPublicKey: acquisitionID(45), IssuerProfileDigest: acquisitionID(46),
		GrantSignerPublicKey: signerPublic, NotAfter: now.Add(10 * time.Second)}
	introductionScope := base
	introductionScope.TransitNodeID, introductionScope.TransitRole = acquisitionID(47), route.IntroductionRole
	responderScope := base
	responderScope.TransitNodeID, responderScope.TransitRole = acquisitionID(48), route.ResponderRole

	root := filepath.Join(t.TempDir(), "transit-acquisitions")
	owners, err := OpenSet(Config{Root: root, Create: true, Clock: func() time.Time { return now }})
	if err != nil {
		t.Fatal(err)
	}
	introduction, err := owners.introduction.begin(introductionScope)
	if err != nil {
		t.Fatal(err)
	}
	responder, err := owners.responder.begin(responderScope)
	if err != nil {
		t.Fatal(err)
	}
	if introduction.Request.TransitRole != route.IntroductionRole || introduction.Request.TransitNodeID != introductionScope.TransitNodeID ||
		responder.Request.TransitRole != route.ResponderRole || responder.Request.TransitNodeID != responderScope.TransitNodeID ||
		introduction.Request.RequestID == responder.Request.RequestID || introduction.Request.AttachmentID == responder.Request.AttachmentID {
		t.Fatalf("role-scoped acquisition requests overlap: introduction=%+v responder=%+v", introduction.Request, responder.Request)
	}
	if err := owners.introduction.commit(introduction.Request.RequestID, credential.Result{Outcome: credential.Issued,
		Grant: acquisitionGrant(t, introductionScope, introduction.Request, signer)}); err != nil {
		t.Fatal(err)
	}
	if err := owners.responder.commit(responder.Request.RequestID, credential.Result{Outcome: credential.Exhausted}); err != nil {
		t.Fatal(err)
	}
	if ready, err := owners.introduction.begin(introductionScope); err != nil || ready.Phase != transitReady {
		t.Fatalf("Introduction lifecycle did not remain ready: %+v, %v", ready, err)
	}
	if _, err := owners.responder.begin(responderScope); !errors.Is(err, errTransitAcquisitionTerminal) {
		t.Fatalf("Responder lifecycle did not retain its independent terminal outcome: %v", err)
	}
	if err := owners.Close(); err != nil {
		t.Fatal(err)
	}
}

func TestTransitAcquisitionReadsAcceptedV1IntroductionStateAsV2(t *testing.T) {
	t.Parallel()
	now := time.Unix(2_000_000_400, 0).UTC()
	root := filepath.Join(t.TempDir(), "transit-acquisition")
	_, signer, err := ed25519.GenerateKey(rand.Reader)
	if err != nil {
		t.Fatal(err)
	}
	var signerPublic [32]byte
	copy(signerPublic[:], signer.Public().(ed25519.PublicKey))
	scope := Scope{NetworkID: acquisitionID(51), Digest: acquisitionID(52), Epoch: 53,
		IssuerNodeID: acquisitionID(54), IssuerPublicKey: acquisitionID(55), IssuerProfileDigest: acquisitionID(56),
		GrantSignerPublicKey: signerPublic, TransitNodeID: acquisitionID(57), TransitRole: route.IntroductionRole,
		NotAfter: now.Add(10 * time.Second)}
	owner, err := openAcquisition(Config{Root: root, Create: true, Clock: func() time.Time { return now }})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := owner.begin(scope); err != nil {
		t.Fatal(err)
	}
	state := owner.stateForTest()
	previous := transitAcquisitionStateV1{Schema: transitAcquisitionSchemaV1, Phase: state.Phase,
		NetworkID: state.NetworkID, Digest: state.Digest, Epoch: state.Epoch, IssuerNodeID: state.IssuerNodeID,
		IssuerPublicKey: state.IssuerPublicKey, IssuerProfileDigest: state.IssuerProfileDigest,
		GrantSignerPublicKey: state.GrantSignerPublicKey, IntroductionNodeID: state.TransitNodeID,
		NotAfter: state.NotAfter, RequestID: state.RequestID, AttachmentID: state.AttachmentID,
		ClientKeyDigest: state.ClientKeyDigest, Certificate: state.Certificate, PrivateKey: state.PrivateKey}
	raw, err := json.Marshal(previous)
	if err != nil {
		t.Fatal(err)
	}
	if err := replaceTransitAcquisitionState(root, append(raw, '\n')); err != nil {
		t.Fatal(err)
	}
	if err := owner.Close(); err != nil {
		t.Fatal(err)
	}
	owner, err = openAcquisition(Config{Root: root, Clock: func() time.Time { return now }})
	if err != nil {
		t.Fatal(err)
	}
	defer owner.Close()
	reconciled, err := owner.begin(scope)
	if err != nil || reconciled.Request.TransitRole != route.IntroductionRole || reconciled.Request.TransitNodeID != scope.TransitNodeID {
		t.Fatalf("accepted v1 state did not reconcile as role-scoped v2: %+v, %v", reconciled.Request, err)
	}
}

func acquisitionGrant(t *testing.T, scope Scope, request credential.Request, private ed25519.PrivateKey) []byte {
	t.Helper()
	raw, err := issueTransitGrantFixture(route.TransitGrant{IssuerID: sha256.Sum256(private.Public().(ed25519.PublicKey)), GrantID: request.RequestID,
		NetworkID: scope.NetworkID, Digest: scope.Digest, AttachmentID: request.AttachmentID,
		TransitNodeID: scope.TransitNodeID, ClientKeyDigest: request.ClientKeyDigest,
		Epoch: scope.Epoch, TransitRole: scope.TransitRole, NotAfter: scope.NotAfter}, private)
	if err != nil {
		t.Fatal(err)
	}
	return raw
}

func acquisitionID(marker byte) [32]byte { return [32]byte{marker} }
