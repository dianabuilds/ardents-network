...
func (s *store) verifySourceBundle(bundle sourceBundle, current *Snapshot, currentDecision *candidateDecision) (candidateDecision, error) {
	if len(bundle.materials) != 1 || bundle.materials[0].Index != s.config.material {
	...
	if current != nil && parsed.number == current.Epoch && parsed.digest == current.Digest {
		...
		if err := verifyMaterializations(parsed, currentDecision.accepted, bundle.materials, true); err != nil {
			return candidateDecision{}, err
		}
	...
	if s.pendingDecision != nil && parsed.number == s.pendingDecision.epoch.number && parsed.digest == s.pendingDecision.epoch.digest {
		...
		if err := verifyMaterializations(parsed, s.pendingDecision.accepted, bundle.materials, true); err != nil {
			return candidateDecision{}, err
		}
