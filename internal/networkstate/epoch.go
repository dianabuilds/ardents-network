...
func verifyEpoch(config config, current *Snapshot, raw []byte) (epochEnvelope, error) {
	epoch, err := parseEpoch(raw)
	if err != nil {
		return epochEnvelope{}, err
	}
	if epoch.networkID != config.networkID {
		return epochEnvelope{}, errors.New("epoch network identity is wrong")
	}
	if epoch.number > maximumEpochChain {
		return epochEnvelope{}, errors.New("epoch exceeds the S1-0 retained chain bound")
	}
	if config.now.Before(epoch.validFrom) || !config.now.Before(epoch.validUntil) {
		return epochEnvelope{}, errors.New("epoch is not strictly current")
	}
	...
	for _, signed := range epoch.signatures {
		...
		if !known {
			return epochEnvelope{}, errors.New("epoch has an unknown signer")
		}
		if !ed25519.Verify(public, epoch.digest[:], signed.signature) {
			return epochEnvelope{}, errors.New("epoch signature is invalid")
		}
		valid++
	}
	if valid < config.threshold {
		return epochEnvelope{}, fmt.Errorf("epoch has %d valid signatures, need %d", valid, config.threshold)
	}
	return epoch, nil
}
