...
func TestRefreshWaitsForTwoAuthenticatedSourcesAndRestarts(t *testing.T) {
	...
	endpoint, err := networkstate.Open(endpointConfig)
	if err != nil {
		t.Fatal(err)
	}
	...
	endpointConfig.SourceAddresses = addresses
	endpointConfig.SourceServerNames = [2]string{"source-one.test", "source-two.test"}
	endpointConfig.SourceIdentities = [2][32]byte{sha256.Sum256([]byte("source-one")), sha256.Sum256([]byte("source-two"))}
	endpointConfig.SourceFamilies = [2]string{"source-family-one", "source-family-two"}
	endpointConfig.SourceEndpointHandles = [2]string{"source-handle-one", "source-handle-two"}
	endpointConfig.SourceRootPEM = [2][]byte{firstAuthority.rootPEM, secondAuthority.rootPEM}
	endpointConfig.SourceLeafKeyDigests = [2][32]byte{firstServer.pin, secondServer.pin}
	endpointConfig.SourceClientCertificate = client.certificate
	endpointConfig.SourceOrderSeed = sha256.Sum256([]byte("fixed-source-order"))
	...
	restarted, err := networkstate.Open(endpointConfig)
	if err != nil {
	...
	if _, err := networkstate.Open(endpointConfig); err == nil || !strings.Contains(err.Error(), "lacks its current pointer") {
...
func TestRefreshPersistsObservedConflictAcrossRestart(t *testing.T) {
	...
	defer closeSources()
	endpoint, err := networkstate.Open(config)
	if err != nil {
		t.Fatal(err)
	}
...
func TestRefreshPersistsTLSFailureBackoff(t *testing.T) {
	...
	defer closeSources()
	config.SourceLeafKeyDigests[0] = sha256.Sum256([]byte("wrong-source-one-pin"))
	config.SourceLeafKeyDigests[1] = sha256.Sum256([]byte("wrong-source-two-pin"))
	endpoint, err := networkstate.Open(config)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSourceClientTransportKeyCannotReuseEpochSigner(t *testing.T) {
	...
	config.SourceClientCertificate = tls.Certificate{Certificate: [][]byte{raw}, PrivateKey: genesis.authorityPrivate}
	if _, err := networkstate.Open(config); err == nil || !strings.Contains(err.Error(), "separate from Epoch signer") {
...
func sourceEnvironment(t *testing.T, genesis, firstValue, secondValue fixture) (networkstate.Config, func()) {
	t.Helper()
	now := time.Unix(genesis.now, 0).UTC()
	...
	installed, err := networkstate.Open(config)
	if err != nil {
		t.Fatal(err)
	}
	...
	config.SourceAddresses = addresses
	config.SourceServerNames = [2]string{"first-source.test", "second-source.test"}
	config.SourceIdentities = [2][32]byte{sha256.Sum256([]byte("first-source")), sha256.Sum256([]byte("second-source"))}
	config.SourceFamilies = [2]string{"first-source-family", "second-source-family"}
	config.SourceEndpointHandles = [2]string{"first-source-handle", "second-source-handle"}
	config.SourceRootPEM = [2][]byte{firstAuthority.rootPEM, secondAuthority.rootPEM}
	config.SourceLeafKeyDigests = [2][32]byte{firstServer.pin, secondServer.pin}
	config.SourceClientCertificate = client.certificate
	config.SourceOrderSeed = sha256.Sum256([]byte("source-environment-order"))
	return config, func() { _ = first.Close(); _ = second.Close() }
}

func fixtureConfig(value fixture, root string, now time.Time) networkstate.Config {
...
func openTestSourceAtIndex(t *testing.T, value fixture, address string, server testCertificate, clientRoot []byte, clientPin [32]byte, materialIndex uint32) interface{ Close() error } {
	...
	config.ServeAddress = address
	config.ServeCertificate = server.certificate
	config.ServeClientRootPEM = clientRoot
	config.ServeClientKeyDigests = [][32]byte{clientPin}
	config.SourceMaterializationIndex = materialIndex
	served, err := networkstate.Open(config)
	if err != nil {
		t.Fatal(err)
	}
	return served
