...
func (control *control) Submit(submission Submission, proof Proof) string {
	...
	if err := control.store.AppendPending(operation.Name, submission.raw, operation.SuccessorRecord, now.UnixMilli()); err != nil {
		return "denied"
	}
	control.records[updated.Name] = updated
	return "submitted"
...
func (control *control) restore() error {
	records, cursor, err := control.store.CurrentRecords()
	if err != nil {
		return errors.New("name Authority control state is tampered")
	}
	for name, record := range records {
		control.records[name] = record
	}
