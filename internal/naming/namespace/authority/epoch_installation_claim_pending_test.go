...
func pendingClaimWinner(t *testing.T, network [32]byte, epoch uint64, name string,
	...
	proof := ClaimProof{Network: network, Epoch: epoch, Rule: "ardents-name-claim-order-v1", CutoffOffset: 2,
		InputRoot: authorityClaimInputLeaf(claim), InputLength: 1,
		MaterializationRoot: authorityClaimMaterializationLeaf([]Claim{claim}), MaterializationLength: 1,
		RejectionRoot: sha256.Sum256([]byte{2}), Claims: []Claim{claim}}
	order := ClaimOrder{Network: network, Rule: "ardents-name-claim-order-v1", MinimumEpoch: epoch,
