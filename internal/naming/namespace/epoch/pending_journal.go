...
// PendingEntry is one immutable durable successor selected for a later Epoch.
type PendingEntry struct {
	Sequence   uint64
	DecisionAt int64
	Submission []byte
...
func (store *Store) appendPending(name string, submission, successor []byte, decisionAt int64) (pendingEntry, error) {
...
// AppendPending durably records one Authority-validated successor.
func (store *Store) AppendPending(name string, submission, successor []byte, decisionAt int64) error {
	_, err := store.appendPending(name, submission, successor, decisionAt)
	return err
}

// Pending returns copied immutable pending journal entries.
func (store *Store) Pending() ([]PendingEntry, error) {
	entries, err := store.pending()
	if err != nil {
		return nil, err
	}
	values := make([]PendingEntry, len(entries))
	for index, entry := range entries {
		values[index] = PendingEntry{Sequence: entry.sequence, DecisionAt: entry.decisionAt,
			Submission: append([]byte(nil), entry.submission...), Successor: append([]byte(nil), entry.successor...)}
	}
	return values, nil
}
