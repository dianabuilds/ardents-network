...
func TestPublishRequiresFutureValidityInsideTheFullLeaseLineage(t *testing.T) {
	t.Parallel()
	policy := namespace.Policy{DefaultLeaseDuration: time.Hour, DefaultGraceDuration: time.Hour}
	root, err := namespace.Apply(nil, 100, namespace.Op{Kind: "claim", Name: "root", Generation: 1,
		Authority: "root-authority", LeaseDuration: time.Minute}, policy)
	if err != nil {
		t.Fatal(err)
	}
	child, err := namespace.Apply(nil, 101, namespace.Op{Kind: "claim", Name: "child.root", Generation: 1,
		Authority: "child-authority", Parents: []namespace.Record{root}}, policy)
	if err != nil {
		t.Fatal(err)
	}
	grandchild, err := namespace.Apply(nil, 102, namespace.Op{Kind: "claim", Name: "grandchild.child.root", Generation: 1,
