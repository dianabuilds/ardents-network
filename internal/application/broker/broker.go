...
func New(config Config) (*Broker, error) {
	...
	grants := make(map[string][32]byte, len(config.Grants))
	for _, grant := range config.Grants {
		...
		grants[grant.Surface] = grant.Principal
	}
	if config.Clock == nil {
		config.Clock = time.Now
	}
	return &Broker{id: config.ID, grants: grants, sessions: make(map[[32]byte]session, maximumSessions), clock: config.Clock}, nil
}

// Admit issues one short-lived, one-use capability for an exact Principal and
// granted surface.
func (broker *Broker) Admit(principal [32]byte, surface string) ([32]byte, error) {
	broker.mu.Lock()
	defer broker.mu.Unlock()
	if broker.closed {
		return [32]byte{}, errors.New("broker is draining")
	}
	if expected, ok := broker.grants[surface]; !ok || expected != principal {
...
func (broker *Broker) Consume(capability, principal [32]byte, surface string) (Receipt, error) {
	broker.mu.Lock()
	defer broker.mu.Unlock()
...
// Drain atomically refuses new admission and invalidates every unconsumed
// capability. It does not claim to stop work that has already consumed one.
...
func (broker *Broker) Isolation() IsolationObservation {
	return IsolationObservation{State: "generic/unqualified"}
}
...
func grantCommitment(id, principal [32]byte, surface string) [32]byte {
