...
func TestClientCloseInterruptsBlockedWrite(t *testing.T) {
	...
	application, err := Dial(context.Background(), path, Request{Destination: TargetLink, Value: "ardents-target:v1:test"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestClientContextCancellationInterruptsBlockedWrite(t *testing.T) {
	...
	application, err := Dial(ctx, path, Request{Destination: TargetLink, Value: "ardents-target:v1:test"})
	if err != nil {
		t.Fatal(err)
	}
	cleanupClient(t, application)
...
func TestClientVerifiedRemoteOutcomePrecedesLaterClose(t *testing.T) {
	...
	if err := writeClientPeerTerminal(peer, want); err != nil {
		t.Fatal(err)
	}
