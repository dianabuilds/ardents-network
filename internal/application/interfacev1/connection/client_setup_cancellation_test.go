...
func TestDialCancellationInterruptsSetupAndClosesTransport(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			go func() {
				_, dialErr := Dial(ctx, path, "ardents-target:v1:cancel-setup")
				result <- dialErr
			}()
...
func readSetupRequest(peer *net.UnixConn) (string, error) {
	...
	link := make([]byte, int(binary.BigEndian.Uint16(header[len(localMagic):])))
...
func TestDialTransfersCancellationToSuccessfulClient(t *testing.T) {
	...
	defer cancel()
	application, err := Dial(ctx, path, "ardents-target:v1:transferred-client")
	if err != nil {
		t.Fatal(err)
	}
...
func TestDialCancellationRacesAcceptedStatusWithoutLeakingClient(t *testing.T) {
	for attempt := range 50 {
		...
		go func() {
			client, dialErr := Dial(ctx, path, "ardents-target:v1:handoff-race")
			result <- struct {
				client Client
				err    error
...
func TestDialDoesNotOpenTransportForAlreadyCanceledContext(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	client, err := Dial(ctx, path, "ardents-target:v1:already-canceled")
