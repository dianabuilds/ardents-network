...
type config struct {
	...
	S41Evidence, Stage3Evidence, Slice                 string
...
func parseConfig(arguments []string) (config, error) {
	...
	if value.Slice == "s4.2" && (value.S41Evidence == "" || value.Stage3Evidence == "") {
		return config{}, errors.New("S4.2 requires explicit passed S4.1 and Stage 3 evidence")
	}
	return value, nil
}
