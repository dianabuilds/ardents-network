...
func independentlyVerifyDomains(epoch offlineEpoch, families map[string][2]uint32) error {
	...
	for family, summary := range families {
		domain, err := independentlyAssignedDomain(epoch, family)
		if err != nil {
			return err
		}
	...
	for _, expected := range epoch.domains {
		...
		if uint16(actual[0]) != expected.count || actual[1] != expected.capacity {
			return errors.New("independent role domain summaries disagree")
		}
	}
	return nil
}

func independentlyAssignedDomain(epoch offlineEpoch, family string) (string, error) {
	var selected string
	var selectedDigest [32]byte
	for index, domain := range epoch.domains {
		digest := independentAssignmentDigest(epoch, family, domain.id)
		if index > 0 && digest == selectedDigest {
			return "", errors.New("independent role assignment digest tie")
		}
...
func independentAssignmentDigest(epoch offlineEpoch, family, domain string) [32]byte {
