...
type independentRejection struct {
	index uint32
	code  uint16
	raw   []byte
}

func independentlyEvaluate(input OfflineCase, epoch offlineEpoch, inputs [][]byte) ([]verifiedRecord, []independentRejection) {
	items := make([]independentItem, len(inputs))
	for index, raw := range inputs {
		record, err := decodeVerifiedRecord(raw)
		items[index] = independentItem{index: index, record: record}
		switch {
		case err != nil:
			items[index].code = 1
	...
	independentlyMarkCollisions(items)
	accepted := make([]verifiedRecord, 0, len(items))
	rejected := make([]independentRejection, 0, len(items))
	for _, item := range items {
		if item.code == 0 {
			accepted = append(accepted, item.record)
		} else {
			rejected = append(rejected, independentRejection{uint32(item.index), item.code, inputs[item.index]})
		}
	}
...
func decodeVerifiedRecord(raw []byte) (verifiedRecord, error) {
	...
	if record.generation, err = d.u64(); err != nil || record.generation == 0 {
	...
	if record.capability, err = d.one(); err != nil {
		return verifiedRecord{}, err
	}
	if record.endpoint, err = d.text(96); err != nil {
		return verifiedRecord{}, err
	}
	if record.capacity, err = d.u16(); err != nil {
		return verifiedRecord{}, err
	}
	public, err := d.take(ed25519.PublicKeySize)
	if err != nil {
		return verifiedRecord{}, err
	}
	record.publicKey = append(ed25519.PublicKey(nil), public...)
	record.keyID = sha256.Sum256(public)
	record.signature, err = d.take(ed25519.SignatureSize)
	if err != nil || d.offset != len(raw) {
...
func authorityKeyCollision(input OfflineCase, key [32]byte) bool {
	_, exists := input.Authorities[key]
	return exists
}

func independentlyMarkCollisions(items []independentItem) {
	...
	for _, item := range items {
		...
		generations[independentGeneration(item.record)]++
	...
	for index := range items {
		...
		case nodes[item.record.nodeID] > 1:
			item.code = 8
		case generations[independentGeneration(item.record)] > 1:
			item.code = 9
...
func independentGeneration(record verifiedRecord) string {
	return string(record.nodeID[:]) + fmt.Sprintf("/%d", record.generation)
}

func independentlyVerifyView(epoch offlineEpoch, accepted []verifiedRecord, rejected []independentRejection) error {
	...
	for index, item := range rejected {
		rejectionLeaves[index] = rejectedLeaf(item.index, item.code, item.raw)
	}
	...
	if uint32(len(view)) != epoch.viewLength || recordRoot(view, 0x11) != epoch.viewRoot ||
		...
		return errors.New("independent candidate view or rejection root disagrees")
	}
	if epoch.eligibleCount != uint32(len(view)) || epoch.eligibleCapacity != totalCapacity ||
		...
		return errors.New("independent candidate summaries disagree")
	}
	if err := independentlyVerifyDomains(epoch, families); err != nil {
		return err
	}
	return nil
}
