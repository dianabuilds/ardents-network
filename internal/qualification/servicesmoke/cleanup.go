...
func removePrivateFixture(root string) error {
	...
	if !filepath.IsAbs(root) {
		return errors.New("stage 3 private fixture cleanup requires an absolute path")
	}
	marker, err := os.ReadFile(filepath.Join(root, ".ardents-owned"))
	...
	if err != nil || string(marker) != fixtureMarker {
		return errors.New("stage 3 private fixture cleanup ownership is invalid")
	}
	...
	if _, err := os.Stat(root); !errors.Is(err, os.ErrNotExist) {
		return errors.New("stage 3 private fixture remains after cleanup")
	}
	return nil
}
