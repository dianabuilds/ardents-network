...
func verifyDomains(epoch verifiedEpoch, families map[string][2]uint32) error {
	...
	for _, expected := range epoch.domains {
		...
		if uint16(actual[0]) != expected.count || actual[1] != expected.capacity {
			return errors.New("role domain summaries disagree")
		}
	}
	return nil
}

func assignedDomain(epoch verifiedEpoch, family string) (string, error) {
	var selected string
...
func assignmentDigest(epoch verifiedEpoch, family, domain string) [32]byte {
