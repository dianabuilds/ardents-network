...
type Registry struct {
	mu        sync.Mutex
	items     []ServiceSpec
	dynamic   map[string]ServiceSpec
...
func New(specs []ServiceSpec) *Registry {
	return NewWithReadiness(specs, hostingreadiness.NewController(hostingreadiness.DefaultPolicy()))
}

func NewWithReadiness(specs []ServiceSpec, controller *hostingreadiness.Controller) *Registry {
	items := make([]ServiceSpec, 0, len(specs))
	...
	return &Registry{items: items, dynamic: map[string]ServiceSpec{}, observed: map[string]Backing{}, readiness: controller}
}

func (r *Registry) List() []ServiceSpec {
	r.mu.Lock()
	defer r.mu.Unlock()
	out := make([]ServiceSpec, 0, len(r.items)+len(r.dynamic))
...
func cloneSpec(spec ServiceSpec) ServiceSpec {
