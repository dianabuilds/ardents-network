...
func TestLoadReconcilesAncillaryRuntimeForCurrentInstance(t *testing.T) {
	...
	require.NoError(t, first.Register(Spec{ID: "work.ingress", Kind: "service", Owner: "node", Desired: DesiredRunning}))
...
func (e *ancillaryRecoveryExecutor) Prepare(_ context.Context, spec Spec) (PreparedWorkload, error) {
	return PreparedWorkload{WorkloadID: spec.ID, Generation: 1, PreparedAt: time.Now().UTC()}, nil
}
