...
func (s *Service) Snapshot() []Status {
	s.mu.Lock()
	defer s.mu.Unlock()
	return snapshotStatusesLocked(s.items)
}
...
func (s *Service) Restore(ctx context.Context, snapshot []Status) error {
	...
	for _, item := range s.items {
		...
		if ok && targetItem.Spec.Desired == DesiredRunning {
			continue
		}
...
func (s *Service) recoverLoadedProcessesLocked() error {
	...
	for id, item := range s.items {
		...
		if inspectNotFound(inspectErr) && execution.ProcessMatchesConfig(item.Instance.PID, item.Spec.Config) {
			s.items[id] = recoveredRunning(item, now)
			continue
		}
...
func recoveredStopped(item Status, current execution.Instance, inspectErr error, now time.Time) Status {
	item.Instance.Running = false
	item.Instance.PID = 0
	if item.Instance.FinishedAt.IsZero() {
		item.Instance.FinishedAt = now
	}
	if item.Spec.Desired == DesiredRunning {
...
func recoveryFailureReason(err error, current execution.Instance, expectedGeneration int64) string {
