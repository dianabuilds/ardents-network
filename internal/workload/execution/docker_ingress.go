...
func (e *DockerExecutor) admitIngress(requests []IngressRequest) ([]IngressBinding, error) {
	bindings := make([]IngressBinding, 0)
...
func (e *DockerExecutor) admitProxyImage() error {
...
func (e *DockerExecutor) admitIngressPair(mode, advertised, probe string) (IngressBinding, error) {
	...
	if err != nil || !isLoopbackHost(probeURL.Hostname()) {
		return IngressBinding{}, fmt.Errorf("hosted service probe endpoint must use loopback")
	}
	if publicURL.Scheme != probeURL.Scheme || publicURL.Port() != probeURL.Port() ||
		...
		return IngressBinding{}, fmt.Errorf("hosted service endpoint pair does not match")
	}
	port64, err := strconv.ParseUint(probeURL.Port(), 10, 16)
	if err != nil || port64 < 1024 {
		return IngressBinding{}, fmt.Errorf("hosted service ingress port must be between 1024 and 65535")
	}
	...
	return IngressBinding{Port: uint16(port64), ProbeHost: probeURL.Hostname(), BindAddress: bindAddress}, nil
}
...
func (e *DockerExecutor) admitNetworkBind(advertisedHost string) (string, error) {
...
func (e *DockerExecutor) workloadIngressOptions(prepared PreparedWorkload) containerIngressOptions {
...
func (e *DockerExecutor) proxyIngressOptions(prepared PreparedWorkload) containerIngressOptions {
...
func dockerPortBindings(binding IngressBinding) []networkapi.PortBinding {
...
func (e *DockerExecutor) workloadNetworkName(prepared PreparedWorkload) string {
	digest := sha256.Sum256([]byte(fmt.Sprintf("%s\x00%s\x00%d", e.nodeID, prepared.WorkloadID, prepared.Generation)))
	return fmt.Sprintf("ardents-workload-%x", digest[:6])
}

func (e *DockerExecutor) ensureIngressNetworks(ctx context.Context, prepared PreparedWorkload) error {
...
func (e *DockerExecutor) ensureNetwork(ctx context.Context, name string, internal bool, labels map[string]string) error {
