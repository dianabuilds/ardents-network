...
func TestRegisterPresentWorkloadRemainsAccepted(t *testing.T) {
	...
	}
	{
		...
		err := svc.Register(workloadregistry.Spec{ID: "work.echo", Kind: "service", Owner: "node", Desired: workloadregistry.DesiredPresent})
...
func TestUnsupportedKindFailsAdmission(t *testing.T) {
	svc := workloadcontroller.New(filepath.Join(t.TempDir(), "ardents.db"), workloadcontroller.NewLocalExecutor())
	{
		err := svc.Register(workloadregistry.Spec{ID: "work.bad", Kind: "unknown", Owner: "node", Desired: workloadregistry.DesiredRunning})
		require.NoErrorf(t, err, "register: %v", err)
	}
	{
...
func TestRepeatedStartFailureBecomesFailed(t *testing.T) {
	...
	{
		err := svc.Register(workloadregistry.Spec{ID: "work.fail", Kind: "service", Owner: "node", Desired: workloadregistry.DesiredRunning})
		require.NoErrorf(t, err, "register: %v", err)
	}

	for i := range workloadcontroller.DefaultRestartBudget + 1 {
		{
			err := svc.Reconcile(context.Background())
...
func TestInspectFailureDegradesWithoutRestart(t *testing.T) {
	...
	{
		err := svc.Register(workloadregistry.Spec{
			ID:      "work.inspect",
			Kind:    "service",
			Owner:   "node",
...
func TestRegisterRejectsDuplicateWorkloadID(t *testing.T) {
	svc := workloadcontroller.New(filepath.Join(t.TempDir(), "ardents.db"), workloadcontroller.NewLocalExecutor())
	spec := workloadregistry.Spec{ID: "work.echo", Kind: "service", Owner: "node", Desired: workloadregistry.DesiredPresent}
	{
...
func TestLoadRejectsPidReuseWhenProcessDoesNotMatchConfig(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		require.True(t, killErr == nil || errors.Is(killErr, os.ErrProcessDone), "kill helper process: %v", killErr)
		_, waitErr := cmd.Process.Wait()
		var exitErr *exec.ExitError
		require.True(t, waitErr == nil || errors.As(waitErr, &exitErr), "wait for helper process: %v", waitErr)
	})
...
func (failExecutor) Prepare(context.Context, workloadcontroller.Request) (workloadcontroller.PreparedWorkload, error) {
...
func (e *inspectFailureExecutor) Prepare(context.Context, workloadcontroller.Request) (workloadcontroller.PreparedWorkload, error) {
	return workloadcontroller.PreparedWorkload{
