...
func TestDockerIngressUsesOnlyAdmittedInternalNetworkPorts(t *testing.T) {
	...
	bindings, err := executor.admitIngress([]IngressRequest{{Mode: "NetworkPublished",
		Endpoints: []string{"tcp://10.0.0.2:19000"}, ProbeEndpoints: []string{"tcp://127.0.0.1:19000"}}})
	require.NoError(t, err)
	require.Equal(t, []IngressBinding{{Port: 19000, ProbeHost: "127.0.0.1", BindAddress: "10.0.0.2"}}, bindings)

	prepared := PreparedWorkload{WorkloadID: "work", Generation: 1, Ingress: bindings}
...
func TestDockerIngressAdmissionFailsClosed(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "unpaired", request: IngressRequest{Mode: "NetworkPublished", Endpoints: []string{"tcp://10.0.0.2:19000"}}, contains: "paired"},
		{name: "wrong host", request: IngressRequest{Mode: "NetworkPublished", Endpoints: []string{"tcp://10.0.0.3:19000"}, ProbeEndpoints: []string{"tcp://127.0.0.1:19000"}}, contains: "not allowed"},
		{name: "dns host", request: IngressRequest{Mode: "NetworkPublished", Endpoints: []string{"tcp://node.example:19000"}, ProbeEndpoints: []string{"tcp://127.0.0.1:19000"}}, contains: "literal"},
		{name: "privileged port", request: IngressRequest{Mode: "NetworkPublished", Endpoints: []string{"tcp://10.0.0.2:443"}, ProbeEndpoints: []string{"tcp://127.0.0.1:443"}}, contains: "between 1024"},
		{name: "remote probe", request: IngressRequest{Mode: "NetworkPublished", Endpoints: []string{"tcp://10.0.0.2:19000"}, ProbeEndpoints: []string{"tcp://10.0.0.2:19000"}}, contains: "loopback"},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, err := executor.admitIngress([]IngressRequest{test.request})
...
func TestDockerIngressRejectsDuplicateHostPort(t *testing.T) {
	executor := ingressTestExecutor()
	_, err := executor.admitIngress([]IngressRequest{{Mode: "NetworkPublished",
...
func ingressTestExecutor() *DockerExecutor {
	return &DockerExecutor{allowedIngressHosts: normalizedSet([]string{"10.0.0.2"}),
