...
func encodeIngressLabel(bindings []IngressBinding) string {
	if len(bindings) == 0 {
		return ""
	}
...
func decodeIngressLabel(encoded string) ([]IngressBinding, error) {
	if encoded == "" {
		return nil, nil
	}
...
func (e *DockerExecutor) ReconcileAncillary(ctx context.Context, current []Instance) error {
	active := make(map[string]Instance, len(current))
...
func (e *DockerExecutor) recoverInstanceIngress(ctx context.Context, instance Instance) error {
	...
	prepared := PreparedWorkload{WorkloadID: instance.WorkloadID, Generation: instance.Generation, Ingress: bindings}
