...
func TestStartNewRemovesCreatedContainerAfterStartFailure(t *testing.T) {
	...
	executor := &DockerExecutor{client: engine, nodeID: "node-1", stopTimeout: time.Second}

	_, err = executor.startNew(t.Context(), containerSpec{Image: testDigestImage}, PreparedWorkload{WorkloadID: "workload-1", Generation: 1})
