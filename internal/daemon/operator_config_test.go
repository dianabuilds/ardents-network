...
func TestLoadRuntimeConfigUsesVersionedOperatorDocument(t *testing.T) {
	...
	require.Equal(t, 100000, cfg.Node.Transport.Limits.StoreMaxMessages)
	require.Equal(t, 7*24*60*60, cfg.Node.Transport.Limits.StoreMaxAgeSeconds)
	require.Equal(t, int64(2<<30), cfg.Node.Transport.Limits.StoreMaxBytes)
