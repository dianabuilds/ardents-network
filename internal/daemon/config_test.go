...
func TestLoadRuntimeConfigDefaults(t *testing.T) {
	...
	require.Equal(t, "ardentsd", cfg.Node.Name)
	require.Equal(t, filepath.Join("var", "ardentsd"), cfg.Node.Data.Dir)
	require.Equal(t, filepath.Join("var", "ardentsd", "waku-store.db"), cfg.Node.Transport.StorePath)
...
func TestLoadRuntimeConfigRejectsInvalidProfile(t *testing.T) {
	...
	_, err := loadRuntimeConfig()
	require.Error(t, err)
	require.Contains(t, err.Error(), "network.transport_profile")
}
