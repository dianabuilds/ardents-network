...
// TestApplyRejectsMoreThanOneActiveWorkBeforeRootInspection fixes the first
// S7.2-04 admission boundary through the public Apply seam.
func TestApplyRejectsMoreThanOneActiveWorkBeforeRootInspection(t *testing.T) {
	...
	if err == nil || result.Outcome != "transaction-invalid" || result.State != "transaction-invalid" ||
		result.Generation != vector.Request.TransactionGeneration || result.CurrentDigest != [32]byte{} ||
		result.RollbackDigest != [32]byte{} || result.StagingPresent || result.SafeNotice != "update transaction invalid" ||
		result.CustodyNotice != "" {
		t.Fatalf("Apply active-work refusal result=%+v err=%v", result, err)
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if err == nil || result.Outcome != "drain-expired" || result.State != test.wantState ||
				...
				result.CustodyNotice != vector.Expected.CommandResult.CustodyNotice {
