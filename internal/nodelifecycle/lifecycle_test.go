...
func TestRunServesBoundProbeThenWithdrawsOnRecordRemoval(t *testing.T) {
	...
	fixture.config.Current = func() (networkstate.Snapshot, error) {
		fixture.mu.RLock()
	...
	if connection, err := tls.Dial("tcp", fixture.config.ListenAddress, probeClientTLS(fixture)); err == nil {
		_ = connection.Close()
...
func TestDrainCancelsEstablishedProbeAtDeadline(t *testing.T) {
	fixture := newLifecycleFixture(t)
	fixture.config.DrainTimeout = 30 * time.Millisecond
	events := make(chan Event, 16)
	fixture.config.Current = func() (networkstate.Snapshot, error) {
		fixture.mu.RLock()
		defer fixture.mu.RUnlock()
...
func TestRunFailsBeforeReadinessOnKeyMismatch(t *testing.T) {
	...
	fixture.config.Current = func() (networkstate.Snapshot, error) { return fixture.snapshot, nil }
	...
	if connection, dialErr := net.DialTimeout("tcp", fixture.config.ListenAddress, 50*time.Millisecond); dialErr == nil {
		_ = connection.Close()
...
func TestUnexpectedListenerTerminationIsObservable(t *testing.T) {
	fixture := newLifecycleFixture(t)
	fixture.config.Current = func() (networkstate.Snapshot, error) { return fixture.snapshot, nil }
	fixture.config.Emit = func(context.Context, Event) error { return nil }
	...
	server, err := startProbeServer(config, fixture.snapshot)
	if err != nil {
		t.Fatal(err)
	}
	...
	select {
	case terminal := <-server.terminal:
		if terminal == nil {
			t.Fatal("unexpected listener close reported normal termination")
		}
	case <-time.After(time.Second):
		t.Fatal("listener termination was hidden")
	}
...
func newLifecycleFixture(t *testing.T) *lifecycleFixture {
	...
	snapshot := networkstate.Snapshot{Generation: "generation-1", NetworkID: [32]byte{1}, Epoch: 1,
	...
	fixture.config = Config{NetworkID: snapshot.NetworkID, NodeID: snapshot.NodeID, IdentityKey: identityPrivate,
		ListenAddress: address, Certificate: server.certificate, ClientRootPEM: ca.pem,
		ClientKeyPins: [][32]byte{sha256.Sum256(pinBytes)}, PollInterval: 10 * time.Millisecond,
		MaximumDuty: time.Second, DrainTimeout: 100 * time.Millisecond, Quarantine: time.Millisecond,
...
func dialProbe(t *testing.T, fixture *lifecycleFixture) *tls.Conn {
	...
	for {
		connection, err := tls.Dial("tcp", fixture.config.ListenAddress, probeClientTLS(fixture))
		if err == nil {
			return connection
		}
...
func encodeProbeRequest(snapshot networkstate.Snapshot, nonce [32]byte, payload []byte) []byte {
	request := make([]byte, probeHeaderBytes+probePayloadBytes)
	copy(request, "ARNP")
	request[4] = 1
	offset := 5
	for _, value := range [][32]byte{snapshot.NetworkID, probeProfile, snapshot.Digest, snapshot.NodeID, snapshot.AssignmentDigest, nonce} {
		copy(request[offset:offset+32], value[:])
		offset += 32
	}
	binary.BigEndian.PutUint16(request[offset:], probePayloadBytes)
	digest := sha256.Sum256(payload)
	copy(request[probeHeaderBytes:], digest[:])
	return request
}
