...
const (
	...
	descriptorPrefix  = "ardents-reachability-descriptor-v1\x00"
)

// Issue creates the only Descriptor v1 encoding for one current Publication
// and live slot. It verifies the current Publication before granting the
// Instance signer authority to issue a descriptor.
func Issue(input IssueInput) ([]byte, Descriptor, error) {
	current, err := verifiedCurrent(input.Current)
	if err != nil || input.InstanceSigner == nil || !validIntroduction(input.Introduction, current.Credential.NotAfter) {
...
// Verify decodes one closed Descriptor v1 and proves that it names expected
// Target under the declared Network at the supplied decision time.
...
func validIntroduction(value Introduction, credentialNotAfter int64) bool {
	return value.StateDigest != [32]byte{} && value.Epoch != 0 && value.IntroductionNodeID != [32]byte{} &&
		...
		len(value.SubmissionAuthorization) > 0 && len(value.SubmissionAuthorization) <= maximumAuthorization
}
...
func encodeBody(value Descriptor) ([]byte, error) {
	...
	var version [2]byte
	binary.BigEndian.PutUint16(version[:], descriptorVersion)
...
func decode(raw []byte) (Descriptor, []byte, error) {
	...
	descriptor := Descriptor{NetworkID: fields[0], Target: fields[1], AuthorityPublic: fields[2], PublicationDigest: fields[3],
