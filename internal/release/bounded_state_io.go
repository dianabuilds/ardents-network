...
func readBoundedFloorFile(path string, bound int64) ([]byte, error) {
	...
	if !before.Mode().IsRegular() || before.Mode()&os.ModeSymlink != 0 {
		return nil, fmt.Errorf("release: %s is not a regular file", path)
	}
	if before.Size() > bound {
		return nil, fmt.Errorf("release: %s exceeds the bound", path)
	}
	...
	if statErr != nil || !after.Mode().IsRegular() || !os.SameFile(before, after) {
		return nil, errors.Join(errors.New("release: state file changed while it was opened"), statErr, file.Close())
	}
...
func readFloorStoreDirectory(path string, bound int) (result []os.DirEntry, resultErr error) {
	...
	if !before.IsDir() || before.Mode()&os.ModeSymlink != 0 {
		return nil, errors.New("release: state directory is not a directory")
	}
	...
	if statErr != nil || !after.IsDir() || !os.SameFile(before, after) {
		return nil, errors.Join(errors.New("release: state directory changed while it was opened"), statErr)
	}
	...
	for {
		...
		if len(entries) > bound {
			return nil, fmt.Errorf("release: %s exceeds the entry bound", path)
		}
...
func requireRegularFile(path string) error {
	...
	if !info.Mode().IsRegular() || info.Mode()&os.ModeSymlink != 0 {
		return errors.New("release: state entry is not a regular file")
	}
	return nil
}
