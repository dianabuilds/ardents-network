...
func TestR049FrozenPublicVector(t *testing.T) {
	...
	verifier, err := Open(filepath.Join(t.TempDir(), "release-state"))
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() {
	...
	if !ok {
		t.Fatal("accepted decision did not return authorization")
	}
	decision.Path = "mutated-after-verification"
	authorized, ok := authorization.AcceptedDecision()
	if !ok || authorized.Path != expected.TargetPath {
		t.Fatalf("authorization did not retain verified decision: %+v, ok=%v", authorized, ok)
	}
}
