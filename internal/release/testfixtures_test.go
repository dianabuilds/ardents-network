...
// release package would receive from the offline-import
// caller.
...
func (store *memoryStore) ReadFloors() (FloorSet, error) {
	if store.closed {
		return FloorSet{}, fmt.Errorf("release: memory store is closed")
	}
...
func (store *memoryStore) CommitFloors(floors FloorSet, _ [][]byte) error {
	if store.closed {
		return fmt.Errorf("release: memory store is closed")
	}
