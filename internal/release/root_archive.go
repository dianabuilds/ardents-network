...
func validateStoredRootArchive(directory string, floors FloorSet) error {
	...
	if err != nil {
		return fmt.Errorf("release: read root archive: %w", err)
	}
	...
	for _, entry := range entries {
		if !entry.Type().IsRegular() || !strings.HasSuffix(entry.Name(), ".root.json") {
			return errors.New("release: root archive contains an invalid entry")
		}
		nameVersion := rootVersionFromName(entry.Name())
		if nameVersion == 1<<63-1 {
			return errors.New("release: root archive filename is invalid")
		}
		...
		if err != nil || root.Signed.Version != nameVersion {
			return errors.New("release: root archive filename disagrees with its bytes")
		}
