...
func TestPublicationPlanSeparatesAllowedAndDeniedServices(t *testing.T) {
	...
	allowed, denied := publicationGatePlan([]hostingregistry.ServiceStatus{
		{Spec: hostingservice.ServiceSpec{ID: "svc.allowed", Type: "echo", Owner: "work.a", Mode: "NetworkPublished",
			Endpoints: []string{"http://10.0.0.2:9000/ready"}, ProbeEndpoints: []string{"http://127.0.0.1:9000/ready"}}, Readiness: ready},
		{Spec: hostingservice.ServiceSpec{ID: "svc.denied", Type: "admin", Owner: "work.a", Mode: "NetworkPublished",
			Endpoints: []string{"http://10.0.0.2:9001/ready"}, ProbeEndpoints: []string{"http://127.0.0.1:9001/ready"}}, Readiness: ready},
		{Spec: hostingservice.ServiceSpec{ID: "svc.inactive", Type: "debug", Owner: "work.a", Mode: "NetworkPublished",
		...
		{Spec: hostingservice.ServiceSpec{ID: "svc.not-exposure-eligible", Type: "debug", Owner: "work.a", Mode: "NetworkPublished",
		...
		{Spec: hostingservice.ServiceSpec{ID: "svc.local", Type: "local", Mode: "LocalOnly", Endpoints: []string{"unix:///tmp/local.sock"}}, Readiness: ready},
	}, networkreadiness.ReachabilitySnapshot{Mode: networkreadiness.ReachabilityPrivateLAN, State: "lan", Reachable: true}, func(spec hostingservice.ServiceSpec) error {
...
func TestPublicationPlanRejectsNetworkCapabilityLoss(t *testing.T) {
	ready := hostingreadiness.Snapshot{State: hostingreadiness.StateReady, Ready: true, ExposureEligible: true}
	items := []hostingregistry.ServiceStatus{{Spec: hostingservice.ServiceSpec{ID: "svc.net", Type: "echo", Mode: "NetworkPublished",
		Endpoints: []string{"http://10.0.0.2:9000/ready"}, ProbeEndpoints: []string{"http://127.0.0.1:9000/ready"}}, Readiness: ready}}
...
func TestPublicationPlanRequiresEndpointScopeToMatchReachabilityMode(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			items := []hostingregistry.ServiceStatus{{Spec: hostingservice.ServiceSpec{ID: "svc", Type: "echo", Mode: "NetworkPublished",
...
func TestStaticHostedServiceStatusHasNoRuntimeBackedPublication(t *testing.T) {
	status := staticHostedServiceStatus(hostingservice.ServiceSpec{
		ID:        "svc.static",
		Type:      "echo",
		Owner:     "manual",
		Mode:      "LocalOnly",
