...
func SignTransition(network [32]byte, current namelease.Record, op namelease.Op,
...
func TransitionDigest(network [32]byte, current namelease.Record, op namelease.Op) ([32]byte, error) {
	transcript, err := transitionTranscript(network, current, op)
	if err != nil {
		return [32]byte{}, err
	}
...
func ApplyAdmittedTransition(admission *namespace.Admission, admissionProof namespace.Proof,
	...
	if err != nil || !allowed || admission == nil || admissionProof.Challenge.Surface != surface ||
		...
		return namelease.Record{}, errors.New("invalid Name Authority transition admission")
	}
	if accepted, _ := admission.Verify(admissionAt, admissionProof); !accepted {
		return namelease.Record{}, errors.New("denied Name Authority transition admission")
	}
	if recoveryThresholdTransition(op.Kind) {
		if len(proof) != 0 || op.Authority != "" {
			return namelease.Record{}, errors.New("recovery transition contains an authority bypass")
		}
...
func applyTransition(network [32]byte, current namelease.Record, op namelease.Op,
	proof []byte, now int64, policy namelease.Policy,
) (namelease.Record, error) {
	public, err := canonicalAuthority(current.Authority)
	if err != nil || len(proof) != ed25519.SignatureSize || !supportedTransition(current, op) {
		return namelease.Record{}, errors.New("invalid Name Authority transition proof")
	}
	transcript, err := transitionTranscript(network, current, op)
	if err != nil || !ed25519.Verify(public, transcript, proof) {
		return namelease.Record{}, errors.New("invalid Name Authority transition signature")
	}
	if op.Kind == "claim" {
		return namelease.Apply(nil, now, op, policy)
	}
	return namelease.Apply(&current, now, op, policy)
}

func supportedTransition(current namelease.Record, op namelease.Op) bool {
	switch op.Kind {
...
func transitionTranscript(network [32]byte, current namelease.Record, op namelease.Op) ([]byte, error) {
	currentWire, err := namelease.EncodeRecord(current)
	if err != nil {
	...
	for _, parent := range op.Parents {
		wire, encodeErr := namelease.EncodeRecord(parent)
		if encodeErr != nil {
