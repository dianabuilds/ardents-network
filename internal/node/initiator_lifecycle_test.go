...
func TestRunKeepsInitiatorReadyWhileItsEntryLedgerIsHeld(t *testing.T) {
	...
	snapshot := dutyFacts{Generation: "initiator-live", NetworkID: [32]byte{1}, Epoch: 2, Digest: [32]byte{3}, Profile: route.Profile,
		...
		RecordValidFrom: now.Add(-time.Second), RecordValidUntil: until, ValidUntil: until, ProbeEndpoint: availableLoopbackEndpoint(t), CarrierProfile: string(route.CarrierTCP),
		Assignment: "initiator", AssignmentDigest: [32]byte{5}, CandidateCount: 3, Candidates: [64]dutyCandidate{
			...
			{NodeID: [32]byte{10}, PublicKey: [32]byte{11}, Endpoint: "127.0.0.1:34110", CarrierProfile: string(route.CarrierQUIC), Assignment: "rendezvous", ValidFrom: now.Add(-time.Second), ValidUntil: until},
	...
	if ready.CarrierProfile != string(route.CarrierQUIC) {
		t.Fatalf("Initiator READY selected Rendezvous Carrier Profile = %q", ready.CarrierProfile)
	}
	...
	select {
	case terminal := <-result:
		if terminal.State != "WITHDRAWN" || terminal.CarrierProfile != string(route.CarrierQUIC) {
