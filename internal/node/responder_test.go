...
func TestResponderDutyUsesOneStateRendezvousPeer(t *testing.T) {
	...
	profile := ResponderProfile{Certificate: certificate, Admit: func([]byte, [32]byte, [32]byte, byte, [32]byte, time.Time) (route.EndpointTransitAdmission, error) {
		return route.EndpointTransitAdmission{}, nil
	},
		HandshakeLimit: 2, RelayLimit: 1, RelayByteLimit: 1024, DrainTimeout: time.Second}
	plan, err := responderDuty(profile, snapshot)
	...
	if _, err := responderDuty(profile, snapshot); err == nil {
