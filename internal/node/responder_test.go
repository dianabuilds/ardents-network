...
func TestResponderDutyUsesOneStateRendezvousPeer(t *testing.T) {
	...
	snapshot := dutyFacts{NetworkID: [32]byte{31}, Epoch: 32, Digest: [32]byte{33}, Profile: route.Profile, NodeID: [32]byte{34}, NodePublicKey: public,
		...
		Candidates: [64]dutyCandidate{{NodeID: [32]byte{35}, PublicKey: [32]byte{36}, Endpoint: "127.0.0.1:30262", CarrierProfile: string(route.CarrierQUIC), Assignment: "rendezvous", ValidFrom: now.Add(-time.Second), ValidUntil: now.Add(time.Minute)}}, CandidateCount: 1}
	profile := ResponderProfile{Certificate: certificate,
		HandshakeLimit: 2, RelayLimit: 1, RelayByteLimit: 1024, AdmissionTimeout: time.Second, DrainTimeout: time.Second}
