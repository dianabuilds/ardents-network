...
func publishFloorGeneration(generations, name string, payload []byte, roots []rootArchiveEntry) error {
	...
	if err := syncDirectory(staging); err != nil {
		return cleanupStagedGeneration(staging, fmt.Errorf("releasedecision: sync staged generation: %w", err))
	}
	final := filepath.Join(generations, name)
	if err := durableRename(staging, final); err != nil {
		return cleanupStagedGeneration(staging, fmt.Errorf("releasedecision: publish generation: %w", err))
	}
	if err := syncDirectory(generations); err != nil {
		return fmt.Errorf("releasedecision: sync generations directory: %w", err)
	}
	return nil
}
...
func writeFloorStorePointer(root, name string) (resultErr error) {
	...
	if err := syncDirectory(root); err != nil {
		return fmt.Errorf("releasedecision: sync state-root pointer: %w", err)
	}
	return nil
}
