...
func acquireFloorStoreLease(root string) (*os.File, error) {
	...
	if err != nil {
		if errors.Is(err, os.ErrExist) {
			return nil, errors.New("releasedecision: state root already has an active or unrecovered lease")
		}
		return nil, fmt.Errorf("releasedecision: acquire state-root lease: %w", err)
	}
	if _, err := lock.WriteString(floorStoreLeaseContents); err != nil {
		return nil, abandonFloorStoreLease(lock, path, fmt.Errorf("releasedecision: write state-root lease: %w", err))
	}
	if err := lock.Sync(); err != nil {
		return nil, abandonFloorStoreLease(lock, path, fmt.Errorf("releasedecision: sync state-root lease: %w", err))
	}
	return lock, nil
}
