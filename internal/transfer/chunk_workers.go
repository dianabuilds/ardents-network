// Package transfer owns authenticated resumable peer content exchange.
// It does not own local content ownership, replica policy, or raw carrier lifecycle.
package transfer
...
type chunkFetchFunc func(context.Context, string) (model.Blob, error)
...
func chunkWorker(
	...
	for id := range jobs {
		...
		if completed%16 == 0 || completed == int64(len(plan.ChunkIDs)) {
			if err := updateTransferProgress(cfg.History, transferID, current, encryptedPlanBytes(plan), "verified manifest chunks"); err != nil {
				select {
				case errorsFound <- err:
				default:
				}
