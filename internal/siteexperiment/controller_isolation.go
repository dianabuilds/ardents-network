...
func runActiveApplicationProbes(ctx context.Context, process *referenceProcess, roleIDs map[string]string) (result error) {
	...
	for _, role := range []string{"http-client", "http-application"} {
		...
		command := exec.CommandContext(ctx, "docker", "exec", containerID, "/usr/local/bin/reference-site-lab", "probe", "--kind", "application", "--observer-name", "gatec-observer", "--observer-address", observerAddress)
