...
func toDiscoveryRecord(in discoveryapi.DiscoveryRecord) *ardentsv1.DiscoveryRecord {
	return &ardentsv1.DiscoveryRecord{
		...
		PublicKey: in.PublicKey, Endpoints: append([]string(nil), in.Endpoints...), IssuedAt: ts(in.IssuedAt), ExpiresAt: ts(in.ExpiresAt), Signature: in.Signature, Source: in.Source,
	}
}

func fromDiscoveryRecord(in *ardentsv1.DiscoveryRecord) discoveryapi.DiscoveryRecord {
	if in == nil {
	...
	return discoveryapi.DiscoveryRecord{
		...
		PublicKey: in.GetPublicKey(), Endpoints: append([]string(nil), in.GetEndpoints()...), IssuedAt: fromTS(in.GetIssuedAt()), ExpiresAt: fromTS(in.GetExpiresAt()), Signature: in.GetSignature(), Source: in.GetSource(),
	}
}

func toDiscoveryResult(in discoveryapi.DiscoveryResult) *ardentsv1.DiscoveryResult {
