...
func TestConfigurationSurfaceReturnsRedactedEffectiveSnapshot(t *testing.T) {
	...
	server := &Server{configuration: stub, auth: configAuth("config.effective")}
...
func TestConfigurationReloadRequiresWriteAndReturnsOutcome(t *testing.T) {
	...
	server := &Server{configuration: stub, auth: configAuth("config.reload")}
...
func configAuth(capability string) AuthConfig {
	return AuthConfig{Token: "token", SubjectID: "operator", Capabilities: []string{capability}}
}
