...
func TestLedgerDurablyRefusesDuplicateAndRecoversPartialTail(t *testing.T) {
	...
	if _, err := Open(root, binding); err == nil {
	...
	recovered, err := Open(root, binding)
	if err != nil {
	...
	if _, err := Open(root, Binding{NetworkID: [32]byte{9}, ProfileDigest: binding.ProfileDigest, ReceiverNodeID: binding.ReceiverNodeID, ReceiverDutyGeneration: binding.ReceiverDutyGeneration}); err == nil {
		t.Fatal("rebound a receiver spend journal")
	}
}

func TestLedgerRecoveryClassifiesOneFinalCrashTail(t *testing.T) {
	binding := Binding{NetworkID: [32]byte{1}, ProfileDigest: [32]byte{2}, ReceiverNodeID: [32]byte{3}, ReceiverDutyGeneration: 4}
	window := time.Unix(1_800_000_000, 0).UTC().Truncate(time.Hour)
...
func TestLedgerRecoveryRefusesRepairFailure(t *testing.T) {
	binding := Binding{NetworkID: [32]byte{1}, ProfileDigest: [32]byte{2}, ReceiverNodeID: [32]byte{3}, ReceiverDutyGeneration: 4}
	repairErr := errors.New("truncate sync failed")
	calls := 0
	ledger, err := decodeLedgerWithRepair("journal", binding, append(encodeClosedSpendHeader(binding), 1), func(string, int64) error {
		calls++
		return repairErr
	})
...
func TestLedgerPrunesOnlyAfterWindowMargin(t *testing.T) {
	binding := Binding{NetworkID: [32]byte{1}, ProfileDigest: [32]byte{2}, ReceiverNodeID: [32]byte{3}, ReceiverDutyGeneration: 4}
	window := time.Unix(1_800_000_000, 0).UTC().Truncate(time.Hour)
