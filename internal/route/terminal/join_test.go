...
func TestClosedJoinOperationMatchesApprovedWire(t *testing.T) {
	request := JoinRequest{Nonce: [32]byte{0x11}, Secret: [32]byte{0x22}, Side: 2, Context: [32]byte{0x33}, Deadline: time.Unix(0x01020304, 0).UTC()}
	raw, err := EncodeJoinRequest(request)
	if err != nil {
		t.Fatal(err)
	}
	...
	decoded, err := DecodeJoinRequest(expected)
	...
	for _, mutate := range []func([]byte){
		...
		mutate(changed)
		if _, err := DecodeJoinRequest(changed); err == nil {
	...
	if _, err := EncodeJoinRequest(request); err == nil {
...
func TestClosedJoinResultsRequireLocalNonceAndEmptyPayload(t *testing.T) {
	...
	for status := uint8(0); status <= 4; status++ {
		raw, err := EncodeJoinResult(nonce, status)
		if err != nil || len(raw) != 16384 {
			t.Fatalf("result: %v", err)
		}
		if got, err := DecodeJoinResult(raw, nonce); err != nil || got != status {
			t.Fatalf("local result: %d %v", got, err)
		}
		if _, err := DecodeJoinResult(raw, [32]byte{8}); err == nil {
			t.Fatal("foreign nonce accepted")
		}
		for _, offset := range []int{33, 36, 37, 16383} {
			changed := bytes.Clone(raw)
			changed[offset] = 1
			if _, err := DecodeJoinResult(changed, nonce); err == nil {
				t.Fatal("nonempty length or padding accepted")
			}
		}
		if _, err := DecodeJoinResult(raw[:len(raw)-1], nonce); err == nil {
			t.Fatal("short result accepted")
		}
	}
	if _, err := EncodeJoinResult(nonce, 5); err == nil {
		t.Fatal("unsupported result status accepted")
	}
	if _, err := EncodeJoinResult([32]byte{}, 0); err == nil {
