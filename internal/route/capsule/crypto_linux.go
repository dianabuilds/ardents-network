...
// Seal encrypts the exact fixed recipient-only tuple with
// the selected Go HPKE suite. The digest binds the later Attachment exporter.
func Seal(input Capsule, recipient [32]byte, plaintext Plaintext) (Capsule, [32]byte, error) {
	if !validClosedIntroductionHeader(input) || input.Encapsulation != [32]byte{} || len(input.Ciphertext) != 0 || recipient == [32]byte{} ||
		...
		return Capsule{}, [32]byte{}, errors.New("closed Introduction sealing binding invalid")
	}
	raw, err := encodeClosedIntroductionPlaintext(plaintext)
	if err != nil {
		return Capsule{}, [32]byte{}, err
	}
	defer clear(raw)
	...
	if err != nil || len(enc) != 32 || len(ciphertext) != closedIntroductionCiphertextSize {
		return Capsule{}, [32]byte{}, errors.Join(errors.New("closed Introduction sealing failed"), err)
	}
...
// Open authenticates the profile and entire visible header
...
func Open(input Capsule, profile [32]byte, recipient Recipient, at time.Time) (Plaintext, [32]byte, error) {
	if !validClosedIntroductionHeader(input) || input.Encapsulation == [32]byte{} || len(input.Ciphertext) != closedIntroductionCiphertextSize ||
		...
		return Plaintext{}, [32]byte{}, errors.New("closed Introduction recipient input invalid")
	}
	...
	if err != nil || plaintext.ProfileDigest != profile || plaintext.Revision != input.Revision || !plaintext.Deadline.Equal(input.Expiry) {
		return Plaintext{}, [32]byte{}, errors.Join(errors.New("closed Introduction plaintext binding invalid"), err)
	}
