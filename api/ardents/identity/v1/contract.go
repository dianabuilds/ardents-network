...
var operatorActions = set(
	"node.start", "node.stop", "node.status", "node.features", "node.runtime", "node.events", "topology.node.fence", "config.effective", "config.reload", "transport.network_status", "transport.route_candidates",
	...
	"realm.authority.create", "realm.authority.migrate.local_v2", "realm.authority.rotate", "realm.authority.transition.adopt", "realm.authority.transition.finalize", "realm.channel.recovery.execute", "realm.channel.audit.read",
