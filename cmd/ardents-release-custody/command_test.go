...
func TestRunRejectsRetiredAssemblyRoutesBeforeSecretInput(t *testing.T) {
	for _, operation := range []string{"assemble", "assemble-successor"} {
		err := run(context.Background(), []string{operation}, &bytes.Buffer{}, unreadInput{})
		if err == nil || !strings.Contains(err.Error(), "usage") {
			t.Fatalf("run %q = %v", operation, err)
		}
	}
}
