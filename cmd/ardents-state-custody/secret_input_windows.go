...
var errDialogUnavailable = errors.New("functional alpha State graphical secret input is unavailable")

func newSecretInput() state.AlphaGenesisSecretInput {
	return dialogSecretInput{read: readWindowsPassphrase}
}

func (input dialogSecretInput) ReadSecret(ctx context.Context, prompt state.AlphaGenesisPrompt) ([]byte, error) {
	...
	message := "Create the local Ardents functional-alpha State passphrase."
	if prompt == state.AlphaGenesisConfirm {
		message = "Confirm the local Ardents functional-alpha State passphrase."
	}
