...
func (input terminalSecretInput) ReadServiceRequestCommitment(ctx context.Context) ([32]byte, error) {
	return input.readRequestCommitment(ctx, "service-request")
}

func (input terminalSecretInput) ReadAdmissionRequestCommitment(ctx context.Context) ([32]byte, error) {
	return input.readRequestCommitment(ctx, "admission-request")
}

func (input terminalSecretInput) readRequestCommitment(ctx context.Context, name string) ([32]byte, error) {
	if err := ctx.Err(); err != nil {
		return [32]byte{}, err
	}
	if input.terminal == nil || !term.IsTerminal(int(input.terminal.Fd())) {
		return [32]byte{}, errors.New(name + " commitment requires an interactive terminal")
	}
	fmt.Fprintf(input.prompts, "%s SHA-256 from the requesting Endpoint: ", name)
	line, err := bufio.NewReader(input.terminal).ReadString('\n')
	if err != nil {
		return [32]byte{}, fmt.Errorf("read %s commitment: %w", name, err)
	}
	text := strings.TrimSpace(line)
	decoded, err := hex.DecodeString(text)
	if err != nil || len(text) != 64 || len(decoded) != 32 || hex.EncodeToString(decoded) != text {
		return [32]byte{}, errors.New(name + " commitment must be lowercase SHA-256")
	}
