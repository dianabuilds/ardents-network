...
func provisionOperatorPrivacy(t *testing.T) runtimeconfig.Document {
	t.Helper()
	dir := t.TempDir()
	state := noderecovery.NewStoreInDir(dir)
	keys := identitycontinuity.NewKeyStoreInDir(dir)
	summary, _, err := identitylifecycle.New().EnsureNode(state, keys)
	require.NoError(t, err)
	...
	authority, err := identitycapability.NewService(
		...
		runtimeprocess.NewPolicyService(runtimeprocess.PolicyConfig{}), time.Now,
	)
	require.NoError(t, err)
	now := time.Now().UTC().Truncate(time.Second)
	discoveryRef := importOperatorGrant(t, authority, issuerPrivate, issuer, subject, 0x71, 0x81,
		identityapi.CapabilityRealmDiscovery, now)
	dataRef := importOperatorGrant(t, authority, issuerPrivate, issuer, subject, 0x72, 0x82,
