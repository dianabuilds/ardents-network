...
func TestRuntimeConfigRejectsUnknownOperatorAction(t *testing.T) {
	doc := runtimeconfig.Defaults()
	doc.API.Capabilities = []string{"node.make_coffee"}

	_, err := runtimeConfigFromDocument(doc, "operator-token")
	require.ErrorContains(t, err, "unknown connect api capability")
}
